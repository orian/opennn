...
void VectorTest::test_get_display(void) { message += "test_get_display\n"; }
...
void VectorTest::test_set_display(void) { message += "test_set_display\n"; }
...
void VectorTest::test_calculate_minimal_maximal_index(void) {
  ...
  Vector<unsigned int> minimal_maximal_index =
      v.calculate_minimal_maximal_index();
...
void VectorTest::test_get_assembly(void) {
  ...
  a.set(1, 0);
  b.set(0, 0), c = a.assemble(b);

  assert_true(c.size() == 1, LOG);

  a.set(0, 0);
  b.set(1, 0), c = a.assemble(b);
...
void VectorTest::test_calculate_less_rank(void) {
  ...
  assert_true(v.calculate_minimal_index() == rank.calculate_minimal_index(),
              LOG);
  assert_true(v.calculate_maximal_index() == rank.calculate_maximal_index(),
              LOG);
}
...
void VectorTest::test_calculate_greater_rank(void) {
  ...
  assert_true(v.calculate_minimal_index() == rank.calculate_maximal_index(),
              LOG);
  assert_true(v.calculate_maximal_index() == rank.calculate_minimal_index(),
              LOG);
}
...
void VectorTest::test_calculate_linear_regression_parameters(void) {
  ...
  assert_true(fabs(linear_regression_parameters.correlation - 1.0) < 1.0e-6,
              LOG);
  ...
  assert_true(
      fabs(fabs(linear_regression_parameters.intercept) - fabs(-39.1468)) < 1.0,
      LOG);
  assert_true(fabs(linear_regression_parameters.slope - 61.6746) < 1.0, LOG);
  assert_true(fabs(linear_regression_parameters.correlation - 0.9945) < 1.0e-3,
              LOG);
}
...
void VectorTest::test_scale_minimum_maximum(void) {
  ...
  assert_true(v.calculate_statistics().has_minimum_minus_one_maximum_one(),
              LOG);
}
...
void VectorTest::test_scale_mean_standard_deviation(void) {
  ...
  assert_true(v.calculate_statistics().has_mean_zero_standard_deviation_one(),
              LOG);
}
