...
void CrossEntropyErrorTest::test_calculate_generalization_performance(void) {
  ...
  nn.set(1, 1);
  nn.get_multilayer_perceptron_pointer()->get_layer_pointer(0)
      ->set_activation_function(Perceptron::Logistic);
  nn.initialize_parameters(0.0);
...
void CrossEntropyErrorTest::test_calculate_gradient(void) {
  ...
  //   gradient = cee.calculate_gradient();
  //   numerical_gradient = nd.calculate_gradient(cee,
  &CrossEntropyError::calculate_performance, parameters);   
  //   assert_true((gradient - numerical_gradient).calculate_absolute_value() <
  1.0e-3, LOG);
  */
}
...
void CrossEntropyErrorTest::test_to_XML(void) { message += "test_to_XML\n"; }
