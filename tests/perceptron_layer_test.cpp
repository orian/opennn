...
void PerceptronLayerTest::test_get_activation_function(void) {
  ...
  assert_true(pl.get_activation_function() == Perceptron::HyperbolicTangent,
              LOG);
  ...
  assert_true(pl.get_activation_function() == Perceptron::SymmetricThreshold,
              LOG);
...
void PerceptronLayerTest::test_set(void) { message += "test_set\n"; }
...
void PerceptronLayerTest::test_set_size(void) { message += "test_set_size\n"; }
...
void PerceptronLayerTest::test_grow_input(void) {
...
void PerceptronLayerTest::test_grow_perceptron(void) {
  message += "test_grow_perceptron\n";
...
void PerceptronLayerTest::test_calculate_combination(void) {
  ...
  assert_true(combination - (biases + synaptic_weights.dot(inputs)) < 1.0e-3,
              LOG);
  ...
  assert_true(combination - (biases + synaptic_weights.dot(inputs))
  ...
  assert_true(pl.calculate_combinations(inputs) ==
                  pl.calculate_combinations(inputs, parameters),
              LOG);
}
...
void PerceptronLayerTest::test_calculate_combination_Jacobian(void) {
  ...
  if (numerical_differentiation_tests) {
    numerical_combination_Jacobian = nd.calculate_Jacobian(
        pl, &PerceptronLayer::calculate_combinations, inputs);

    assert_true((combination_Jacobian - numerical_combination_Jacobian)
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
  }
...
void PerceptronLayerTest::test_calculate_combination_Hessian_form(void) {
  ...
  if (numerical_differentiation_tests) {
    numerical_combination_Hessian_form = nd.calculate_Hessian_form(
        pl, &PerceptronLayer::calculate_combinations, inputs);

    assert_true(
        (combination_Hessian_form[0] - numerical_combination_Hessian_form[0])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
    assert_true(
        (combination_Hessian_form[1] - numerical_combination_Hessian_form[1])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
    assert_true(
        (combination_Hessian_form[2] - numerical_combination_Hessian_form[2])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
    assert_true(
        (combination_Hessian_form[3] - numerical_combination_Hessian_form[3])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
  }
...
void PerceptronLayerTest::test_calculate_combination_parameters_Jacobian(void) {
  ...
  combination_parameters_Jacobian =
      pl.calculate_combinations_Jacobian(inputs, parameters);

  if (numerical_differentiation_tests) {
    numerical_combination_parameters_Jacobian = nd.calculate_Jacobian(
        pl, &PerceptronLayer::calculate_combinations, inputs, parameters);

    assert_true(
        (combination_parameters_Jacobian -
         numerical_combination_parameters_Jacobian).calculate_absolute_value() <
            1.0e-3,
        LOG);
  }
...
void PerceptronLayerTest::test_calculate_combination_parameters_Hessian_form(
  ...
  combination_parameters_Hessian_form =
      pl.calculate_combinations_Hessian_form(inputs, parameters);

  assert_true(combination_parameters_Hessian_form.size() == 4, LOG);
  assert_true(combination_parameters_Hessian_form[0].get_rows_number() ==
                  parameters_number,
              LOG);
  assert_true(combination_parameters_Hessian_form[0].get_columns_number() ==
                  parameters_number,
              LOG);
  assert_true(
      combination_parameters_Hessian_form[0].calculate_absolute_value() <
          1.0e-6,
      LOG);

  assert_true(combination_parameters_Hessian_form[1].get_rows_number() ==
                  parameters_number,
              LOG);
  assert_true(combination_parameters_Hessian_form[1].get_columns_number() ==
                  parameters_number,
              LOG);
  assert_true(
      combination_parameters_Hessian_form[1].calculate_absolute_value() <
          1.0e-6,
      LOG);

  assert_true(combination_parameters_Hessian_form[2].get_rows_number() ==
                  parameters_number,
              LOG);
  assert_true(combination_parameters_Hessian_form[2].get_columns_number() ==
                  parameters_number,
              LOG);
  assert_true(
      combination_parameters_Hessian_form[2].calculate_absolute_value() <
          1.0e-6,
      LOG);

  assert_true(combination_parameters_Hessian_form[3].get_rows_number() ==
                  parameters_number,
              LOG);
  assert_true(combination_parameters_Hessian_form[3].get_columns_number() ==
                  parameters_number,
              LOG);
  assert_true(
      combination_parameters_Hessian_form[3].calculate_absolute_value() <
          1.0e-6,
      LOG);

  if (numerical_differentiation_tests) {
    numerical_combination_parameters_Hessian_form = nd.calculate_Hessian_form(
        pl, &PerceptronLayer::calculate_combinations, inputs, parameters);

    assert_true((combination_parameters_Hessian_form[0] -
                 numerical_combination_parameters_Hessian_form[0])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
    assert_true((combination_parameters_Hessian_form[1] -
                 numerical_combination_parameters_Hessian_form[1])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
    assert_true((combination_parameters_Hessian_form[2] -
                 numerical_combination_parameters_Hessian_form[2])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
    assert_true((combination_parameters_Hessian_form[3] -
                 numerical_combination_parameters_Hessian_form[3])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
  }
...
void PerceptronLayerTest::test_calculate_activation_derivative(void) {
  ...
  if (numerical_differentiation_tests) {
    ...
    pl.set_activation_function(Perceptron::Threshold);
    activation_derivative = pl.calculate_activations_derivatives(combination);
    numerical_activation_derivative = nd.calculate_derivative(
        pl, &PerceptronLayer::calculate_activations, combination);
    assert_true((activation_derivative - numerical_activation_derivative)
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
...
void PerceptronLayerTest::test_calculate_activation_second_derivative(void) {
  ...
  activation_second_derivative =
      pl.calculate_activations_second_derivatives(combination);
  ...
  if (numerical_differentiation_tests) {
    ...
    activation_second_derivative =
        pl.calculate_activations_second_derivatives(combination);
    numerical_activation_second_derivative = nd.calculate_second_derivative(
        pl, &PerceptronLayer::calculate_activations, combination);
    assert_true(
        (activation_second_derivative - numerical_activation_second_derivative)
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
...
void PerceptronLayerTest::test_calculate_outputs(void) {
  ...
  assert_true(
      pl.calculate_outputs(inputs) == pl.calculate_outputs(inputs, parameters),
      LOG);
}
...
void PerceptronLayerTest::test_calculate_Jacobian(void) {
  ...
  if (numerical_differentiation_tests) {
    ...
    numerical_Jacobian =
        nd.calculate_Jacobian(pl, &PerceptronLayer::calculate_outputs, inputs);
...
void PerceptronLayerTest::test_calculate_Hessian_form(void) {
  ...
  if (numerical_differentiation_tests) {
    ...
    numerical_Hessian_form = nd.calculate_Hessian_form(
        pl, &PerceptronLayer::calculate_outputs, inputs);

    assert_true((Hessian_form[0] - numerical_Hessian_form[0])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
  }
  ...
  if (numerical_differentiation_tests) {
    ...
    assert_true((Hessian_form[0] - numerical_Hessian_form[0])
                    ...
                    1.0e-3,
                LOG);
    assert_true((Hessian_form[1] - numerical_Hessian_form[1])
                        .calculate_absolute_value() <
                    1.0e-3,
                LOG);
  }
  ...
  assert_true((pl.get_perceptron(0).calculate_Hessian(inputs) - Hessian_form[0])
                      .calculate_absolute_value() <
                  1.0e-3,
              LOG);
  assert_true((pl.get_perceptron(1).calculate_Hessian(inputs) - Hessian_form[1])
                      .calculate_absolute_value() <
                  1.0e-3,
              LOG);
...
void PerceptronLayerTest::test_calculate_parameters_Jacobian(void) {
  ...
  if (numerical_differentiation_tests) {
    numerical_parameters_Jacobian = nd.calculate_Jacobian(
        pl, &PerceptronLayer::calculate_outputs, inputs, parameters);
...
void PerceptronLayerTest::test_calculate_parameters_Hessian_form(void) {
  ...
  if (numerical_differentiation_tests) {
    numerical_parameters_Hessian_form = nd.calculate_Hessian_form(
        pl, &PerceptronLayer::calculate_outputs, inputs, parameters);

    assert_true(
        (parameters_Hessian_form[0] - numerical_parameters_Hessian_form[0])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
    assert_true(
        (parameters_Hessian_form[1] - numerical_parameters_Hessian_form[1])
                .calculate_absolute_value() <
            1.0e-3,
        LOG);
  }
