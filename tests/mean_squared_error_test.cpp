...
void MeanSquaredErrorTest::test_calculate_performance(void) {
  ...
  assert_true(
      mse.calculate_performance() == mse.calculate_performance(parameters),
      LOG);
}
...
void MeanSquaredErrorTest::test_calculate_gradient(void) {
  ...
  objective_gradient = mse.calculate_gradient();
  numerical_objective_gradient = nd.calculate_gradient(
      mse, &MeanSquaredError::calculate_performance, parameters);
  assert_true((objective_gradient - numerical_objective_gradient)
...
void MeanSquaredErrorTest::test_calculate_performance_terms(void) {
  ...
  assert_true(
      fabs((evaluation_terms * evaluation_terms).calculate_sum() - objective) <
          1.0e-3,
      LOG);
}
...
void MeanSquaredErrorTest::test_calculate_Jacobian_terms(void) {
  ...
  assert_true(terms_Jacobian.get_rows_number() ==
                  ds.get_instances().count_training_instances_number(),
              LOG);
  assert_true(
      terms_Jacobian.get_columns_number() == nn.count_parameters_number(), LOG);
  ...
  terms_Jacobian = mse.calculate_terms_Jacobian();
  numerical_Jacobian_terms = nd.calculate_Jacobian(
      mse, &MeanSquaredError::calculate_terms, parameters);

  assert_true(
      (terms_Jacobian - numerical_Jacobian_terms).calculate_absolute_value() <
          1.0e-3,
      LOG);
  ...
  assert_true(
      ((terms_Jacobian.calculate_transpose()).dot(evaluation_terms) * 2.0 -
       objective_gradient).calculate_absolute_value() <
          1.0e-3,
      LOG);
}
...
void MeanSquaredErrorTest::test_to_XML(void) { message += "test_to_XML\n"; }

void MeanSquaredErrorTest::test_from_XML(void) { message += "test_from_XML\n"; }
