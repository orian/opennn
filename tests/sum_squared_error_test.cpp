...
void SumSquaredErrorTest::test_calculate_gradient(void) {
  ...
  numerical_gradient = nd.calculate_gradient(
      sse, &SumSquaredError::calculate_performance, parameters);
...
void SumSquaredErrorTest::test_calculate_Hessian(void) {
  ...
  /*
     ...
     Hessian = sse.calculate_Hessian();
     numerical_Hessian = nd.calculate_Hessian(sse,
  &SumSquaredError::calculate_performance, parameters);
  //   assert_true((Hessian - numerical_Hessian).calculate_absolute_value() <
  1.0e-3, LOG);
  ...
  1.0e-3, LOG);
     ...
     numerical_Hessian = nd.calculate_Hessian(sse,
  &SumSquaredError::calculate_performance, parameters);
  //   assert_true((objective_Hessian -
  numerical_objective_Hessian).calculate_absolute_value() < 1.0e-3, LOG);
...
void SumSquaredErrorTest::test_calculate_Jacobian_terms(void) {
  ...
  assert_true(terms_Jacobian.get_rows_number() ==
                  ds.get_instances().get_instances_number(),
              LOG);
  assert_true(
  ...
  terms_Jacobian = sse.calculate_terms_Jacobian();
  numerical_Jacobian_terms =
      nd.calculate_Jacobian(sse, &SumSquaredError::calculate_terms, parameters);
  ...
  assert_true(((terms_Jacobian.calculate_transpose()).dot(terms) * 2.0 -
               gradient).calculate_absolute_value() <
                  1.0e-3,
              LOG);
}
