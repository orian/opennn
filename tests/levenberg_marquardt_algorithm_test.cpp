...
void LevenbergMarquardtAlgorithmTest::test_calculate_gradient(void) {
  ...
  assert_true(
      (gradient - pf.calculate_gradient()).calculate_absolute_value() < 1.0e-3,
      LOG);
...
void LevenbergMarquardtAlgorithmTest::test_calculate_Hessian_approximation(
  ...
  assert_true(Hessian_approximation.get_rows_number() == parameters_number,
              LOG);
  assert_true(Hessian_approximation.get_columns_number() == parameters_number,
              LOG);
  assert_true(Hessian_approximation.is_symmetric(), LOG);

  // Test

  pf.set_objective_type(
      PerformanceFunctional::NORMALIZED_SQUARED_ERROR_OBJECTIVE);
  ...
  assert_true((lma.calculate_Hessian_approximation(terms_Jacobian) - Hessian)
  ...
  numerical_Hessian = nd.calculate_Hessian(
      pf, &PerformanceFunctional::calculate_performance, parameters);
  ...
  assert_true(
      (numerical_Hessian - Hessian_approximation).calculate_absolute_value() >=
          0.0,
      LOG);
}
...
void LevenbergMarquardtAlgorithmTest::test_set_reserve_all_training_history(
  ...
  assert_true(lma.get_reserve_generalization_performance_history() == true,
              LOG);
