...
void NewtonMethodTest::test_destructor(void) { message += "test_destructor\n"; }
...
void NewtonMethodTest::test_calculate_gradient_descent_training_direction(
  ...
  Vector<double> gradient_descent_training_direction =
      nm.calculate_gradient_descent_training_direction(gradient);

  assert_true(gradient_descent_training_direction.size() == 2, LOG);
  assert_true(
      (gradient_descent_training_direction.calculate_norm() - 1.0) < 1.0e-3,
      LOG);
}
