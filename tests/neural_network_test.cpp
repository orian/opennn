...
void NeuralNetworkTest::test_initialize_random(void) {
  ...
  //   unsigned independent_parameters_number;
  ...
  //   assert_true(nn.has_inputs() == true, LOG);
  //   assert_true(nn.has_scaling_layer() == true, LOG);
  //   assert_true(nn.has_multilayer_perceptron() == true, LOG);
  //   assert_true(nn.has_outputs() == true, LOG);

  //   inputs_number = nn.get_inputs_number();

  //   assert_true(inputs_number >= 1 && inputs_number <= 10, LOG);

  //   layers_number = nn.get_layers_number();

  //   assert_true(layers_number >= 1 && layers_number <= 10, LOG);

  //   multilayer_perceptron_architecture = nn.get_layers_size();

  //   assert_true(multilayer_perceptron_architecture >= 1 &&
  // multilayer_perceptron_architecture <= 10, LOG);

  //   independent_parameters_number = nn.get_independent_parameters_number();

  //   assert_true(independent_parameters_number >= 1 &&
  // independent_parameters_number <= 10, LOG);
}
...
void NeuralNetworkTest::test_calculate_parameters_Jacobian(void) {
  ...
  //   assert_true(parameters_Jacobian.get_rows_number() == outputs_number,
  // LOG);
  ...
  if (numerical_differentiation_tests) {
    ...
    //      numerical_parameters_Jacobian = nd.calculate_Jacobian(nn,
    // &NeuralNetwork::calculate_parameters_output, inputs, parameters);

    //      assert_true((parameters_Jacobian-numerical_parameters_Jacobian).calculate_absolute_value()
    // < 1.0e-3, LOG);
  }
