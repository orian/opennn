...
/// Default constructor.
/// It creates a training rate algorithm object not associated to any
...
/// Returns a pointer to the performance functional object
/// to which the training rate algorithm is associated.
/// If the performance functional pointer is NULL, this method throws an
/// exception.

PerformanceFunctional*
TrainingRateAlgorithm::get_performance_functional_pointer(void) const {
#ifndef NDEBUG
...
/// Returns true if this training rate algorithm has an associated performance
/// functional,
/// and false otherwise.
...
const TrainingRateAlgorithm::TrainingRateMethod&
TrainingRateAlgorithm::get_training_rate_method(void) const {
...
const bool& TrainingRateAlgorithm::get_display(void) const { return (display); }
...
void TrainingRateAlgorithm::set_performance_functional_pointer(
    PerformanceFunctional* new_performance_functional_pointer) {
  performance_functional_pointer = new_performance_functional_pointer;
}
...
void TrainingRateAlgorithm::set_training_rate_method(
    const TrainingRateAlgorithm::TrainingRateMethod& new_training_rate_method) {
  training_rate_method = new_training_rate_method;
}

// void set_training_rate_method(const std::string&) method

/// Sets the method for obtaining the training rate from a string with the name
/// of the method.
/// @param new_training_rate_method Name of training rate method ("Fixed",
/// "GoldenSection", "BrentMethod" or "UserTrainingRate").

void TrainingRateAlgorithm::set_training_rate_method(
    const std::string& new_training_rate_method) {
...
/// Sets a new increase factor value to be used for line minimization when
/// bracketing a minimum.
/// @param new_bracketing_factor Bracketing factor value.

void TrainingRateAlgorithm::set_bracketing_factor(
    const double& new_bracketing_factor) {
...
void TrainingRateAlgorithm::set_training_rate_tolerance(
    const double& new_training_rate_tolerance) {
// Control sentence (if debug)
...
void TrainingRateAlgorithm::set_warning_training_rate(
...
void TrainingRateAlgorithm::set_error_training_rate(
...
// Vector<double> calculate_directional_point(const double&, const
// Vector<double>&, const double&) const method

/// Returns a vector with two elements:
/// (i) the training rate calculated by means of the corresponding algorithm,
/// and
...
Vector<double> TrainingRateAlgorithm::calculate_directional_point(
    const double& performance, const Vector<double>& training_direction,
  ...
  switch (training_rate_method) {
    case TrainingRateAlgorithm::Fixed: {
      return (calculate_fixed_directional_point(performance, training_direction,
                                                initial_training_rate));
    } break;

    case TrainingRateAlgorithm::GoldenSection: {
      return (calculate_golden_section_directional_point(
          performance, training_direction, initial_training_rate));
    } break;

    case TrainingRateAlgorithm::BrentMethod: {
      return (calculate_Brent_method_directional_point(
          performance, training_direction, initial_training_rate));
    } break;
    ...
    default: {
      ...
      buffer << "OpenNN Exception: TrainingRateAlgorithm class\n"
             << "Vector<double> calculate_directional_point(const double&, "
                "const Vector<double>&, const double&) const method.\n"
             << "Unknown training rate method.\n";
...
// Triplet calculate_bracketing_triplet(const double&, const Vector<double>&,
// const double&) const method
...
TrainingRateAlgorithm::Triplet
TrainingRateAlgorithm::calculate_bracketing_triplet(
    const double& performance, const Vector<double>& training_direction,
    const double& initial_training_rate) const {
  Triplet triplet;
  ...
  triplet.B[1] = performance_functional_pointer
      ->calculate_performance(training_direction, triplet.B[0]);
  ...
  while (triplet.A[1] > triplet.B[1]) {
    ...
    if (triplet.B[0] > error_training_rate) {
      ...
      buffer << "OpenNN Warning: TrainingRateAlgorithm class.\n"
             << "Vector<double> calculate_bracketing_triplet(double, const "
                "Vector<double>&, double) const method\n."
  ...
  triplet.U[1] = performance_functional_pointer
      ->calculate_performance(training_direction, triplet.U[0]);

  while (triplet.A[1] < triplet.U[1]) {
    triplet.U[0] =
        triplet.A[0] + (triplet.U[0] - triplet.A[0]) / bracketing_factor;
    triplet.U[1] = performance_functional_pointer
        ->calculate_performance(training_direction, triplet.U[0]);
...
// Vector<double> calculate_fixed_directional_point(const double&, const
// Vector<double>&, const double&) const method
...
///
Vector<double> TrainingRateAlgorithm::calculate_fixed_directional_point(
    const double&, const Vector<double>& training_direction,
    const double& initial_training_rate) const {
  Vector<double> directional_point(2);

  directional_point[0] = initial_training_rate;
  directional_point[1] = performance_functional_pointer->calculate_performance(
      training_direction, initial_training_rate);

  return (directional_point);
}

// Vector<double> calculate_golden_section_directional_point(double,
// Vector<double>, double) const method

/// Returns the training rate by searching in a given direction to locate the
...
Vector<double>
TrainingRateAlgorithm::calculate_golden_section_directional_point(
    const double& performance, const Vector<double>& training_direction,
    const double& initial_training_rate) const {
  std::ostringstream buffer;
  ...
  try {
    Triplet triplet = calculate_bracketing_triplet(
        performance, training_direction, initial_training_rate);
    ...
    do {
      ...
      V[1] = performance_functional_pointer->calculate_performance(
          training_direction, V[0]);
      ...
      if (V[0] < triplet.U[0] && V[1] >= triplet.U[1]) {
        ...
        buffer << "OpenNN Exception: TrainingRateAlgorithm class.\n"
               << "Vector<double> "
                  "calculate_golden_section_directional_point(double, const "
                  "Vector<double>, double) const method.\n"
    ...
    X[1] = performance_functional_pointer->calculate_performance(
        training_direction, X[0]);
...
// Vector<double> calculate_Brent_method_directional_point(const double&, const
// Vector<double>, const double&) const method

/// Returns the training rate by searching in a given direction to locate the
/// minimum of the performance
/// function in that direction. It uses the Brent's method.
/// @param performance Neural network performance value.
/// @param training_direction Training direction vector.
/// @param initial_training_rate Initial training rate in line minimization.
  ...
  try {
    ...
    while (triplet.B[0] - triplet.A[0] > training_rate_tolerance) {
      ...
      if (V[0] < triplet.U[0] && V[1] >= triplet.U[1]) {
        ...
        buffer << "OpenNN Exception: TrainingRateAlgorithm class.\n"
               << "Vector<double> "
                  "calculate_Brent_method_directional_point(double, const "
                  "Vector<double>, double) const method.\n"
               << "Both interior points have the same ordinate.\n";
...
/// Calculates the golden section point within a minimum interval defined by
/// three points.
/// @param triplet Triplet containing a minimum.

double TrainingRateAlgorithm::calculate_golden_section_training_rate(
  ...
  if (training_rate < triplet.A[0]) {
    ...
    buffer << "OpenNN Error: TrainingRateAlgorithm class.\n"
           << "double calculate_golden_section_training_rate(const Triplet&) "
              "const method.\n"
           << "Training rate (" << training_rate
           << ") is less than triplet left point (" << triplet.A[0] << ").\n";
  ...
  if (training_rate > triplet.B[0]) {
    ...
    buffer << "OpenNN Error: TrainingRateAlgorithm class.\n"
           << "double calculate_golden_section_training_rate(const Triplet&) "
              "const method.\n"
           << "Training rate (" << training_rate
           << ") is greater than triplet right point (" << triplet.B[0]
           << ").\n";
...
/// Returns the minimimal training rate of a parabola defined by three
/// directional points.
/// @param triplet Triplet containing a minimum.

double TrainingRateAlgorithm::calculate_Brent_method_training_rate(
    const Triplet& triplet) const {
  const double c =
      -(triplet.A[1] * (triplet.U[0] - triplet.B[0]) +
        triplet.U[1] * (triplet.B[0] - triplet.A[0]) +
        triplet.B[1] * (triplet.A[0] - triplet.U[0])) /
      ((triplet.A[0] - triplet.U[0]) * (triplet.U[0] - triplet.B[0]) *
       (triplet.B[0] - triplet.A[0]));

  if (c == 0) {
    std::ostringstream buffer;

    buffer << "OpenNN Exception: TrainingRateAlgorithm class.\n"
           << "double calculate_Brent_method_training_rate(Vector<double>&, "
              "Vector<double>&, Vector<double>&) const method.\n"
  ...
  const double b =
      (triplet.A[1] *
           (triplet.U[0] * triplet.U[0] - triplet.B[0] * triplet.B[0]) +
       triplet.U[1] *
           (triplet.B[0] * triplet.B[0] - triplet.A[0] * triplet.A[0]) +
       triplet.B[1] *
           (triplet.A[0] * triplet.A[0] - triplet.U[0] * triplet.U[0])) /
      ((triplet.A[0] - triplet.U[0]) * (triplet.U[0] - triplet.B[0]) *
       (triplet.B[0] - triplet.A[0]));

  const double Brent_method_training_rate = -b / (2.0 * c);

  if (Brent_method_training_rate <= triplet.A[0] ||
      Brent_method_training_rate >= triplet.B[0]) {
    std::ostringstream buffer;

    buffer
        << "OpenNN Exception: TrainingRateAlgorithm class.\n"
        << "double calculate_parabola_minimal_training_rate(Vector<double>&, "
           "Vector<double>&, Vector<double>&) const method.\n"
        << "Brent method training rate is not inside interval.\n"
        << "Interval: (" << triplet.A[0] << "," << triplet.B[0] << ")\n"
        << "Brent method training rate: " << Brent_method_training_rate
        << std::endl;
...
void TrainingRateAlgorithm::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("TrainingRateAlgorithm");
  ...
  // Training rate method
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("TrainingRateMethod");
  ...
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("BracketingFactor");
  ...
  */
  ...
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("TrainingRateTolerance");
