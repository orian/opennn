...
//   else if(!(*numerical_differentiation_pointer == *other_performance_term.numerical_differentiation_pointer))
//   {
//       return(false);
//   }
 ...
 const Vector<double>& output_objective_gradient) const
{
   ...
   if(layers_number > 0)
   {
      const unsigned output_objective_gradient_size = output_objective_gradient.size();
 ...
 const Vector<double>& output_objective_gradient) const
{
   ...
   const unsigned objective_function_output_gradient_size = output_objective_gradient.size();
 ...
 const Vector< Vector<double> >& layers_delta) const
{
   ...
   const unsigned layers_number = multilayer_perceptron_pointer->get_layers_number();
   const Vector<unsigned> layers_size = multilayer_perceptron_pointer->arrange_layers_perceptrons_numbers();
   ...
   const unsigned layers_combination_parameters_Jacobian_size = layers_combination_parameters_Jacobian.size();
 ...
 const Vector< Vector<double> >& layers_delta) const
{
   ...
   if(layers_number != 0)
   {
      ...
      const unsigned output_objective_Hessian_rows_number = output_objective_Hessian.get_rows_number();
      const unsigned output_objective_Hessian_columns_number = output_objective_Hessian.get_columns_number();
   ...
   if(layers_number > 0)
   {
      ...
      {   
         ...
         {   
            if((unsigned)i != layers_number-1 && (unsigned)j != layers_number-1)
            {
...
//   const unsigned perceptrons_combination_parameters_gradient_size = perceptrons_combination_parameters_gradient.size();

//   const unsignederlayers_combination_combination_Jacobian_rows_number = interlayers_combination_combination_Jacobian.get_rows_number();
//   const unsignederlayers_combination_combination_Jacobian_columns_number = interlayers_combination_combination_Jacobian.get_columns_number();

//   const unsigned layers_delta_size = layers_delta.size();

//   const unsignederlayers_Delta_rows_number = interlayers_Delta.get_rows_number();
//   const unsignederlayers_Delta_columns_number = interlayers_Delta.get_columns_number();
...
// unsigned calculate_Kronecker_delta(const unsigned&, const unsigned&) const method

/// Returns the Knronecker delta of two integers a and b, which equals 1 if they are equal and 0 otherwise.
/// @param a First integer.
/// @param b Second integer.

unsigned PerformanceTerm::calculate_Kronecker_delta(const unsigned& a, const unsigned& b) const
{
   if(a == b)
   {
      return(1);
