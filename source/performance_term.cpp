...
/// Default constructor.
/// It creates a default performance term object, with all pointers initialized
/// to NULL.
...
/// Mathematical model constructor.
/// It creates a performance term object associated to a given mathematical
/// model object.
/// The rest of pointers are initialized to NULL.
/// It also initializes all the rest of class members to their default values.
...
/// Neural network and data set constructor.
/// It creates a performance term object associated to a neural network and to
/// be measured on a data set.
/// The rest of pointers are initialized to NULL.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_data_set_pointer Pointer to a data set object.
...
/// Neural network and mathematical model constructor.
/// It creates a performance term object associated to a neural network and to
/// be measured on a mathematical model.
/// The rest of pointers are initialized to NULL.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_mathematical_model_pointer Pointer to a mathematical model
...
/// Neural network, mathematical model and data set constructor.
/// It creates a performance term object associated to a neural network and to
/// be measured on both a mathematical model and a data set.
/// The rest of pointers are initialized to NULL.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_mathematical_model_pointer Pointer to a mathematical model
...
/// @param performance_term_document Pointer to a TinyXML document with the
/// object data.

PerformanceTerm::PerformanceTerm(
    const tinyxml2::XMLDocument& performance_term_document)
    : neural_network_pointer(NULL),
      data_set_pointer(NULL),
      mathematical_model_pointer(NULL),
      numerical_differentiation_pointer(NULL) {
...
PerformanceTerm::PerformanceTerm(const PerformanceTerm& other_performance_term)
  ...
  mathematical_model_pointer =
      other_performance_term.mathematical_model_pointer;
  ...
  if (other_performance_term.numerical_differentiation_pointer) {
    numerical_differentiation_pointer = new NumericalDifferentiation(
        *other_performance_term.numerical_differentiation_pointer);
  }
...
/// Destructor.
/// It deletes the numerical differentiation object composing this performance
/// term object.
...
/// Assignment operator.
/// It assigns to this performance term object the members from another
/// performance term object.
/// @param other_performance_term Performance term object to be copied.

PerformanceTerm& PerformanceTerm::operator=(
    const PerformanceTerm& other_performance_term) {
...
/// The return is true if both objects have the same member data, and false
/// otherwise.

/// @todo Numerical differentiation comparison.

bool PerformanceTerm::operator==(
    const PerformanceTerm& other_performance_term) const {
  if (neural_network_pointer != other_performance_term.neural_network_pointer ||
    ...
    return (false);
      ...
      //   else if(!(*numerical_differentiation_pointer ==
      // *other_performance_term.numerical_differentiation_pointer))
      //   {
      //       return(false);
      //   }
...
const bool& PerformanceTerm::get_display(void) const { return (display); }
...
/// Returns true if this performance term object contains a numerical
...
/// Sets all the member pointers to NULL (neural network, data set, mathematical
/// model and numerical differentiation).
/// It also initializes all the rest of class members to their default values.
...
/// Sets all the member pointers to NULL, but the neural network, which set to a
/// given pointer.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
...
/// Sets all the member pointers to NULL, but the data set, which set to a given
/// pointer.
/// It also initializes all the rest of class members to their default values.
/// @param new_data_set_pointer Pointer to a data set object.
...
/// Sets all the member pointers to NULL, but the mathematical model, which set
/// to a given pointer.
/// It also initializes all the rest of class members to their default values.
/// @param new_mathematical_model_pointer Pointer to a mathematical model
/// object.
...
/// It also sets the mathematical model and numerical differentiation pointers
/// to NULL.
/// Finally, it initializes all the rest of class members to their default
/// values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_data_set_pointer Pointer to a data set object.
...
/// Sets to this performance term object the members of another performance term
/// object.
...
/// Sets a pointer to a neural network object which is to be associated to the
/// performance term.
/// @param new_neural_network_pointer Pointer to a neural network object to be
/// associated to the performance term.

void PerformanceTerm::set_neural_network_pointer(
    NeuralNetwork* new_neural_network_pointer) {
  neural_network_pointer = new_neural_network_pointer;
}
...
/// Sets a new mathematical model on which the performance term is to be
/// measured.

void PerformanceTerm::set_mathematical_model_pointer(
    MathematicalModel* new_mathematical_model_pointer) {
  mathematical_model_pointer = new_mathematical_model_pointer;
}
...
/// Sets a new numerical differentiation pointer in this performance term
/// object.
/// @param new_numerical_differentiation_pointer Pointer to a numerical
/// differentiation object.

void PerformanceTerm::set_numerical_differentiation_pointer(
    NumericalDifferentiation* new_numerical_differentiation_pointer) {
...
void PerformanceTerm::set_default(void) { display = true; }
...
/// This method constructs the numerical differentiation object which composes
/// the performance term class.
...
/// This method deletes the numerical differentiation object which composes the
/// performance term class.
...
/// @param layers_activation_derivative Forward propagation activation
/// derivative.
...
// Neural network stuff
  ...
  if (!multilayer_perceptron_pointer) {
    buffer << "OpenNN Exception: PerformanceTerm class.\n"
           << "Vector< Vector<double> > calculate_layers_delta(const Vector< "
...
// Control sentence (if debug)
  ...
  if (layers_number > 0) {
    const unsigned output_objective_gradient_size =
        output_objective_gradient.size();

    if (output_objective_gradient_size !=
        layers_perceptrons_number[layers_number - 1]) {
      buffer << "OpenNN Exception: PerformanceTerm class.\n"
             << "Vector<double> calculate_layers_delta(const Vector< "
                "Vector<double> >&, const Vector<double>&) method.\n"
             << "Size of outputs objective gradient ("
             << output_objective_gradient_size << ") must be equal to "
             << "number of outputs ("
             << layers_perceptrons_number[layers_number - 1] << ").\n";
  ...
  if (layers_number > 0) {
    ...
    for (int i = layers_number - 2; i >= 0; i--) {
      layer_synaptic_weights =
          neural_network_pointer->get_multilayer_perceptron_pointer()
              ->get_layer(i + 1).arrange_synaptic_weights();

      layers_delta[i] = layers_activation_derivative[i] *
                        (layers_delta[i + 1].dot(layer_synaptic_weights));
    }
  }
...
// Vector< Vector<double> > calculate_layers_delta(const Vector< Vector<double>
// >&, const Vector<double>&, const Vector<double>&) method
...
/// @param layers_activation_derivative Forward propagation activation
/// derivative.
/// @param homogeneous_solution Homogeneous solution for calculating the
/// conditioned outputs.
/// @param output_objective_gradient Gradient of the outputs objective function.
...
// Control sentence (if debug)
  ...
  const unsigned objective_function_output_gradient_size =
      output_objective_gradient.size();

  if (objective_function_output_gradient_size !=
      layers_perceptrons_number[layers_number - 1]) {
    std::ostringstream buffer;
    ...
    buffer << "OpenNN Exception: PerformanceTerm class.\n"
           << "Vector<double> calculate_layers_delta(const Vector< "
              "Vector<double> >&, const Vector<double>&) const method.\n"
           << "Size of objective function outputs derivative ("
           << objective_function_output_gradient_size << ")must be equal to "
           << "number of outputs ("
           << layers_perceptrons_number[layers_number - 1] << ").\n";
  ...
  const Vector<PerceptronLayer>& layers =
      multilayer_perceptron_pointer->get_layers();
  ...
  layers_delta[layers_number - 1] =
      layers_activation_derivative[layers_number - 1] * homogeneous_solution *
      output_objective_gradient;

  // Rest of hidden layers

  for (int h = layers_number - 2; h >= 0; h--) {
    for (unsigned i = 0; i < layers_perceptrons_number[h]; i++) {
      sum = 0.0;

      for (unsigned j = 0; j < layers_perceptrons_number[h + 1]; j++) {
...
// Vector<double> calculate_point_gradient(const Vector<double>&, const Vector<
// Vector<double> >&, const Vector<double>&) const method

/// Returns the gradient of the performance term function at some input point.
/// @param inputs Input vector.
/// @param layers_activation Activations of all layers in the multilayer
/// perceptron
/// @param layers_delta Vector of vectors containing the partial derivatives of
/// the outputs objective function with respect to all the combinations of all
/// layers.
  ...
  if (!multilayer_perceptron_pointer) {
    buffer << "OpenNN Exception: PerformanceTerm class.\n"
           << "Vector<double> calculate_point_gradient(const Vector<double>&, "
              "const Vector< Vector<double> >&, const Vector<double>&) const "
              "method.\n"
...
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    buffer
        ...
        << "Vector< Vector<double> > calculate_layers_objective_gradient(const "
           "Vector< Vector<double> >&, const Vector<double>&, const "
  ...
  const unsigned layers_delta_size = layers_delta.size();

  if (layers_delta_size != layers_number) {
    buffer
        << "OpenNN Exception: PerformanceTerm class.\n"
        << "Vector< Vector<double> > calculate_layers_objective_gradient(const "
           "Vector< Vector<double> >&, const Vector<double>&) method.\n"
  ...
  const Vector<Vector<double> > layers_inputs =
      multilayer_perceptron_pointer->arrange_layers_input(inputs,
                                                          layers_activation);
  ...
  for (unsigned i = 0; i < layers_number; i++) {
    point_gradient.tuck_in(
        index, layers_delta[i].dot(layers_combination_parameters_Jacobian[i]));

    index +=
        multilayer_perceptron_pointer->get_layer(i).count_parameters_number();
  }
  ...
  if (layers_number != 0) {
    ...
    for (unsigned i = 0; i < layers_perceptrons_number[0]; i++) {
      // Bias
      ...
      synaptic_weights = multilayer_perceptron_pointer->get_layer(0)
          .get_perceptron(i).arrange_synaptic_weights();
    ...
    for (unsigned h = 1; h < layers_number; h++) {
      for (unsigned i = 0; i < layers_perceptrons_number[h]; i++) {
        // Bias
        ...
        synaptic_weights = multilayer_perceptron_pointer->get_layer(h)
            .get_perceptron(i).arrange_synaptic_weights();

        for (unsigned j = 0; j < layers_perceptrons_number[h - 1]; j++) {
          point_gradient[index] =
              layers_delta[h][i] * layers_activation[h - 1][j];
          index++;
        }
      }
    }
...
// const Vector< Vector<double> >&) const method
...
// Control sentence (if debug)
  ...
  const unsigned layers_combination_parameters_Jacobian_size =
      layers_combination_parameters_Jacobian.size();
  ...
  if (layers_combination_parameters_Jacobian_size != layers_number) {
    buffer
        ...
        << "Vector< Vector<double> > calculate_layers_objective_gradient(const "
           "Vector< Vector<double> >&, const Vector<double>&, const "
           "Vector<double>&) method.\n"
        << "Size of forward propagation activation ("
        << layers_combination_parameters_Jacobian_size
        << ") must be equal to number of layers (" << layers_number << ").\n";
  ...
  for (unsigned i = 0; i < layers_number; i++) {
    ...
    index += neural_network_pointer->get_multilayer_perceptron_pointer()
        ->get_layer(i).count_parameters_number();
  }

  return (point_gradient);
}

// Matrix< Matrix<double> > calculate_interlayers_Delta(const Vector<
// Vector<double> >&, const Vector< Vector<double> >&, const Vector<double>&,
// const Matrix<double>&, const Vector< Vector<double> >&) method

/// Returns the second partial derivatives of the outputs objective function
/// with respect to the combinations of two layers.
/// That quantity is called interlayers Delta, and it is represented as a matrix
/// of matrices.
/// @param layers_activation_derivative Activation derivatives of all layers in
/// the multilayer perceptron
/// @param layers_activation_second_derivative Activation second derivatives of
/// all layers in the multilayer perceptron
/// @param interlayers_combination_combination_Jacobian_form Matrix of matrices
/// containing the partial derivatives of all layers combinations with respect
/// to all layers combinations.
/// @param output_objective_gradient Gradient vector of the outputs objective
/// function.
/// @param output_objective_Hessian Hessian matrix of the outputs objective
/// function.
/// @param layers_delta Vector of vectors containing the partial derivatives of
/// the outputs objective function with respect to the combinations of all
/// layers.
/// @todo
...
Matrix<Matrix<double> > PerformanceTerm::calculate_interlayers_Delta(
    ...
    const Matrix<Matrix<double> >&
        interlayers_combination_combination_Jacobian_form,
...
// Control sentence (if debug)
  ...
  if (layers_number != 0) {
    ...
    if (output_objective_gradient_size != layers_size[layers_number - 1]) {
      buffer << "OpenNN Exception: PerformanceTerm class.\n"
          //       << "Vector<double> calculate_interlayers_Delta(const
          // Vector<double>&) method.\n"
          //       << "Size (" << size << ") must be equal to number of
          // parameters (" << parameters_number << ")."
             << std::endl;

      throw std::logic_error(buffer.str());
    }

    const unsigned output_objective_Hessian_rows_number =
        output_objective_Hessian.get_rows_number();
    const unsigned output_objective_Hessian_columns_number =
        output_objective_Hessian.get_columns_number();

    if (output_objective_Hessian_rows_number !=
        layers_size[layers_number - 1]) {
      buffer << "OpenNN Exception: PerformanceTerm class.\n"
          //       << "Vector<double> calculate_interlayers_Delta(const
          // Vector<double>&) method.\n"
          //       << "Size (" << size << ") must be equal to number of
    ...
    if (output_objective_Hessian_columns_number !=
        layers_size[layers_number - 1]) {
      buffer << "OpenNN Exception: PerformanceTerm class.\n"
          //       << "Vector<double> calculate_interlayers_Delta(const
          // Vector<double>&) method.\n"
  ...
  const Vector<Matrix<double> > layers_synaptic_weights =
      multilayer_perceptron_pointer->arrange_layers_synaptic_weights();
  ...
  if (layers_number > 0) {
    ...
    interlayers_Delta[layers_number - 1][layers_number - 1] = (
        output_objective_Hessian *
        (layers_activation_derivative[layers_number - 1] *
         layers_activation_derivative[layers_number - 1]))
        .sum_diagonal(output_objective_gradient *
    ...
    for (int i = layers_number - 1; i >= 0; i--) {
      for (int j = layers_number - 1; j >= 0; j--) {
        if ((unsigned)i != layers_number - 1 &&
            (unsigned)j != layers_number - 1) {
...
// Matrix<double> calculate_point_Hessian(const Vector<double>&, const Matrix<
// Matrix<double> >&, const Vector< Vector<double> >&, const Matrix<
...
// Neural network stuff
  ...
  if (!multilayer_perceptron_pointer) {
    buffer << "OpenNN Exception: PerformanceTerm class.\n"
           << "Matrix<double> calculate_point_Hessian(const Vector<double>&, "
              "const Matrix< Matrix<double> >&, const Vector< Vector<double> "
              ">&, const Matrix< Matrix<double> >&) const method.\n"
           << "Multilayer perceptron pointer is NULL.\n";
  ...
  if (layers_activation_derivative_size != layers_number) {
    buffer << "OpenNN Exception: PerformanceTerm class.\n"
           << "Matrix<double> calculate_point_Hessian(const Vector<double>&, "
              "const Matrix< Matrix<double> >&, const Vector< Vector<double> "
              ">&, const Matrix< Matrix<double> >&) const method.\n"
           << "Size of layers activation derivative must be equal to number of "
              "layers in multilayer perceptron.\n";

    throw std::logic_error(buffer.str());
  }

//   const unsigned perceptrons_combination_parameters_gradient_size =
// perceptrons_combination_parameters_gradient.size();

//   const unsignederlayers_combination_combination_Jacobian_rows_number =
// interlayers_combination_combination_Jacobian.get_rows_number();
//   const unsignederlayers_combination_combination_Jacobian_columns_number =
// interlayers_combination_combination_Jacobian.get_columns_number();

//   const unsigned layers_delta_size = layers_delta.size();

//   const unsignederlayers_Delta_rows_number =
// interlayers_Delta.get_rows_number();
//   const unsignederlayers_Delta_columns_number =
// interlayers_Delta.get_columns_number();
  ...
  unsigned layer_index_i;
  unsigned neuron_index_i;
  unsigned parameter_index_i;

  unsigned layer_index_j;
  unsigned neuron_index_j;
  unsigned parameter_index_j;

  if (layers_number > 0) {
    for (unsigned i = 0; i < parameters_number; i++) {
      parameter_indices =
          multilayer_perceptron_pointer->arrange_parameter_indices(i);
      ...
      for (unsigned j = 0; j < parameters_number; j++) {
        parameter_indices =
            multilayer_perceptron_pointer->arrange_parameter_indices(j);
        layer_index_j = parameter_indices[0];
        neuron_index_j = parameter_indices[1];
        parameter_index_j = parameter_indices[2];

        point_Hessian[i][j] =
            perceptrons_combination_parameters_gradient[layer_index_i][
                neuron_index_i][parameter_index_i] *
                perceptrons_combination_parameters_gradient[layer_index_j][
                    neuron_index_j][parameter_index_j] *
                interlayers_Delta[layer_index_j][layer_index_i][neuron_index_j][
                    neuron_index_i] +
            perceptrons_combination_parameters_gradient[layer_index_i][
                neuron_index_i][parameter_index_i] *
                layers_delta[layer_index_j][neuron_index_j] *
                layers_activation_derivative[layer_index_j][neuron_index_j] *
                interlayers_combination_combination_Jacobian[layer_index_j][
                    layer_index_i][neuron_index_j][neuron_index_i];
      }
    }
...
// Performance functional stuff
  ...
  return (numerical_differentiation_pointer->calculate_gradient(
      *this, &PerformanceTerm::calculate_performance, parameters));
}
...
Vector<double> PerformanceTerm::calculate_gradient(
...
// Neural network stuff
  ...
  return (numerical_differentiation_pointer->calculate_Hessian(
      *this, &PerformanceTerm::calculate_performance, parameters));
}
...
Matrix<double> PerformanceTerm::calculate_Hessian(
    const Vector<double>& parameters) const {
// Neural network stuff
...
PerformanceTerm::FirstOrderTerms PerformanceTerm::calculate_first_order_terms(
    void) const {
  std::ostringstream buffer;
...
/// Returns a string with the default type of performance term,
...
std::string PerformanceTerm::write_information(void) const { return (""); }
...
/// Serializes a default performance term object into a XML document of the
/// TinyXML library.
...
/// Returns the Knronecker delta of two integers a and b, which equals 1 if they
/// are equal and 0 otherwise.
/// @param a First integer.
/// @param b Second integer.

unsigned PerformanceTerm::calculate_Kronecker_delta(const unsigned& a,
