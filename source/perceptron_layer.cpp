...
PerceptronLayer::PerceptronLayer(void) { set(); }
...
PerceptronLayer::PerceptronLayer(const unsigned& new_inputs_number,
...
PerceptronLayer::PerceptronLayer(
    const PerceptronLayer& other_perceptron_layer) {
...
/// Assignment operator.
/// It assigns to this object the members of an existing perceptron layer
/// object.
/// @param other_perceptron_layer Perceptron layer object to be assigned.

PerceptronLayer& PerceptronLayer::operator=(
    const PerceptronLayer& other_perceptron_layer) {
...
bool PerceptronLayer::operator==(
    const PerceptronLayer& other_perceptron_layer) const {
...
/// Returns a constant reference to the vector of perceptrons defining the
/// layer.
...
unsigned PerceptronLayer::get_inputs_number(void) const {
  if (is_empty()) {
    return (0);
  } else {
...
// const unsigned& get_perceptrons_number(void) const

/// Returns the size of the perceptrons vector.

unsigned PerceptronLayer::get_perceptrons_number(void) const {
  const unsigned& perceptrons_number = perceptrons.size();

  return (perceptrons_number);
}

// const Perceptron& get_perceptron(const unsigned&) const method

/// Returns a reference to a given element in the perceptrons vector.
/// @param index Index of perceptron element.

const Perceptron& PerceptronLayer::get_perceptron(const unsigned& index) const {
// Control sentence (if debug)

#ifndef NDEBUG

  const unsigned perceptrons_number = get_perceptrons_number();
...
unsigned PerceptronLayer::count_parameters_number(void) const {
  const unsigned perceptrons_number = get_perceptrons_number();

  unsigned parameters_number = 0;

  for (unsigned i = 0; i < perceptrons_number; i++) {
...
// Vector<unsigned> count_cumulative_parameters_number(void) const method

/// Returns a vector of size the number of neurons in the layer,
/// where each element is equal to the total number of parameters in the current
/// and all the previous neurons.

Vector<unsigned> PerceptronLayer::count_cumulative_parameters_number(
    void) const {
  const unsigned perceptrons_number = get_perceptrons_number();

  Vector<unsigned> cumulative_parameters_number(perceptrons_number);

  if (perceptrons_number > 0) {
    cumulative_parameters_number[0] = perceptrons[0].count_parameters_number();

    for (unsigned i = 1; i < perceptrons_number; i++) {
      cumulative_parameters_number[i] =
          cumulative_parameters_number[i - 1] +
...
Vector<double> PerceptronLayer::arrange_parameters(void) const {
  ...
  if (perceptrons_number == 0) {
    ...
    const unsigned perceptron_parameters_number =
...
// unsigned count_perceptron_parameters_number(void) const method

/// Returns the number of parameters of each perceptron in this layer.
/// This is equal to one plus the number of inputs.

unsigned PerceptronLayer::count_perceptron_parameters_number(void) const {
  const unsigned inputs_number = get_inputs_number();
...
/// Each element contains the parameters of a perceptron, with size one plus the
/// number of inputs in this layer.

Vector<Vector<double> > PerceptronLayer::arrange_perceptrons_parameters(
    void) const {
  const unsigned perceptrons_number = get_perceptrons_number();
...
/// The activation function of a layer is the activation function of all
/// perceptrons in it.

const Perceptron::ActivationFunction& PerceptronLayer::get_activation_function(
    void) const {
  const unsigned perceptrons_number = get_perceptrons_number();
  ...
  if (perceptrons_number > 0) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "Perceptron::ActivationFunction& get_activation_function(void) "
              "method.\n"
...
/// This can be: Logistic, HyperbolicTangent, Threshold, SymmetricThreshold or
/// Linear.

std::string PerceptronLayer::write_activation_function(void) const {
  const Perceptron::ActivationFunction activation_function =
...
const bool& PerceptronLayer::get_display(void) const { return (display); }
...
void PerceptronLayer::set(const unsigned& new_inputs_number,
                          const unsigned& new_perceptrons_number) {
  perceptrons.set(new_perceptrons_number);

  for (unsigned i = 0; i < new_perceptrons_number; i++) {
...
/// Sets the members of this perceptron layer object with those from other
/// perceptron layer object.
...
void PerceptronLayer::set_perceptrons(
    const Vector<Perceptron>& new_perceptrons) {
  perceptrons = new_perceptrons;
}

// void set_perceptron(const unsigned&, const Perceptron&) method

/// Sets a single perceptron in the layer.
/// @param i Index of perceptron.
/// @param new_perceptron Perceptron neuron to be set.

void PerceptronLayer::set_perceptron(const unsigned& i,
...
/// Sets those members not related to the vector of perceptrons to their default
/// value.
/// <ul>
/// <li> Display: True.
/// </ul>

void PerceptronLayer::set_default(void) { display = true; }
...
void PerceptronLayer::set_inputs_number(const unsigned& new_inputs_number) {
  const unsigned perceptrons_number = get_perceptrons_number();
...
// void set_perceptrons_number(const unsigned&) method
...
void PerceptronLayer::set_perceptrons_number(
    const unsigned& new_perceptrons_number) {
  const unsigned perceptrons_number = get_perceptrons_number();
  const unsigned inputs_number = get_inputs_number();
...
// Control sentence (if debug)
  ...
  const unsigned new_biases_size = new_biases.size();
...
void PerceptronLayer::set_synaptic_weights(
    const Matrix<double>& new_synaptic_weights) {
  const unsigned inputs_number = get_inputs_number();
  const unsigned perceptrons_number = get_perceptrons_number();
...
// Control sentence (if debug)
  ...
  const unsigned rows_number = new_synaptic_weights.get_rows_number();
  const unsigned columns_number = new_synaptic_weights.get_columns_number();
...
// Control sentence (if debug)
  ...
  const unsigned new_parameters_size = new_parameters.size();
  ...
  if (new_parameters_size != parameters_number) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "void set_parameters(const Vector<double>&) method.\n"
           << "Size of new parameters vector must be equal to number of "
              "parameters.\n";
  ...
  if (perceptrons_number != 0) {
    ...
    for (unsigned i = 0; i < perceptrons_number; i++) {
      perceptron_parameters =
          new_parameters.take_out(position, perceptron_parameters_number);
...
void PerceptronLayer::set_activation_function(
...
/// The second argument is a string containing the name of the function
/// ("Logistic", "HyperbolicTangent", "Threshold", etc).
/// @param new_activation_function Activation function for that layer.

void PerceptronLayer::set_activation_function(
    const std::string& new_activation_function) {
  const unsigned perceptrons_number = get_perceptrons_number();
...
void PerceptronLayer::prune_input(const unsigned& index) {
// Control sentence (if debug)
...
// void prune_perceptron(const unsigned&) method

/// This method removes a given perceptron from the layer.
/// @param index Index of perceptron to be pruned.

void PerceptronLayer::prune_perceptron(const unsigned& index) {
// Control sentence (if debug)
  ...
  if (index >= perceptrons_number) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "void prune_perceptron(const unsigned&) method.\n"
           << "Index of perceptron is equal or greater than number of "
              "perceptrons.\n";
...
/// Initializes the perceptron layer with a random number of inputs and a randon
/// number of perceptrons.
/// That can be useful for testing purposes.

void PerceptronLayer::initialize_random(void) {
  const unsigned inputs_number = rand() % 10 + 1;
  const unsigned perceptrons_number = rand() % 10 + 1;
...
/// Initializes the biases of all the perceptrons in the layer of perceptrons
/// with a given value.
...
/// Initializes the synaptic weights of all the perceptrons in the layer of
/// perceptrons perceptron with a given value.
...
/// Initializes all the biases and synaptic weights in the layer of perceptrons
/// at random with values
/// comprised between a minimum and a maximum values.
/// @param minimum Minimum initialization value.
/// @param maximum Maximum initialization value.

void PerceptronLayer::randomize_parameters_uniform(const double& minimum,
...
/// Initializes all the biases an synaptic weights in the layer of perceptrons
...
/// Returns the combination to every perceptron in the layer as a function of
/// the inputs to that layer.
/// @param inputs Inputs to the layer.

Vector<double> PerceptronLayer::calculate_combinations(
...
/// Returns the partial derivatives of the combination of a layer with respect
/// to the inputs.
/// All that partial derivatives are arranged in the so called Jacobian matrix
/// of the layer combination function.

Matrix<double> PerceptronLayer::calculate_combinations_Jacobian(
    const Vector<double>&) const {
  return (arrange_synaptic_weights());
}

// Vector< Matrix<double> > calculate_combinations_Hessian_form(const
// Vector<double>&) const method

/// Returns the second partial derivatives of the combination of a layer with
/// respect to the inputs of that layer.
/// All that partial derivatives are arranged in the so called Hessian form,
/// represented as a vector of matrices,
/// of the layer combination function.

Vector<Matrix<double> > PerceptronLayer::calculate_combinations_Hessian_form(
    const Vector<double>&) const {
  const unsigned inputs_number = get_inputs_number();
  const unsigned perceptrons_number = get_perceptrons_number();
...
/// Returns which would be the combination of a layer as a function of the
/// inputs and for a set of parameters.
...
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "Vector<double> calculate_combination_parameters(const "
  ...
  if (parameters_size != parameters_number) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           ...
           << ") must be equal to number of lasyer parameters ("
           << parameters_number << ").\n";
  ...
  for (unsigned i = 0; i < perceptrons_number; i++) {
    perceptron_parameters = parameters.take_out(
        i * perceptron_parameters_number, perceptron_parameters_number);
...
// Matrix<double> calculate_combination_parameters_Jacobian(const
// Vector<double>&) const method

/// Returns the partial derivatives of the combination of a layer with respect
/// to the parameters in that layer, for a given set of inputs.
/// All that partial derivatives are arranged in the so called Jacobian matrix
/// of the layer combination function.
/// @param inputs Vector of inputs to that layer.
...
Matrix<double> PerceptronLayer::calculate_combinations_Jacobian(
  ...
  Matrix<double> combinations_Jacobian(perceptrons_number, parameters_number,
                                       0.0);
...
// Vector< Matrix<double> > calculate_combination_parameters_Hessian_form(const
// Vector<double>&) const method
...
Vector<Matrix<double> > PerceptronLayer::calculate_combinations_Hessian_form(
  ...
  for (unsigned i = 0; i < perceptrons_number; i++) {
    combination_parameters_Hessian_form[i]
...
/// Returns the activations from every perceptron in a layer as a function of
/// their combination.
/// @param combinations Combination from every neuron in the layer.

Vector<double> PerceptronLayer::calculate_activations(
...
// Control sentence (if debug)
  ...
  const unsigned combination_size = combinations.size();

  if (combination_size != perceptrons_number) {
    std::ostringstream buffer;

    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "Vector<double> calculate_activation(const Vector<double>&) "
              "const method.\n"
...
/// Returns the activation derivative from every perceptron in the layer as a
/// function of its combination.
/// @param combination Combination to every neuron in the layer.

Vector<double> PerceptronLayer::calculate_activations_derivatives(
    const Vector<double>& combination) const {
  const unsigned perceptrons_number = get_perceptrons_number();
...
// Control sentence (if debug)
  ...
  const unsigned combination_size = combination.size();

  if (combination_size != perceptrons_number) {
    std::ostringstream buffer;

    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "Vector<double> calculate_activations_derivatives(const "
              "Vector<double>&) const method.\n"
...
// Vector<double> calculate_activations_second_derivatives(const
// Vector<double>&) const method

/// Returns the activation second derivative from every perceptron as a function
/// of their combination.
/// @param combination Combination to every perceptron in the layer.

Vector<double> PerceptronLayer::calculate_activations_second_derivatives(
    const Vector<double>& combination) const {
  const unsigned perceptrons_number = get_perceptrons_number();
...
// Control sentence (if debug)
  ...
  for (unsigned i = 0; i < perceptrons_number; i++) {
    activation_second_derivatives[i] =
...
Matrix<double> PerceptronLayer::arrange_activations_Jacobian(
    const Vector<double>& activation_derivative) const {
  const unsigned perceptrons_number = get_perceptrons_number();

  Matrix<double> activation_Jacobian(perceptrons_number, perceptrons_number,
                                     0.0);
...
// Vector< Matrix<double> > arrange_activations_Hessian_form(const
// Vector<double>&) const method

/// Arranges a "Hessian form" vector of matrices from a vector of second
/// derivatives.
/// @param activation_second_derivative Vector of activation function second
/// derivatives.

Vector<Matrix<double> > PerceptronLayer::arrange_activations_Hessian_form(
    const Vector<double>& activation_second_derivative) const {
  const unsigned perceptrons_number = get_perceptrons_number();
...
/// Returns the outputs from every perceptron in a layer as a function of their
/// inputs.
/// @param inputs Input vector to the layer.

Vector<double> PerceptronLayer::calculate_outputs(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
...
/// This is composed by the derivatives of the layer outputs with respect to
/// their inputs.
/// The number of rows is the number of neurons in the layer.
/// The number of columns is the number of inputs to that layer.
/// @param inputs Input to layer.

Matrix<double> PerceptronLayer::calculate_Jacobian(
    const Vector<double>& inputs) const {
...
// Control sentence (if debug)
  ...
  const Vector<double> activations_derivatives =
      calculate_activations_derivatives(combinations);
...
/// Returns the second partial derivatives of the outputs from a layer with
/// respect to the inputs to that layer.
/// @param inputs Vector of inputs to that layer.

Vector<Matrix<double> > PerceptronLayer::calculate_Hessian_form(
    const Vector<double>& inputs) const {
  const unsigned perceptrons_number = get_perceptrons_number();
  ...
  const Vector<double> activations_second_derivatives =
      calculate_activations_second_derivatives(combination);
  ...
  for (unsigned i = 0; i < perceptrons_number; i++) {
    ...
    Hessian_form[i] = synaptic_weights.calculate_transpose()
        .dot(activation_Hessian_form[i]).dot(synaptic_weights);
  }
...
/// Returns which would be the outputs from a layer for a given inputs to that
/// layer and a set of parameters in that layer.
/// @param inputs Vector of inputs to that layer.
...
/// This is composed by the derivatives of the layer outputs with respect to the
/// layer parameters.
/// The number of rows is the number of neurons in the layer.
...
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: PerceptronLayer class.\n"
           << "Matrix<double> calculate_parameters_Jacobian(const "
              "Vector<double>&, const Vector<double>&) const method.\n"
  ...
  const Vector<double> combinations =
      calculate_combinations(inputs, parameters);

  const Matrix<double> combinations_Jacobian =
      calculate_combinations_Jacobian(inputs, parameters);

  const Vector<double> activation_derivatives =
      calculate_activations_derivatives(combinations);

  const Matrix<double> activation_Jacobian =
      arrange_activations_Jacobian(activation_derivatives);
...
/// Returns the second partial derivatives of the outputs from a layer with
/// respect a given set of potential parameters for this layer,
/// and for a given set of inputs.
/// This quantity is the Hessian form of the layer outputs function, and it is
/// represented as a vector of matrices.
...
// Control sentence (if debug)
  ...
  const Matrix<double> combination_parameters_Jacobian =
      calculate_combinations_Jacobian(inputs, parameters);

  const Vector<double> activation_second_derivatives =
      calculate_activations_second_derivatives(combination);

  const Vector<Matrix<double> > activation_Hessian_form =
      arrange_activations_Hessian_form(activation_second_derivatives);
  ...
  for (unsigned i = 0; i < perceptrons_number; i++) {
    parameters_Hessian_form[i] = combination_parameters_Jacobian
        .calculate_transpose().dot(activation_Hessian_form[i])
...
/// Returns a string with the expression of the inputs-outputs relationship of
/// the layer.
...
std::string PerceptronLayer::write_expression(
...
// Control sentence (if debug)
  ...
  const unsigned inputs_number = get_inputs_number();
  const unsigned inputs_name_size = inputs_name.size();
  ...
  const unsigned outputs_name_size = outputs_name.size();
