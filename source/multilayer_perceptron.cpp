...
/// This constructor also initializes the rest of class members to their default
/// values.

MultilayerPerceptron::MultilayerPerceptron(void) { set(); }
...
MultilayerPerceptron::MultilayerPerceptron(
    const Vector<PerceptronLayer>& new_layers) {
  set(new_layers);
}
...
/// @param new_architecture Vector of unsignedegers representing the
/// architecture of the multilayer perceptron.

MultilayerPerceptron::MultilayerPerceptron(
    const Vector<unsigned>& new_architecture) {
...
MultilayerPerceptron::MultilayerPerceptron(const unsigned& new_inputs_number,
                                           const unsigned& new_neurons_number) {
...
/// @param new_hidden_neurons_number Number of neurons in the hidden layer of
/// the multilayer perceptron
/// @param new_outputs_number Number of outputs neurons.

MultilayerPerceptron::MultilayerPerceptron(
    const unsigned& new_inputs_number,
    const unsigned& new_hidden_neurons_number,
    const unsigned& new_outputs_number) {
...
/// @param other_multilayer_perceptron Multilayer perceptron object to be
/// copied.

MultilayerPerceptron::MultilayerPerceptron(
    const MultilayerPerceptron& other_multilayer_perceptron) {
...
/// Assignment operator.
/// It assigns to this object the members of an existing multilayer perceptron
/// object.
/// @param other_multilayer_perceptron Multilayer perceptron object to be
/// assigned.

MultilayerPerceptron& MultilayerPerceptron::operator=(
    const MultilayerPerceptron& other_multilayer_perceptron) {
...
/// It returns true if the members of the two objects have the same values, and
/// false otherwise.
/// @ param other_multilayer_perceptron Multilayer perceptron to be compared
/// with.

bool MultilayerPerceptron::operator==(
    const MultilayerPerceptron& other_multilayer_perceptron) const {
...
const PerceptronLayer& MultilayerPerceptron::get_layer(
...
unsigned MultilayerPerceptron::count_perceptrons_number(void) const {
  const Vector<unsigned> layers_perceptrons_number =
      arrange_layers_perceptrons_numbers();
...
/// Returns a vector of size the number of layers, where each element is equal
/// to the total number of neurons in the current and all the previous layers.

Vector<unsigned> MultilayerPerceptron::count_cumulative_perceptrons_number(
    void) const {
  const unsigned layers_number = get_layers_number();
  ...
  if (layers_number != 0) {
    ...
    for (unsigned i = 1; i < layers_number; i++) {
      cumulative_neurons_number[i] =
          cumulative_neurons_number[i - 1] + layers_size[i];
    }
  }
...
/// where each element contains the number of parameters in the corresponding
/// layer.

Vector<unsigned> MultilayerPerceptron::arrange_layers_parameters_number(
    void) const {
  const unsigned layers_number = get_layers_number();
...
/// Returns a vector of integers with size the number of layers,
/// where each element contains the total number of parameters in the
/// corresponding and the previous layers.

Vector<unsigned>
MultilayerPerceptron::arrange_layers_cumulative_parameters_number(void) const {
  const unsigned layers_number = get_layers_number();
  ...
  for (unsigned i = 1; i < layers_number; i++) {
    layers_cumulative_parameters_number[i] =
        layers_cumulative_parameters_number[i - 1] +
...
/// The size of each subvector is the number of neurons in the corresponding
/// layer.

Vector<Vector<double> > MultilayerPerceptron::arrange_layers_biases(
    void) const {
  const unsigned layers_number = get_layers_number();
...
/// The size of this vector is the number of layers.
/// The number of rows of each sub_matrix is the number of neurons in the
/// corresponding layer.
/// The number of columns of each sub_matrix is the number of inputs to the
/// corresponding layer.

Vector<Matrix<double> > MultilayerPerceptron::arrange_layers_synaptic_weights(
    void) const {
  const unsigned layers_number = get_layers_number();
...
/// Returns the neural parameter values (biases and synaptic weights) from the
/// neurons in all
/// the layers.
/// The format is a vector of matrices of real values.
/// The size of this vector is the number of layers.
/// The number of rows of each sub_matrix is the number of neurons in the
/// corresponding layer.
/// The number of columns of each sub_matrix is the number of parameters (inputs
/// + 1) to the corresponding layer.

Vector<Vector<double> > MultilayerPerceptron::get_layers_parameters(
    void) const {
  const unsigned layers_number = get_layers_number();
...
/// Returns the number of parameters (biases and synaptic weights) in the
...
/// Returns the values of all the biases and synaptic weights in the multilayer
/// perceptron as a single vector.
...
/// Returns the number of parameters for each layer in this multilayer
/// perceptron.
/// The format is a vector with size the number of layers.
/// Each element contains the number of parameters (biases and synaptic weights)
/// in the corresponding layer.

Vector<unsigned> MultilayerPerceptron::count_layers_parameters_numbers(
    void) const {
  const unsigned layers_number = get_layers_number();
...
unsigned MultilayerPerceptron::get_layer_index(
    const unsigned& neuron_index) const {
// Control sentence (if debug)
  ...
  const Vector<unsigned> cumulative_neurons_number =
      count_cumulative_perceptrons_number();

  const unsigned layer_index =
      cumulative_neurons_number.calculate_cumulative_index(neuron_index);
...
/// Returns the index of a neuron, given the layer it belongs and its position
/// in that layer.
/// @param layer_index Index of layer.
/// @param perceptron_position Position on the perceptron in that layer.

unsigned MultilayerPerceptron::get_perceptron_index(
    const unsigned& layer_index, const unsigned& perceptron_position) const {
  if (layer_index == 0) {
...
unsigned MultilayerPerceptron::get_layer_bias_index(
    const unsigned& layer_index, const unsigned& perceptron_index) const {
// Control sentence (if debug)
  ...
  if (layer_index >= layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "unsigned get_layer_bias_index(const unsigned&, const unsigned&) "
              "const method.\n"
           << "Index of layer must be less than number of layers.\n";
  ...
  const unsigned layer_perceptrons_number =
  ...
  if (perceptron_index >= layer_perceptrons_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "unsigned get_layer_bias_index(const unsigned&, const unsigned&) "
              "const method.\n"
           << "Index of perceptron must be less than number of perceptrons in "
              "that layer.\n";
...
// unsigned get_layer_synaptic_weight_index(const unsigned&, const unsigned&,
...
unsigned MultilayerPerceptron::get_layer_synaptic_weight_index(
    const unsigned& layer_index, const unsigned& perceptron_index,
    const unsigned& input_index) const {
// Control sentence (if debug)
  ...
  if (layer_index >= layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "unsigned get_layer_synaptic_weight_index(const unsigned&, const "
              "unsigned&, const unsigned&) method.\n"
           << "Index of layer must be less than number of layers.\n";
  ...
  if (input_index >= layer_inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "unsigned get_layer_synaptic_weight_index(const unsigned&, const "
              "unsigned&, const unsigned&) method.\n"
           << "Index of inputs must be less than number of inputs in "
              "perceptron.\n";
  ...
  if (layer_index > 0) {
    for (unsigned i = 0; i < layer_index - 1; i++) {
      layer_synaptic_weight_index +=
          layers[layer_index].count_parameters_number();
    }
  }
  ...
  if (perceptron_index > 0) {
    for (unsigned i = 0; i < perceptron_index - 1; i++) {
      layer_synaptic_weight_index +=
          layers[layer_index].get_perceptron(i).count_parameters_number();
    }
  }
...
Vector<unsigned> MultilayerPerceptron::arrange_parameter_indices(
    const unsigned& parameter_index) const {
// Control sentence (if debug)
  ...
  if (parameter_index >= parameters_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector<int> arrange_parameter_indices(const unsigned&) const "
              "method.\n"
           << "Index of neural parameter must be less than number of "
...
/// That indices include the layer index, the neuron index and the parameter
/// index.
...
// Vector<Perceptron::ActivationFunction> get_layers_activation_function(void)
// const method

/// Returns the activation function of every layer in a single vector.

Vector<Perceptron::ActivationFunction>
MultilayerPerceptron::get_layers_activation_function(void) const {
  const unsigned layers_number = get_layers_number();
...
/// Returns a vector of strings with the name of the activation functions for
/// the layers.
/// The size of this vector is the number of layers.

Vector<std::string> MultilayerPerceptron::write_layers_activation_function(
    void) const {
  const unsigned layers_number = get_layers_number();
...
const bool& MultilayerPerceptron::get_display(void) const { return (display); }

// void set_default(void) method

/// Sets those members not related to the multilayer perceptron architecture to
...
void MultilayerPerceptron::set(void) { layers.set(); }

// void set(const Vector<PerceptronLayer>&) method

/// Sets a multilayer_perceptron_pointer architecture with given layers of
/// perceptrons.
/// @param new_layers Vector of vectors of perceptrons, which represent the
/// multilayer perceptron architecture.
...
/// The rest of elements represent the corresponding number of perceptrons in
/// each layer.
...
/// @param new_perceptrons_number Number of perceptrons in the unique layer.
/// This is also the number of outputs.

void MultilayerPerceptron::set(const unsigned& new_inputs_number,
                               const unsigned& new_perceptrons_number) {
// Control sentence (if debug)
...
/// Sets a new multilayer_perceptron_pointer architecture with two layers, a
/// hidden layer and an outputs layer.
/// @param new_inputs_number Number of inputs to the multilayer perceptron.
...
/// @param new_layers_size New numbers of neurons for the layers of the
/// multilayer perceptron
/// The number of elements of this vector is the number of layers.

void MultilayerPerceptron::set_layers_perceptrons_number(
    const Vector<unsigned>& new_layers_size) {
...
/// All the parameters of the multilayer perceptron are initialized at random.
/// @param layer_index Index of layer.
/// @param new_layer_perceptrons_number New numbers of neurons for that layer of
/// the multilayer perceptron

void MultilayerPerceptron::set_layer_perceptrons_number(
    const unsigned& layer_index, const unsigned& new_layer_perceptrons_number) {
  const unsigned layer_inputs_number = layers[layer_index].get_inputs_number();
...
/// Sets a new multilayer_perceptron_pointer architecture by means of a pack of
/// layers of perceptrons.
/// @param new_layers Vector o vectors of perceptron neurons representing a
/// multilayer_perceptron_pointer architecture arranged in layers.

void MultilayerPerceptron::set_layers(
    const Vector<PerceptronLayer>& new_layers) {
  layers = new_layers;
}
...
void MultilayerPerceptron::set_layers_biases(
    const Vector<Vector<double> >& new_layers_biases) {
  const unsigned layers_number = get_layers_number();
...
// Control sentence (if debug)
  ...
  if (size != layers_number) {
    ...
    buffer
        << "OpenNN Exception: MultilayerPerceptron class.\n"
        << "void set_layers_biases(const Vector< Vector<double> >&) method.\n"
        << "Size (" << size << ") must be equal to number of layers ("
        << layers_number << ").\n";
...
/// The number of columns of each sub_matrix is the number of inputs to the
/// corresponding layer.
/// @param new_layers_synaptic_weights New set of synaptic weights in the
/// layers.

void MultilayerPerceptron::set_layers_synaptic_weights(
    const Vector<Matrix<double> >& new_layers_synaptic_weights) {
  const unsigned layers_number = get_layers_number();
...
/// Sets the multilayer perceptron parameters of all layers in the multilayer
/// perceptron
...
void MultilayerPerceptron::set_layers_parameters(
    const Vector<Vector<double> >& new_layers_parameters) {
  const unsigned layers_number = get_layers_number();
...
/// Sets all the biases and synaptic weights in the multilayer perceptron from a
/// single vector.
/// @param new_parameters New set of biases and synaptic weights values.

void MultilayerPerceptron::set_parameters(
...
// Control sentence (if debug)
  ...
  if (size != parameters_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "void set_parameters(const Vector<double>&) method.\n"
           << "Size (" << size
           << ") must be equal to number of biases and synaptic weights ("
           << parameters_number << ").\n";
  ...
  for (unsigned i = 0; i < layers_number; i++) {
    ...
    layer_parameters =
        new_parameters.take_out(position, layer_parameters_number);
    layers[i].set_parameters(layer_parameters);
    position += layer_parameters_number;
  }
}

// void set_layers_activation_function(const
// Vector<Perceptron::ActivationFunction>&) method

/// This class sets a new activation (or transfer) function in all the layers of
/// the multilayer perceptron
/// @param new_layers_activation_function Activation function for the layers.
/// The size of this Vector must be equal to the number of layers, and each
/// element corresponds
/// to the activation function of one layer.

void MultilayerPerceptron::set_layers_activation_function(const Vector<
    Perceptron::ActivationFunction>& new_layers_activation_function) {
  const unsigned layers_number = get_layers_number();
...
// Control sentence (if debug)
  ...
  const unsigned new_layers_activation_function_size =
      new_layers_activation_function.size();
  ...
  if (new_layers_activation_function_size != layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "void set_layers_activation_function(const "
              "Vector<Perceptron::ActivationFunction>&) method.\n"
           << "Size of activation function of layers must be equal to number "
              "of layers.\n";
...
// void set_layer_activation_function(const Perceptron::ActivationFunction&)
// method

/// This class sets a new activation (or transfer) function in a single layer of
/// the multilayer perceptron
/// @param i Index of layer.
...
// Control sentence (if debug)
  ...
  if (i >= layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "void set_layer_activation_function(const "
              "Perceptron::ActivationFunction&) method.\n"
...
/// The argument is a string containing the name of the function ("Logistic",
/// "HyperbolicTangent", "Threshold", etc).
/// @param new_layers_activation_function Activation function for the layers.

void MultilayerPerceptron::set_layers_activation_function(
    const Vector<std::string>& new_layers_activation_function) {
  const unsigned layers_number = get_layers_number();
...
// Control sentence (if debug)
  ...
  if (size != layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "void set_layer_activation_function(const Vector<std::string>&) "
              "method.\n"
           << "Size of layers activation function is not equal to number of "
              "layers.\n";
...
/// Returns true if the number of layers in the multilayer perceptron is zero,
/// and false otherwise.
...
/// Initializes the biases of all the perceptrons in the multilayer perceptron
/// with a given value.
...
/// Initializes the synaptic weights of all the perceptrons in the multilayer
/// perceptron with a given value.
...
/// Initializes all the biases and synaptic weights in the neural newtork with a
/// given value.
...
/// Initializes all the biases and synaptic weights in the neural newtork at
/// random with values comprised
/// between -1 and +1.
...
/// Initializes all the biases and synaptic weights in the multilayer perceptron
/// at random with values
/// comprised between a minimum and a maximum values.
/// @param minimum Minimum initialization value.
/// @param maximum Maximum initialization value.

void MultilayerPerceptron::randomize_parameters_uniform(const double& minimum,
...
// void randomize_parameters_uniform(const Vector<double>&, const
...
/// Initializes all the biases and synaptic weights in the multilayer perceptron
/// at random, with values
/// comprised between a different minimum and maximum numbers for each
/// parameter.
/// All minimum are maximum initialization values must be given from a vector of
/// two real vectors.
/// The first element must contain the minimum inizizalization value for each
/// parameter.
/// The second element must contain the maximum inizizalization value for each
/// parameter.
/// @param minimum_maximum Vector of minimum and maximum initialization values.

void MultilayerPerceptron::randomize_parameters_uniform(
    const Vector<Vector<double> >& minimum_maximum) {
  const unsigned parameters_number = count_parameters_number();
...
/// Initializes all the biases and synaptic weights in the newtork with random
/// values chosen from a
...
/// Initializes all the biases and synaptic weights in the multilayer perceptron
/// with random random values
/// chosen from a normal distribution with a given mean and a given standard
/// deviation.
...
// void randomize_parameters_normal(const Vector<double>&, const
// Vector<double>&) method

/// Initializes all the biases an synaptic weights in the multilayer perceptron
/// with random values chosen
/// from normal distributions with different mean and standard deviation for
/// each parameter.
...
/// from normal distributions with different mean and standard deviation for
/// each parameter.
/// All mean and standard deviation values are given from a vector of two real
/// vectors.
/// The first element must contain the mean value for each parameter.
/// The second element must contain the standard deviation value for each
/// parameter.
/// @param mean_standard_deviation Vector of mean and standard deviation values.

void MultilayerPerceptron::randomize_parameters_normal(
    const Vector<Vector<double> >& mean_standard_deviation) {
  const unsigned parameters_number = count_parameters_number();

  Vector<double> parameters(parameters_number);

  parameters.randomize_normal(mean_standard_deviation[0],
                              mean_standard_deviation[1]);
...
/// Initializes the parameters at random with values chosen from a normal
...
/// Returns the outputs from the multilayer perceptron for a given set of inputs
/// to it.
/// That is, it computes the inputs-outputs relationship of the raw multilayer
/// perceptron
/// @param inputs Vector of inputs to the first layer of the multilayer
/// perceptron.

Vector<double> MultilayerPerceptron::calculate_outputs(
...
/// Returns the partial derivatives of the outputs from the last layer with
/// respect to the inputs to the first layer.
/// That is, it computes the inputs-outputs partial derivatives of the raw
/// multilayer perceptron.
/// @param inputs Vector of inputs to the first layer of the multilayer
/// perceptron architecture.

Matrix<double> MultilayerPerceptron::calculate_Jacobian(
    const Vector<double>& inputs) const {
#ifndef NDEBUG
  ...
  if (layers_number == 0) {
    ...
    const Vector<Matrix<double> > layers_Jacobian =
        calculate_layers_Jacobian(inputs);
...
/// Returns the second partial derivatives of the outputs from the last layer
/// with respect to the inputs to the first layer.
/// That is, the Hessian matrix of the multilayer perceptron architectue
/// inputs-outputs function.
/// @todo

Vector<Matrix<double> > MultilayerPerceptron::calculate_Hessian_form(
    const Vector<double>& inputs) const {
#ifndef NDEBUG
  ...
  if (size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Matrix<double> > calculate_Hessian_form(const "
  ...
  const Vector<Vector<Matrix<double> > > layers_Hessian_form =
      calculate_layers_Hessian_form(inputs);
  ...
  for (unsigned layer_index = layers_number - 2; layer_index != 0;
       layer_index--) {
    for (unsigned output_index = 0; output_index < outputs_number;
         output_index++) {
      Hessian_form[output_index] = (layers_Jacobian[layer_index]
                                        .calculate_transpose())
          .dot(Hessian_form[output_index]).dot(layers_Jacobian[layer_index]);

      for (unsigned neuron_index = 0; neuron_index < layers_size[layer_index];
           neuron_index++) {
        Hessian_form[output_index] +=
            layers_Hessian_form[layer_index][neuron_index] *
            Jacobian[output_index][neuron_index];
      }
    }
  ...
  //   const Vector< Matrix<double> > hidden_layer_Hessian_form =
  // layers_Hessian_form[0];
  //   const Vector< Matrix<double> > output_layer_Hessian_form =
  // layers_Hessian_form[1];
  ...
  //   {
  //      Hessian_form[output_index] =
  // (hidden_layer_Jacobian.calculate_transpose()).dot(output_layer_Hessian_form[output_index]).dot(hidden_layer_Jacobian);

  //      for(unsigned neuron_index = 0; neuron_index < layers_size[0];
  // neuron_index++)
  //      {
  //         Hessian_form[output_index] +=
  // hidden_layer_Hessian_form[neuron_index]*output_layer_Jacobian[output_index][neuron_index];
  //	  }
  //   }
...
// Vector<double> calculate_layer_combination_combination(const unsigned&, const
// Vector<double>&) const method

/// Returns the combination vector of a given layer as a function of the
/// combination vector of the previous layer.
/// @param layer_index Index of layer.
/// @param previous_layer_combination Combination vector of previous layer.

Vector<double> MultilayerPerceptron::calculate_layer_combination_combination(
    const unsigned& layer_index,
    const Vector<double>& previous_layer_combination) const {
// Control sentence (if debug)
  ...
  if (layer_index == 0) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix<double> calculate_layer_combination_combination(const "
              "unsigned&, const Vector<double>&) const.\n"
  ...
  const unsigned previous_layer_perceptrons_number =
  ...
  const Vector<double> previous_layer_activation =
      layers[layer_index - 1].calculate_activations(previous_layer_combination);

  const Vector<double> layer_combination_combination =
      layers[layer_index].calculate_combinations(previous_layer_activation);

  return (layer_combination_combination);
}

// Matrix<double> calculate_layer_combination_combination_Jacobian(const
// unsigned&, const Vector<double>&) const method

/// Returns the partial derivatives of the combinations of one layer with
/// respect to the combinations in the previous layer.
/// Note that, for efficiency issues, the argument here is the activation
/// derivative of the previous layer, and not the combination of the previous
/// layer.
/// This quantity is the Jacobian matrix of the layer combination-combination
/// function.
/// @param layer_index Index of layer.
/// @param previous_layer_activation_derivative Vector of activation derivatives
/// of the previous layer.

Matrix<double>
MultilayerPerceptron::calculate_layer_combination_combination_Jacobian(
    const unsigned& layer_index,
    const Vector<double>& previous_layer_activation_derivative) const {
  const unsigned previous_layer_perceptrons_number =
      layers[layer_index - 1].get_perceptrons_number();
...
// Control sentence (if debug)
  ...
  if (layer_index == 0) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix<double> "
              "calculate_layer_combination_combination_Jacobian(const "
              "unsigned&, const Vector<double>&) const.\n"
           << "Index of layer must be greater than zero.\n";
  ...
  if (size != previous_layer_perceptrons_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix<double> "
              "calculate_layer_combination_combination_Jacobian(const "
              "unsigned&, const Vector<double>&).\n"
           << "Size of activation derivative must be equal to size of previous "
              "layer.\n";
  ...
  const Matrix<double> layer_synaptic_weights =
      layers[layer_index].arrange_synaptic_weights();

  Matrix<double> previous_layer_activation_Jacobian(
      previous_layer_perceptrons_number, previous_layer_perceptrons_number,
      0.0);
  previous_layer_activation_Jacobian.set_diagonal(
      previous_layer_activation_derivative);

  return (layer_synaptic_weights.dot(previous_layer_activation_Jacobian));
}

// Vector<double> calculate_interlayer_combination_combination(const unsigned&,
// const unsigned&, const Vector<double>&) const method

/// Returns the combination vector of a layer (image) with respect to the
/// combination vector of another layer (domain).
...
MultilayerPerceptron::calculate_interlayer_combination_combination(
    const unsigned& domain_layer_index, const unsigned& image_layer_index,
    const Vector<double>& domain_layer_combination) const {
  if (domain_layer_index > image_layer_index) {
    std::ostringstream buffer;

    buffer
        << "OpenNN Exception: MultilayerPerceptron class.\n"
        << "Vector<double> calculate_interlayer_combination_combination(const "
           "unsigned&, const unsigned&, const Vector<double>&) const method.\n"
        << "Index of domain layer must be less or equal than index of image "
           "layer.\n";
  ...
  if (domain_layer_index == image_layer_index) {
    ...
    for (unsigned i = domain_layer_index + 1; i <= image_layer_index; i++) {
      interlayer_combination_combination =
          calculate_layer_combination_combination(
              i, interlayer_combination_combination);
    }
...
// Matrix<double> calculate_interlayer_combination_combination_Jacobian(const
// unsigned&, const unsigned&, const Vector<double>&) const method

/// Returns the partial derivatives of the combinations of a layer (image) with
/// respect to the combinations of another layer (domain).
/// This quantity is the Jacobian matrix of the interlayers combination
/// combination function.
/// @param image_layer_index Index of image layer.
/// @param domain_layer_index Index of domain layer.
/// @param domain_layer_combination Vector of combination values of the domain
/// layer.

Matrix<double>
MultilayerPerceptron::calculate_interlayer_combination_combination_Jacobian(
    const unsigned& domain_layer_index, const unsigned& image_layer_index,
    const Vector<double>& domain_layer_combination) const {
  ...
  if (domain_layer_index < image_layer_index) {
    ...
    layers_combination_combination[0] = calculate_layer_combination_combination(
        domain_layer_index + 1, domain_layer_combination);
    layers_combination_combination_Jacobian[0] =
        calculate_layer_combination_combination_Jacobian(
            domain_layer_index + 1,
            layers[domain_layer_index]
                .calculate_activations_derivatives(domain_layer_combination));

    for (unsigned i = 1; i < size; i++) {
      layers_combination_combination[i] =
          calculate_layer_combination_combination(
              domain_layer_index + i + 1,
              layers_combination_combination[i - 1]);

      layers_combination_combination_Jacobian[i] =
          calculate_layer_combination_combination_Jacobian(
              domain_layer_index + i + 1,
              layers_combination_combination[i - 1]);
    }

    interlayer_combination_combination_Jacobian =
        layers_combination_combination_Jacobian[size - 1];

    for (int i = size - 2; i > -1; i--) {
      interlayer_combination_combination_Jacobian =
          interlayer_combination_combination_Jacobian.dot(
              layers_combination_combination_Jacobian[i]);
    }
  } else if (domain_layer_index == image_layer_index) {
    const unsigned image_layer_perceptrons_number =
        layers[image_layer_index].get_perceptrons_number();

    interlayer_combination_combination_Jacobian.set_identity(
        image_layer_perceptrons_number);
  } else {
    const unsigned image_layer_perceptrons_number =
        layers[image_layer_index].get_perceptrons_number();
    const unsigned domain_layer_perceptrons_number =
        layers[domain_layer_index].get_perceptrons_number();

    interlayer_combination_combination_Jacobian.set(
        image_layer_perceptrons_number, domain_layer_perceptrons_number, 0.0);
  }

  return (interlayer_combination_combination_Jacobian);
}

// Vector<double> calculate_output_layer_combination(const unsigned&, const
// Vector<double>&) const method

/// Calculates the output values from the neural network from the combination
/// values of a given layer.
/// @param layer_index Index of a layer.
/// @param layer_combinations Combination values of the layer with the previous
/// index.

Vector<double> MultilayerPerceptron::calculate_output_layer_combination(
    const unsigned& layer_index,
    const Vector<double>& layer_combinations) const {
// Control sentence (if debug)
  ...
  if (size != layer_perceptrons_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector<double> calculate_output_layer_combination(const "
              "unsigned&, const Vector<double>&) const method.\n"
  ...
  Vector<double> outputs =
      layers[layer_index].calculate_activations(layer_combinations);
...
// Vector< Matrix<double> > calculate_output_layers_delta(const Vector<
// Vector<double> >&, const Vector<double>&) method

/// Calculates the output delta matrix for each layer in the multilayer
/// perceptron.
/// The format is a vector of matrices.
/// @param layers_activation_derivative Activation derivative values for each
/// layer.

Vector<Matrix<double> > MultilayerPerceptron::calculate_output_layers_delta(
    const Vector<Vector<double> >& layers_activation_derivative) const {
  // Control sentence (if debug)
...
// Control sentence (if debug)
  ...
  const unsigned layers_activation_derivative_size =
      layers_activation_derivative.size();

  if (layers_activation_derivative_size != layers_number) {
    std::ostringstream buffer;

    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector<double> > calculate_output_layers_delta(const "
              "Vector< Vector<double> >&) method.\n"
           << "Size of forward propagation activation derivative vector must "
  ...
  if (layers_number > 0) {
    ...
    output_layers_delta[layers_number - 1] =
        layers[layers_number - 1].arrange_activations_Jacobian(
            layers_activation_derivative[layers_number - 1]);

    // Rest of layers

    for (int i = layers_number - 2; i >= 0; i--) {
      output_layers_delta[i] = output_layers_delta[i + 1]
          .dot(layers[i + 1].arrange_synaptic_weights())
          .dot(layers[i].arrange_activations_Jacobian(
...
// Matrix< Vector< Matrix<double> > > calculate_output_interlayers_Delta(void)
// method
...
Matrix<Vector<Matrix<double> > >
MultilayerPerceptron::calculate_output_interlayers_Delta(
...
// Control sentence (if debug)
  ...
  const unsigned second_order_forward_propagation_size =
      second_order_forward_propagation.size();

  if (second_order_forward_propagation_size != 3) {
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix< Vector< Matrix<double> > > "
              "calculate_output_interlayers_delta() method.\n"
  ...
  const unsigned interlayers_combination_combination_Jacobian_rows_number =
      interlayers_combination_combination_Jacobian.get_rows_number();

  if (interlayers_combination_combination_Jacobian_rows_number !=
      layers_number) {
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix< Vector< Matrix<double> > > "
              "calculate_output_interlayers_delta() method.\n"
           << "Number of rows of interlayers combination-combination Jacobian "
  ...
  if (output_layers_delta_size != layers_number) {
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix< Vector< Matrix<double> > > "
              "calculate_output_interlayers_delta() method.\n"
           << "Size of multilayer perceptron outputs layers delta must be "
  ...
  const Vector<Matrix<double> > layers_synaptic_weights =
      arrange_layers_synaptic_weights();

  const Vector<Vector<double> >& layers_activation_derivative =
      second_order_forward_propagation[1];
  const Vector<Vector<double> >& layers_activation_second_derivative =
      second_order_forward_propagation[2];

  // The Delta form consists of a square matrix of size the number of layers

  Matrix<Vector<Matrix<double> > > output_interlayers_Delta(layers_number,
                                                            layers_number);
  ...
  // The subelements are matrices with rows and columns numbers the sizes of the
  ...
  if (layers_number > 0) {
    ...
    output_interlayers_Delta[layers_number - 1][layers_number - 1] =
        layers[layers_number - 1].arrange_activations_Hessian_form(
          ...
          for (unsigned i = 0; i < outputs_number; i++) {
            for (unsigned j = 0; j < layers_size[L]; j++) {
              ...
              for (unsigned l = 0; l < layers_size[L + 1]; l++) {
                sum_1 += output_layers_delta[L + 1][i][l] *
                         layers_synaptic_weights[L + 1][l][j];
              }
              ...
              for (unsigned k = 0; k < layers_size[M]; k++) {
                ...
                for (unsigned l = 0; l < layers_size[L + 1]; l++) {
                  sum_2 += output_interlayers_Delta[L + 1][M][i][l][k] *
                           layers_synaptic_weights[L + 1][l][j];
                }

                output_interlayers_Delta[L][M][i][j][k] =
                    layers_activation_second_derivative[L][j] *
                        interlayers_combination_combination_Jacobian[L][M][j][
                            k] *
                        sum_1 +
                    layers_activation_derivative[L][j] * sum_2;

                output_interlayers_Delta[M][L][i][k][j] =
                    output_interlayers_Delta[L][M][i][j][k];
              }
            }
          }
...
// Vector<double> calculate_parametrs_outputs(const Vector<double>&, const
// Vector<double>&) const method

/// Returns the outputs from the multilayer perceptron for given sets of inputs
/// and parameters.
...
Vector<double> MultilayerPerceptron::calculate_outputs(
    const Vector<double>& inputs, const Vector<double>& parameters) const {
// Control sentence (if debug)
  ...
  if (layers_number == 0) {
    ...
    const Vector<unsigned> layers_parameters_numbers =
        count_layers_parameters_numbers();

    const Vector<unsigned> layers_cumulative_parameters_number =
        arrange_layers_cumulative_parameters_number();
    ...
    for (unsigned i = 1; i < layers_number; i++) {
      layer_parameters =
          parameters.take_out(layers_cumulative_parameters_number[i - 1],
                              layers_parameters_numbers[i]);
...
// Matrix<double> calculate_parameters_Jacobian(const Vector<double>&, const
// Vector<double>&) const method

/// Calculates the parameters matrix of the multilayer perceptron for an inputs
/// vector.
/// The elements of that matrix are the partial derivatives of the outputs with
/// respect to the multilayer perceptron parameters.

Matrix<double> MultilayerPerceptron::calculate_Jacobian(
    const Vector<double>& inputs, const Vector<double>&) const {
#ifndef NDEBUG
  ...
  if (size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "void calculate_Jacobian(Vector<double>&, const Vector<double>&) "
              "const method.\n"
           << "Size must be equal to number of inputs.\n";
  ...
  const Vector<Vector<double> > layers_inputs =
      arrange_layers_input(inputs, layers_activation);

  const Vector<Matrix<double> > layers_combination_parameters_Jacobian =
      calculate_layers_combination_parameters_Jacobian(layers_inputs);

  const Vector<Matrix<double> > output_layers_delta =
      calculate_output_layers_delta(layers_activation_derivative);

  Matrix<double> parameters_Jacobian =
      output_layers_delta[0].dot(layers_combination_parameters_Jacobian[0]);

  for (unsigned i = 1; i < layers_number; i++) {
    parameters_Jacobian = parameters_Jacobian.assemble_columns(
        output_layers_delta[i].dot(layers_combination_parameters_Jacobian[i]));
  }
...
/// Returns the second partial derivatives of the outputs from the multilayer
/// perceptron with respect to the multilayer perceptron parameters.
/// @todo
...
Vector<Matrix<double> > MultilayerPerceptron::calculate_Hessian_form(
  ...
  const Vector<Vector<Vector<double> > > second_order_forward_propagation =
      calculate_second_order_forward_propagation(inputs);

  const Vector<Vector<double> >& layers_activation =
      second_order_forward_propagation[0];
  const Vector<Vector<double> >& layers_activation_derivative =
      second_order_forward_propagation[1];
  //   const Vector< Vector<double> >& layers_activation_second_derivative =
  // second_order_forward_propagation[2];
  ...
  const Vector<Vector<Vector<double> > >
      perceptrons_combination_parameters_gradient =
          calculate_perceptrons_combination_parameters_gradient(layers_inputs);

  const Matrix<Matrix<double> > interlayers_combination_combination_Jacobian =
      calculate_interlayers_combination_combination_Jacobian(inputs);
  ...
  const Matrix<Vector<Matrix<double> > > output_interlayers_Delta =
      calculate_output_interlayers_Delta(
          ...
          interlayers_combination_combination_Jacobian, output_layers_delta);
      ...
      for (unsigned j = 0; j < parameters_number; j++) {
        ...
        for (unsigned k = j; k < parameters_number; k++) {
          ...
          parameters_Hessian_form[i][j][k] =
              output_interlayers_Delta[layer_j][layer_k][i][neuron_j][
                  neuron_k] *
              perceptrons_combination_parameters_gradient[layer_j][neuron_j][
                  parameter_j] *
              perceptrons_combination_parameters_gradient[layer_k][neuron_k][
                  parameter_k];

          if (layer_j != 0 && parameter_j != 0)  // Neither the first layer nor
                                                 // parameter is a bias
              {
            // parameters_Hessian_form[i][j][k] +=
          ...
          if (layer_k != 0 && parameter_k != 0) {
            parameters_Hessian_form[i][j][k] +=
                ...
                interlayers_combination_combination_Jacobian[layer_k - 1][
                    layer_j][parameter_k - 1][neuron_j] *
                perceptrons_combination_parameters_gradient[layer_j][neuron_j][
                    parameter_j];
          }
...
// Vector< Vector<double> > arrange_layers_input(const Vector<double>&, const
// Vector< Vector<double> >&) const method

/// Returns the layers inputs from the multilayer perceptron inputs and the
/// layers outputs.
/// The format is a vector of subvectors.
/// The size of the vector is the number of layers.
/// The size of each subvector is the number of inputs to the corresponding
/// layer.
/// @param inputs Input values to the multilayer perceptron.
/// @param layers_activation Output values from each layer.

Vector<Vector<double> > MultilayerPerceptron::arrange_layers_input(
    const Vector<double>& inputs,
    const Vector<Vector<double> >& layers_activation) const {
  // Neural network stuff
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer
        << "OpenNN Exception: MultilayerPerceptron class.\n"
        << "Vector< Vector<double> > arrange_layers_input(const "
           "Vector<double>&, const Vector< Vector<double> >&) const method.\n"
        << "Size must be equal to number of inputs.\n";
...
/// Returns a vector of vectors, where each element contains the input values of
/// a layer in response
/// to an inputs to the multilayer perceptron
/// @param inputs Input values to the multilayer perceptron

Vector<Vector<double> > MultilayerPerceptron::calculate_layers_input(
    const Vector<double>& inputs) const {
  const unsigned layers_number = get_layers_number();
...
// Vector< Vector<double> > calculate_layers_combination(const Vector<double>&)
// const method

/// Returns a vector of vectors, where each element contains the combination
/// values of a layer in response
/// to an inputs to the multilayer perceptron
/// @param inputs Input values to the multilayer perceptron

Vector<Vector<double> > MultilayerPerceptron::calculate_layers_combination(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector<double> > calculate_layers_combination(const "
              "Vector<double>&) const method.\n"
           << "Size must be equal to number of inputs.\n";
...
// Vector< Matrix<double> > calculate_layers_combination_Jacobian(const
// Vector<double>&) const method
...
MultilayerPerceptron::calculate_layers_combination_Jacobian(
    const Vector<double>& inputs) const {
  const unsigned layers_number = get_layers_number();
  ...
  Matrix<double> output_layer_Jacobian = layers[layers_number - 1]
      .calculate_Jacobian(layers_output[layers_number - 1]);
  ...
  for (int i = layers_number - 1; i > -1; i--) {
    layers_Jacobian[layers_number] =
        layers_Jacobian[layers_number].dot(layers_Jacobian[i]);
  }
...
// Vector< Matrix<double> >
// calculate_layers_combination_parameters_Jacobian(const Vector< Vector<double>
// >&) const method
...
Vector<Matrix<double> >
MultilayerPerceptron::calculate_layers_combination_parameters_Jacobian(
    const Vector<Vector<double> >& layers_inputs) const {
  const unsigned layers_number = get_layers_number();
  ...
  for (unsigned i = 0; i < layers_number; i++) {
    layers_combination_parameters_Jacobian[i] =
...
// calculate_perceptrons_combination_parameters_gradient(const Vector<
// Vector<double> >&) const method

/// Returns the combination parameters gradient of all neurons in the network
/// architecture.
...
/// That quantities will be useful for calculating some multilayer perceptron
/// derivatives.
/// @param layers_inputs Vector of subvectors with the inputs to each layer.

Vector<Vector<Vector<double> > >
MultilayerPerceptron::calculate_perceptrons_combination_parameters_gradient(
    const Vector<Vector<double> >& layers_inputs) const {
  const unsigned layers_number = get_layers_number();
...
// Control sentence (if debug)
  ...
  if (layers_input_size != layers_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector< Vector<double> > > "
              "calculate_perceptrons_combination_parameters_gradient(const "
              "Vector< Vector<double> >&) const method.\n"
           << "Size must be equal to number of layers.\n";
  ...
  Vector<Vector<Vector<double> > > perceptrons_combination_gradient(
      layers_number);
  ...
  for (unsigned i = 0; i < layers_number; i++) {
    ...
    for (unsigned j = 0; j < layers_size[i]; j++) {
      ...
      perceptrons_combination_gradient[i][j] =
          perceptron.calculate_combination_gradient(layers_inputs[i],
                                                    dummy_layer_parameters);
    }
  }
...
// Vector< Vector<double> > calculate_layers_activation(const Vector<double>&)
// const method

/// Returns a vector of vectors, where each element contains the activation
/// values of a layer in response
/// to an inputs to the multilayer perceptron
/// @param inputs Input values to the multilayer perceptron

Vector<Vector<double> > MultilayerPerceptron::calculate_layers_activation(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector<double> > calculate_layers_activation(const "
              "Vector<double>&) const method.\n"
           << "Size must be equal to number of inputs.\n";
...
// Vector< Vector<double> > calculate_layers_activation_derivative(const
// Vector<double>&) const method

/// Returns a vector of vectors, where each element contains the activation
/// derivatives of a layer in response
/// to an inputs to the multilayer perceptron
/// @param inputs Input values to the multilayer perceptron

Vector<Vector<double> >
MultilayerPerceptron::calculate_layers_activation_derivative(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector<double> > "
              "calculate_layers_activation_derivative(const Vector<double>&) "
              "const method.\n"
           << "Size must be equal to number of inputs.\n";
  ...
  if (layers_number != 0) {
    ...
    layers_activation[0] =
        layers[0].calculate_activations(layers_combination[0]);
    layers_activation_derivatives[0] =
    ...
    for (unsigned i = 1; i < layers_number; i++) {
      ...
      layers_activation[i] =
          layers[i].calculate_activations(layers_combination[i]);
      layers_activation_derivatives[i] =
...
// Vector< Vector<double> > calculate_layers_activation_second_derivative(const
// Vector<double>&) const method

/// Returns a vector of vectors with the forward propagation values, their
...
Vector<Vector<double> >
MultilayerPerceptron::calculate_layers_activation_second_derivative(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector<double> > "
              "calculate_layers_activation_second_derivative(const "
              "Vector<double>&) const method.\n"
           << "Size must be equal to number of inputs.\n";
  ...
  layers_combination[0] = layers[0].calculate_combinations(inputs);
  layers_activation[0] = layers[0].calculate_activations(layers_combination[0]);
  layers_activation_second_derivatives[0] =
  ...
  for (unsigned i = 1; i < layers_number; i++) {
    ...
    layers_activation_second_derivatives[i] = layers[i]
        .calculate_activations_second_derivatives(layers_combination[i]);
  }

  return (layers_activation_second_derivatives);
}

// Vector< Matrix<double> > calculate_layers_Jacobian(const Vector<double>&)
// const method

/// Returns the partial derivatives of the outputs from each layer with respect
/// to the inputs to the corresponding layer,
/// for a vector of inputs to the neural netwok.
/// The format of this quantity is a vector of matrices.
/// @param inputs Vector of inputs to the multilayer perceptron

Vector<Matrix<double> > MultilayerPerceptron::calculate_layers_Jacobian(
    const Vector<double>& inputs) const {
  const unsigned layers_number = get_layers_number();
...
/// Returns the second partial derivatives of the outputs from each layer with
/// respect to the inputs to the corresponding layer,
/// for a vector of inputs to the neural netwok.
/// The format of this quantity is a vector of vectors of matrices.
/// @param inputs Vector of inputs to the multilayer perceptron

Vector<Vector<Matrix<double> > >
MultilayerPerceptron::calculate_layers_Hessian_form(
    const Vector<double>& inputs) const {
  const unsigned layers_number = get_layers_number();
...
// calculate_interlayers_combination_combination_Jacobian(const Vector<double>&)
// const method

/// Returns the partial derivatives of the combination of one neuron with
/// respect to the combination of another neuron for all layers in the
/// multilayer perceptron
/// This quantity is a Jacobian form, and it is represented as a matrix of
/// matrices.
/// @param inputs Vector of inputs to the multilayer perceptron

Matrix<Matrix<double> >
MultilayerPerceptron::calculate_interlayers_combination_combination_Jacobian(
    const Vector<double>& inputs) const {
#ifndef NDEBUG
  ...
  if (size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Matrix< Matrix<double> > "
              "calculate_interlayers_combination_combination_Jacobian(const "
              "Vector<double>&) const method.\n"
  ...
  const Vector<Vector<double> > layers_combination =
      calculate_layers_combination(inputs);

  return (calculate_interlayers_combination_combination_Jacobian(
      layers_combination));
}

// Matrix <Matrix<double> >
// calculate_interlayers_combination_combination_Jacobian(const Vector<
// Vector<double> >&) const method
...
/// This quantity is a Jacobian form, and it is represented as a matrix of
/// matrices.
/// @param layers_combination Vector vectors representing the combinations of
/// all layers.

Matrix<Matrix<double> >
MultilayerPerceptron::calculate_interlayers_combination_combination_Jacobian(
    const Vector<Vector<double> >& layers_combination) const {
  const unsigned layers_number = get_layers_number();
  ...
  for (unsigned image_index = 0; image_index < layers_number; image_index++) {
    for (unsigned domain_index = 0; domain_index < layers_number;
         domain_index++) {
      interlayers_combination_combination_Jacobian[image_index][domain_index] =
          calculate_interlayer_combination_combination_Jacobian(
              domain_index, image_index, layers_combination[domain_index]);
    }
  }
...
// Vector< Vector< Vector<double> > >
// calculate_first_order_forward_propagation(const Vector<double>&) const method

/// Returns the first order forward propagation quantities from the multilayer
/// perceptron for a given inputs.
/// That quantites include the activation and the activation derivative of all
/// layers.
/// The format is a vector of vectors of vectors.
/// The first index refers to the quantity (0 for the activation and 1 for the
...
Vector<Vector<Vector<double> > >
MultilayerPerceptron::calculate_first_order_forward_propagation(
    const Vector<double>& inputs) const {
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector< Vector<double> > > "
              "calculate_first_order_forward_propagation(const "
              "Vector<double>&) const method.\n"
           << "Size must be equal to number of inputs.\n";
  ...
  first_order_forward_propagation[0][0] =
      layers[0].calculate_activations(layers_combination[0]);

  first_order_forward_propagation[1][0] =
      layers[0].calculate_activations_derivatives(layers_combination[0]);

  for (unsigned i = 1; i < layers_number; i++) {
    layers_combination[i] = layers[i]
        .calculate_combinations(first_order_forward_propagation[0][i - 1]);

    first_order_forward_propagation[0][i] =
        layers[i].calculate_activations(layers_combination[i]);

    first_order_forward_propagation[1][i] =
        layers[i].calculate_activations_derivatives(layers_combination[i]);
  }
...
// Vector< Vector< Vector<double> > >
// calculate_second_order_forward_propagation(const Vector<double>&) const
// method

/// Returns the second order forward propagation quantities from the multilayer
/// perceptron for a given inputs.
/// That quantites include the activation, the activation derivative and the
/// activation second derivative of all layers.
/// The format is a vector of vectors of vectors.
/// The first index refers to the activation derivative order (0 for the
/// activation, 1 for the activation derivative and 2 for the activation second
/// derivative).
/// The second index is the index of the layer.
/// The third index is the index of the neuron.
/// @param inputs Vector of inputs to the multilayer perceptron
...
// Control sentence (if debug)
  ...
  if (inputs_size != inputs_number) {
    ...
    buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
           << "Vector< Vector< Vector<double> > > "
              "calculate_second_order_forward_propagation(const "
              "Vector<double>&) const method.\n"
           << "Size of multilayer perceptron inputs must be equal to number of "
              "inputs.\n";
  ...
  second_order_forward_propagation[0][0] =
      layers[0].calculate_activations(layers_combination[0]);

  second_order_forward_propagation[1][0] =
      layers[0].calculate_activations_derivatives(layers_combination[0]);

  second_order_forward_propagation[2][0] =
      layers[0].calculate_activations_second_derivatives(layers_combination[0]);

  for (unsigned i = 1; i < layers_number; i++) {
    layers_combination[i] = layers[i]
        .calculate_combinations(second_order_forward_propagation[0][i - 1]);

    second_order_forward_propagation[0][i] =
        layers[i].calculate_activations(layers_combination[i]);

    second_order_forward_propagation[1][i] =
        layers[i].calculate_activations_derivatives(layers_combination[i]);

    second_order_forward_propagation[2][i] = layers[i]
        .calculate_activations_second_derivatives(layers_combination[i]);
  }
...
/// Serializes the multilayer perceptron object into a XML document of the
/// TinyXML library.
...
tinyxml2::XMLDocument* MultilayerPerceptron::to_XML(void) const {
  ...
  tinyxml2::XMLElement* multilayer_perceptron_element =
      document->NewElement("MultilayerPerceptron");

  document->InsertFirstChild(multilayer_perceptron_element);

  // Architecture
  {
    tinyxml2::XMLElement* architecture_element =
        document->NewElement("Architecture");
    ...
    tinyxml2::XMLText* architecture_text =
        document->NewText(architecture_string.c_str());
    architecture_element->LinkEndChild(architecture_text);
  }

  // Layers activation function
  {
    tinyxml2::XMLElement* layers_activation_function_element =
        document->NewElement("LayersActivationFunction");
    multilayer_perceptron_element->LinkEndChild(
        layers_activation_function_element);

    std::string layers_activation_function_string =
        write_layers_activation_function().to_string();

    tinyxml2::XMLText* layers_activation_function_text =
        document->NewText(layers_activation_function_string.c_str());
    layers_activation_function_element->LinkEndChild(
        layers_activation_function_text);
  }

  // Parameters
  {
    tinyxml2::XMLElement* parameters_element =
        document->NewElement("Parameters");
    ...
    tinyxml2::XMLText* parameters_text =
        document->NewText(parameters_string.c_str());
...
void MultilayerPerceptron::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("MultilayerPerceptron");
  ...
  // Architecture
  {
    const tinyxml2::XMLElement* architecture_element =
        root_element->FirstChildElement("Architecture");
  ...
  // Layers activation function
  {
    const tinyxml2::XMLElement* layers_activation_function_element =
        root_element->FirstChildElement("LayersActivationFunction");

    if (layers_activation_function_element) {
      const char* layers_activation_function_text =
          layers_activation_function_element->GetText();
  ...
  // Parameters
  {
    const tinyxml2::XMLElement* parameters_element =
        root_element->FirstChildElement("Parameters");
...
/// Returns a string matrix with certain information about the multilayer
/// perceptron,
/// which includes the number of inputs, the number of perceptrons and the
/// activation function of all the layers.
/// The  number of rows is the number of layers, and the number of columns is
/// three.
...
/// Returns a string with the expression of the forward propagation process in a
/// multilayer perceptron.
/// @todo

std::string MultilayerPerceptron::write_expression(
  ...
  if (layers_number == 0) {
    ...
    for (unsigned i = 1; i < layers_number - 1; i++) {
      buffer << layers[i].write_expression(layers_outputs_name[i - 1],
                                           layers_outputs_name[i]);
    }

    buffer << layers[layers_number - 1].write_expression(
                  layers_outputs_name[layers_number - 2], outputs_name);
  }
