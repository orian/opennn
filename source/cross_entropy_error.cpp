...
/// which is not associated to any neural network and not measured on any data
/// set.
...
/// Neural network constructor.
/// It creates a cross entropy error performance term associated to a neural
...
/// Data set constructor.
/// It creates a cross entropy error not associated to any neural network but to
...
/// Neural network and data set constructor.
/// It creates a cross entropy error performance term object associated to a
...
/// XML constructor.
/// It creates a cross entropy error not associated to any neural network and
/// not measured on any data set.
...
CrossEntropyError::CrossEntropyError(
    const CrossEntropyError& new_cross_entropy_error)
...
CrossEntropyError& CrossEntropyError::operator=(
    const CrossEntropyError& other_cross_entropy_error) {
...
bool CrossEntropyError::operator==(
    const CrossEntropyError& other_cross_entropy_error) const {
  if (*neural_network_pointer ==
          *other_cross_entropy_error.neural_network_pointer &&
      *mathematical_model_pointer ==
          *other_cross_entropy_error.mathematical_model_pointer &&
      display == other_cross_entropy_error.display) {
    return (true);
  } else {
    return (false);
...
void CrossEntropyError::check(void) const {
  ...
  const ProbabilisticLayer* probabilistic_layer_pointer =
      neural_network_pointer->get_probabilistic_layer_pointer();
  ...
  const ProbabilisticLayer::ProbabilisticMethod& outputs_probabilizing_method =
      probabilistic_layer_pointer->get_probabilistic_method();
...
double CrossEntropyError::calculate_performance(void) const {
  ...
  instances.count_training_instances_number();
     ...
     inputs = data_set_pointer->get_training_input_instance(i);
        ...
        {
           objective -= targets[j]*log(outputs[j]) + (1.0 - targets[j])*log(1.0
  - outputs[j]);
        }
     }
...
double CrossEntropyError::calculate_minimum_performance(void) { return (0.0); }

// double calculate_generalization_performance(void) const method

/// Returns the cross entropy error of the neural network measured on the
...
// Control sentence
  ...
  for (unsigned i = 0; i < generalization_instances_number; i++) {
    ...
    for (unsigned j = 0; j < outputs_number; j++) {
      generalization_performance -= targets[j] * log(outputs[j]) +
                                    (1.0 - targets[j]) * log(1.0 - outputs[j]);
    }
  }
...
Vector<double> CrossEntropyError::calculate_gradient(void) const {
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    if (!has_conditions_layer) {
      ...
      for (unsigned j = 0; j < outputs_number; j++) {
        //            output_objective_gradient[j]
        //            = -targets[j]/outputs[j] + (1.0 - targets[j])*(1.0 -
        // outputs[j]);
...
/// Returns a string with the name of the cross entropy error performance type,
/// "CROSS_ENTROPY_ERROR".
...
/// Serializes the cross entropy error object into a XML document of the TinyXML
/// library.
...
tinyxml2::XMLDocument* CrossEntropyError::to_XML(void) const {
  ...
  tinyxml2::XMLElement* cross_entropy_error_element =
      document->NewElement("CrossEntropyError");
...
void CrossEntropyError::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("CrossEntropyError");
