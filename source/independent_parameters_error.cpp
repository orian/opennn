...
/// Default constructor.
/// It creates a independent parameters error performance term with all pointers
/// initialized to NULL.
...
/// Neural network constructor.
/// It creates a independent parameters error performance term associated to a
/// neural network.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.

IndependentParametersError::IndependentParametersError(
...
/// @param independent_parameters_error_document TinyXML document of a
/// independent parameters values object.
...
IndependentParametersError& IndependentParametersError::operator=(
    const IndependentParametersError& other_independent_parameters_error) {
  if (this != &other_independent_parameters_error) {
    *neural_network_pointer =
        *other_independent_parameters_error.neural_network_pointer;
    *data_set_pointer = *other_independent_parameters_error.data_set_pointer;
    *mathematical_model_pointer =
        *other_independent_parameters_error.mathematical_model_pointer;
    *numerical_differentiation_pointer =
        *other_independent_parameters_error.numerical_differentiation_pointer;
    display = other_independent_parameters_error.display;

    target_independent_parameters =
        other_independent_parameters_error.target_independent_parameters;
    independent_parameters_errors_weights = other_independent_parameters_error
...
bool IndependentParametersError::operator==(
    const IndependentParametersError& other_independent_parameters_error)
    const {
...
const Vector<double>&
IndependentParametersError::get_target_independent_parameters(void) const {
  return (target_independent_parameters);
}

// const double& get_target_independent_parameter(const unsigned&) const method

/// Returns the desired value of a single independent parameter.
/// @param i Index of independent parameter.

const double& IndependentParametersError::get_target_independent_parameter(
...
/// Returns the weight for each error between the actual independent parameters
/// and their target values.

const Vector<double>&
IndependentParametersError::get_independent_parameters_errors_weights(
    void) const {
...
/// Returns the weight for a singel error between an independent parameters and
/// its target value.
/// @param i Index of independent parameter parameter.

const double&
IndependentParametersError::get_independent_parameter_error_weight(
    const unsigned& i) const {
...
/// @param new_target_independent_parameters Vector of desired values for the
/// independent parameters.

void IndependentParametersError::set_target_independent_parameters(
    const Vector<double>& new_target_independent_parameters) {
  target_independent_parameters = new_target_independent_parameters;
}

// void set_target_independent_parameter(const unsigned&, const double&) method
...
void IndependentParametersError::set_target_independent_parameter(
    const unsigned& i, const double& new_target_independent_parameter) {
...
/// Sets new weights for each error between the actual independent parameters
/// and their target values.
/// @param new_independent_parameters_errors_weights Vector of weights, with
/// size the number of independent parameters.

void IndependentParametersError::set_independent_parameters_errors_weights(
    const Vector<double>& new_independent_parameters_errors_weights) {
  independent_parameters_errors_weights =
      new_independent_parameters_errors_weights;
}

// void set_independent_parameter_error_weight(const unsigned&, const double&)
// method

/// Sets a new weight for the error between a single independent parameter and
/// its target value.
/// @param i Index of independent parameter.
/// @param new_independent_parameter_error_weight Weight value.

void IndependentParametersError::set_independent_parameter_error_weight(
    const unsigned& i, const double& new_independent_parameter_error_weight) {
  independent_parameters_errors_weights[i] =
      new_independent_parameter_error_weight;
}
  ...
  if (neural_network_pointer) {
    if (neural_network_pointer->has_independent_parameters()) {
      const IndependentParameters* independent_parameters_pointer =
          neural_network_pointer->get_independent_parameters_pointer();
      ...
      independent_parameters_errors_weights.set(independent_parameters_number,
                                                1.0);
    }
  } else {
...
/// Checks that there are a neural network and a data set associated to the sum
/// squared error,
/// and that the number of independent parameters in the neural network is equal
/// to the number of size of the target independent parameters in the
/// performance term.
...
void IndependentParametersError::check(void) const {
  ...
  const unsigned target_independent_parameters_size =
      target_independent_parameters.size();
  ...
  if (target_independent_parameters_size != independent_parameters_number) {
    buffer << "OpenNN Exception: IndependentParametersError class." << std::endl
           ...
           << "Size of target independent parameters must be equal to number "
              "of independent parameters." << std::endl;

    throw std::logic_error(buffer.str());
  }

  const unsigned independent_parameters_errors_weights_size =
      independent_parameters_errors_weights.size();

  if (independent_parameters_errors_weights_size !=
      independent_parameters_number) {
    buffer << "OpenNN Exception: IndependentParametersError class." << std::endl
           << "double calculate_performance(void) const method." << std::endl
           << "Size of independent parameters errors weights must be equal to "
...
/// Returns the dot product between the independent parameters vector and its
/// targets vector.
...
// Neural network stuff
  ...
  const Vector<double> independent_parameters_error =
      independent_parameters - target_independent_parameters;

  return ((independent_parameters_errors_weights *
           independent_parameters_error * independent_parameters_error)
              .calculate_sum());
}
...
double IndependentParametersError::calculate_performance(
...
// Neural network stuff
  ...
  if (size != parameters_number) {
    ...
    buffer
        ...
        << "double calculate_performance(const Vector<double>&) const method."
        << std::endl << "Size of parameters (" << size
  ...
  independent_parameters_error_copy.set_neural_network_pointer(
      &neural_network_copy);
...
// Neural network stuff
  ...
  const unsigned neural_parameters_number =
      parameters_number - independent_parameters_number;
  ...
  const Vector<double> independent_parameters_gradient =
      (independent_parameters - target_independent_parameters) * 2.0;

  return (
      multilayer_perceptron_gradient.assemble(independent_parameters_gradient));
}
...
// Neural network stuff
  ...
  for (unsigned i = neural_parameters_number; i < parameters_number; i++) {
    for (unsigned j = neural_parameters_number; j < parameters_number; j++) {
      Hessian[i][j] = 2.0;
    }
  }
...
/// Returns a string with the name of the independent parameters error
/// performance type, "INDEPENDENT_PARAMETERS_ERROR".

std::string IndependentParametersError::write_performance_term_type(
    void) const {
...
/// Returns a representation of the independent parameters error object, in XML
/// format.
...
tinyxml2::XMLDocument* IndependentParametersError::to_XML(void) const {
  ...
  tinyxml2::XMLElement* independent_parameters_error_element =
      document->NewElement("IndependentParametersError");
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("TargetIndependentParamters");
    independent_parameters_error_element->LinkEndChild(element);
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("IndependentParametersErrorsWeights");
    independent_parameters_error_element->LinkEndChild(element);
...
void IndependentParametersError::from_XML(
    const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("IndependentParametersError");
