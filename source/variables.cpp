...
/// Variables number constructor.
/// It creates a variables object with a given number of input and target
/// variables.
/// It also initializes the rest of class members to their default values.
/// @param new_inputs_number Number of input variables.
/// @param new_targets_number Number of target variables.

Variables::Variables(const unsigned& new_inputs_number,
...
/// Assignment operator.
/// It assigns to the current object the members of an existing variables
/// object.
...
/// Returns the number of variables which will neither be used as input nor as
/// target.
...
/// Returns a vector containing the numbers of input, target and unused
/// variables,
/// respectively.
...
/// Returns all the available information about the variables as a matrix of
/// strings.
/// The number of rows is the number of variables.
/// The number of columns is four.
/// Each row contains the information of a single variable (name, units,
/// description and use).
...
/// Returns a single matrix with the names, units and descriptions of all the
/// input variables.
...
const bool& Variables::get_display(void) const { return (display); }
...
/// All the variables are set as inputs, but the last one, wich is set as
/// targets.
/// @param new_variables_number Number of variables.
...
/// The total number of variables is the number of inputs plus the number of
/// targets.
...
void Variables::set(const unsigned& inputs_number,
                    const unsigned& targets_number) {
...
/// @param variables_document Pointer to a TinyXML document containing the
/// member data.
...
void Variables::set_default(void) { display = true; }
...
void Variables::set_items(const Vector<Item>& new_items) { items = new_items; }

// void set_uses(const Vector<Use>&) method

/// Sets new uses for the all the variables from a single vector.
/// It does not modify the other information on the variables (name, units or
/// description).
...
/// If the vector of names is zero the rest of elements are initialized as empty
/// strings.
/// @param i Index of variable.
...
/// If the vector of units is zero the rest of elements are initialized as empty
/// strings.
/// @param i Index of variable.
...
/// Sets new descriptions for the variables in the data set from a vector of
/// strings.
/// The size of that vector must be equal to the total number of variables.
...
/// If the vector of descriptions is zero the rest of elements are initialized
/// as empty strings.
/// @param i Index of variable.
/// @param new_description Description of variable.

void Variables::set_description(const unsigned& i,
...
void Variables::set_display(const bool& new_display) { display = new_display; }
...
tinyxml2::XMLDocument* Variables::to_XML(void) const {
  ...
  for (unsigned i = 0; i < variables_number; i++) {
    ...
    tinyxml2::XMLElement* description_element =
        document->NewElement("Description");
    element->LinkEndChild(description_element);

    tinyxml2::XMLText* descriptionText =
        document->NewText(items[i].description.c_str());
...
void Variables::from_XML(const tinyxml2::XMLDocument& document) {
  ...
  const tinyxml2::XMLElement* variables_element =
      document.FirstChildElement("Variables");
  ...
  const tinyxml2::XMLElement* variables_number_element =
      variables_element->FirstChildElement("VariablesNumber");
  ...
  for (unsigned i = 0; i < variables_number; i++) {
    ...
    const tinyxml2::XMLElement* name_element =
        item_element->FirstChildElement("Name");
    ...
    const tinyxml2::XMLElement* units_element =
        item_element->FirstChildElement("Units");
    ...
    const tinyxml2::XMLElement* description_element =
        item_element->FirstChildElement("Description");
