...
LinearAlgebraicEquations& LinearAlgebraicEquations::operator=(
    const LinearAlgebraicEquations&) {
  return (*this);
}
...
bool LinearAlgebraicEquations::operator==(
    const LinearAlgebraicEquations&) const {
  return (true);
}

// METHODS

// static Matrix<double> perform_Gauss_Jordan_elimination(Matrix<double>&,
// Matrix<double>&) method

/// Performs a linear equation solution by Gauss-Jordan eliminiation with full
/// pivoting.
...
void LinearAlgebraicEquations::perform_Gauss_Jordan_elimination(
  ...
  const unsigned m =
      b.get_columns_number();  // Number of right hand side vectors
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    if (a[icol][icol] == 0.0) {
      ...
      buffer << "OpenNN Exception: LinearAlgebraicEquations class. "
             << "perform_Gauss_Jordan_elimination(Matrix<double>&, "
                "Matrix<double>&) method.\n"
...
// static void perform_Gauss_Jordan_elimination(Matrix<double>& a,
// Vector<double>& b) method
...
void LinearAlgebraicEquations::perform_Gauss_Jordan_elimination(
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    if (a[icol][icol] == 0.0) {
      ...
      buffer << "OpenNN Exception: LinearAlgebraicEquations class. "
             << "perform_Gauss_Jordan_elimination(const Matrix<double>&, const "
                "Matrix<double>&) method.\n"
             << "Matrix A is singular\n";
...
/// Returns the solution vector x to the linear equation Ax = b, using the
/// Gauss-Jordan elimination method.
/// @param a A matrix.
/// @param b b vector.

Vector<double> LinearAlgebraicEquations::calculate_Gauss_Jordan_solution(
