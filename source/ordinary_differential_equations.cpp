...
/// XML constructor.
/// It creates a ordinary differential equations mathematical modeel and loads
/// its members from a TinyXML document.
/// @param ordinary_differential_equations_document XML document containing the
/// member data.

OrdinaryDifferentialEquations::OrdinaryDifferentialEquations(
    const tinyxml2::XMLDocument& ordinary_differential_equations_document)
...
/// File constructor.
/// It creates an ordinary differential equations mathematical model and loads
/// its members from a XML file.
...
/// Copy constructor.
/// It creates an ordinary differential equations mathematical model object and
/// copies its members from another object.
/// @param other_ordinary_differential_equations Ordinary differential equations
/// object to be copied.

OrdinaryDifferentialEquations::OrdinaryDifferentialEquations(
    const OrdinaryDifferentialEquations& other_ordinary_differential_equations)
    : MathematicalModel() {
...
// OrdinaryDifferentialEquations& operator = (const
// OrdinaryDifferentialEquations&) method

/// Assignment operator.
/// It assigns to this object the members of an existing ordinary differential
/// equations object.
/// @param other_ordinary_differential_equations Ordinary differential equations
/// object to be assigned.

OrdinaryDifferentialEquations& OrdinaryDifferentialEquations::operator=(
    const OrdinaryDifferentialEquations&
        other_ordinary_differential_equations) {
  if (this != &other_ordinary_differential_equations) {
    initial_independent_variable =
        other_ordinary_differential_equations.initial_independent_variable;
    final_independent_variable =
        other_ordinary_differential_equations.final_independent_variable;
    initial_dependent_variables =
        other_ordinary_differential_equations.initial_dependent_variables;
  }
...
bool OrdinaryDifferentialEquations::operator==(
    const OrdinaryDifferentialEquations& other_ordinary_differential_equations)
    const {
  if (initial_independent_variable ==
          other_ordinary_differential_equations.initial_independent_variable &&
      final_independent_variable ==
          other_ordinary_differential_equations.final_independent_variable &&
      initial_dependent_variables ==
...
const double& OrdinaryDifferentialEquations::get_initial_independent_variable(
    void) const {
...
const double& OrdinaryDifferentialEquations::get_final_independent_variable(
    void) const {
...
const Vector<double>&
OrdinaryDifferentialEquations::get_initial_dependent_variables(void) const {
  return (initial_dependent_variables);
}

// const double& get_initial_dependent_variable(const unsigned&) const method

/// Returns the initial value of a single independent variable.
/// @param i Index of dependent variable.

const double& OrdinaryDifferentialEquations::get_initial_dependent_variable(
...
/// Returns the numerical method to be used for integration of the ordinary
/// differential equation.

const OrdinaryDifferentialEquations::SolutionMethod&
OrdinaryDifferentialEquations::get_solution_method(void) const {
  return (solution_method);
}

// std::string write_solution_method(void) const method

/// Returns a string with the name of the numerical method to be used for
...
// const unsigned& get_points_number(void) const method

/// Returns the number of integration points in the Runge-Kutta method.

const unsigned& OrdinaryDifferentialEquations::get_points_number(void) const {
...
// const unsigned& get_initial_size(void) const method

/// Returns the initial size to be reserved for the solutions in the
/// Runge-Kutta-Fehlberg method.

const unsigned& OrdinaryDifferentialEquations::get_initial_size(void) const {
  return (initial_size);
}

// const unsigned& get_warning_size(void) const method

/// Returns a warning size for the solutions in the Runge-Kutta-Fehlberg method.

const unsigned& OrdinaryDifferentialEquations::get_warning_size(void) const {
  return (warning_size);
}

// const unsigned& get_error_size(void) const method

/// Returns an error size for the solutions in the Runge-Kutta-Fehlberg method.

const unsigned& OrdinaryDifferentialEquations::get_error_size(void) const {
  return (error_size);
}
...
void OrdinaryDifferentialEquations::set(
    const OrdinaryDifferentialEquations&
        other_ordinary_differential_equations) {
  independent_variables_number =
      other_ordinary_differential_equations.independent_variables_number;
  dependent_variables_number =
      other_ordinary_differential_equations.dependent_variables_number;
...
/// @param new_initial_independent_variable Initial value for the independent
/// variable.

void OrdinaryDifferentialEquations::set_initial_independent_variable(
    const double& new_initial_independent_variable) {
...
/// @param new_final_independent_variable Final value for the independent
/// variable.

void OrdinaryDifferentialEquations::set_final_independent_variable(
    const double& new_final_independent_variable) {
...
/// @param new_initial_dependent_variables Initial values for the dependent
/// variables.

void OrdinaryDifferentialEquations::set_initial_dependent_variables(
    const Vector<double>& new_initial_dependent_variables) {
  initial_dependent_variables = new_initial_dependent_variables;
}

// void set_initial_dependent_variable(const unsigned&, const double&) const
// method

/// Sets the initial value for a single dependent variable.
/// @param i Index of dependent variable.
/// @param new_initial_dependent_variable Initial value for the corresponding
/// dependent variable.

void OrdinaryDifferentialEquations::set_initial_dependent_variable(
    const unsigned& i, const double& new_initial_dependent_variable) {
  initial_dependent_variables[i] = new_initial_dependent_variable;
}

// void set_solution_method(const SolutionMethod&) method

/// Sets the numerical method for calculating the solution to the ordinary
/// differential equations.
/// Available methos include the Runge-Kutta and Runge-Kutta-Fehlber methods.
/// @param new_solution_method Solution method.

void OrdinaryDifferentialEquations::set_solution_method(
    const SolutionMethod& new_solution_method) {
...
/// differential equations.
/// Available methos include the Runge-Kutta and Runge-Kutta-Fehlber methods.
/// @param new_solution_method String with the name of the solution method
/// ("RungeKutta" or "RungeKuttaFehlberg").

void OrdinaryDifferentialEquations::set_solution_method(
    const std::string& new_solution_method) {
...
// void set_points_number(const unsigned&) method

/// Sets the number of points in the Runge-Kutta method.
/// @param new_points_number Number of points.

void OrdinaryDifferentialEquations::set_points_number(
    const unsigned& new_points_number) {
...
void OrdinaryDifferentialEquations::set_default(void) {
  ...
  initial_size = (unsigned)1.0e3;
  warning_size = (unsigned)1.0e6;
  error_size = (unsigned)1.0e9;

  display = true;
}

// Matrix<double> calculate_Runge_Kutta_solution(const NeuralNetwork&
// neural_network, const double& xi, const double& xf, const Vector<double>& yi)
// const method

/// This method calculate the numerical solution of the ordinary differential
/// equations model using the Runge-Kutta method.
/// @param neural_network Neural network which represents the external inputs to
/// the mathematical model.

Matrix<double> OrdinaryDifferentialEquations::calculate_Runge_Kutta_solution(
    const NeuralNetwork& neural_network) const {
  const unsigned variables_number = count_variables_number();

  const double h = (final_independent_variable - initial_independent_variable) /
                   (points_number - 1.0);
  ...
  for (unsigned j = 0; j < dependent_variables_number; j++) {
    solution[0][1 + j] = initial_dependent_variables[j];
  }

  // Main loop

  for (unsigned i = 0; i < points_number - 1; i++) {
...
// Matrix<double> calculate_Runge_Kutta_final_solution(const NeuralNetwork&
// neural_network) const method

/// This method calculate the solution at the final independent variable of the
/// ordinary differential equations model using the Runge-Kutta method.
/// @param neural_network Neural network which represents the external inputs to
/// the mathematical model.

Vector<double>
OrdinaryDifferentialEquations::calculate_Runge_Kutta_final_solution(
    const NeuralNetwork& neural_network) const {
  const unsigned variables_number = count_variables_number();
...
// Matrix<double> calculate_Runge_Kutta_Fehlberg_solution(const NeuralNetwork&)
// const method
...
Matrix<double>
OrdinaryDifferentialEquations::calculate_Runge_Kutta_Fehlberg_solution(
    const NeuralNetwork& neural_network) const {
  ...
  double hmin = 0.0;
  double h =
      (final_independent_variable - initial_independent_variable) * 1.0e-3;
  ...
  while (solution[point_index][0] < final_independent_variable) {
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] =
          solution[point_index][1 + j] + b31 * c[0][j] + b32 * c[1][j];
    }
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] = solution[point_index][1 + j] + b41 * c[0][j] +
                         b42 * c[1][j] + b43 * c[2][j];
    }
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] = solution[point_index][1 + j] + b51 * c[0][j] +
                         b52 * c[1][j] + b53 * c[2][j] + b54 * c[3][j];
    }
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] =
          solution[point_index][1 + j] + b61 * c[0][j] + b62 * c[1][j] +
          b63 * c[2][j] + b64 * c[3][j] + b65 * c[4][j];
    }
    ...
    if (error <= tolerance) {
      ...
      for (unsigned j = 0; j < dependent_variables_number; j++) {
        solution[point_index + 1][1 + j] =
            solution[point_index][1 + j] + c51 * c[0][j] + c52 * c[1][j] +
            c53 * c[2][j] + c54 * c[3][j] + c55 * c[4][j] + c56 * c[5][j];
      }
      ...
      if (point_index >= size) {
        ...
        if (display && size > warning_size) {
          std::cout << "OpenNN Warning: OrdinaryDifferentialEquations class."
                    << std::endl
                    << "calculate_Runge_Kutta_Fehlberg_solution() method."
                    << std::endl << "Solution size is " << size << std::endl;
          ...
          buffer << "OpenNN Exception: OrdinaryDifferentialEquations class."
                 ...
                 << "calculate_Runge_Kutta_Fehlberg_solution() method."
                 << std::endl
                 << "Solution size is bigger than greatest allowable."
                 << std::endl;
...
// Vector<double> calculate_Runge_Kutta_Fehlberg_final_solution(const
// NeuralNetwork&) const method

/// This method calculate the numerical solution of the ordinary differential
/// equations model using the Runge-Kutta-Fehlberg method.
/// @param neural_network Neural network which represents the external inputs to
/// the mathematical model.

Vector<double>
OrdinaryDifferentialEquations::calculate_Runge_Kutta_Fehlberg_final_solution(
    const NeuralNetwork& neural_network) const {
  const double epsilon = 1.0e-12;  // std::numeric_limits<double>::epsilon();
  ...
  while (final_solution[0] < final_independent_variable) {
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] =
          final_solution[1 + j] + b41 * c[0][j] + b42 * c[1][j] + b43 * c[2][j];
    }
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] = final_solution[1 + j] + b51 * c[0][j] + b52 * c[1][j] +
                         b53 * c[2][j] + b54 * c[3][j];
    }
    ...
    for (unsigned j = 0; j < dependent_variables_number; j++) {
      variables[1 + j] = final_solution[1 + j] + b61 * c[0][j] + b62 * c[1][j] +
                         b63 * c[2][j] + b64 * c[3][j] + b65 * c[4][j];
    }
...
Matrix<double> OrdinaryDifferentialEquations::calculate_solutions(
    const NeuralNetwork& neural_network) const {
  switch (solution_method) {
    ...
    default: {
      ...
      buffer << "OpenNN Error: OrdinaryDifferentialEquations class\n"
             << "Vector<double> calculate_solutions(const NeuralNetwork&) "
                "const method.\n"
             << "Unknown solution method.\n";
...
Vector<double> OrdinaryDifferentialEquations::calculate_final_solutions(
    const NeuralNetwork& neural_network) const {
  switch (solution_method) {
    case RungeKutta: {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: ScalingLayer class\n"
             << "Vector<double> calculate_final_solutions(const "
                "NeuralNetwork&) const method.\n"
             << "Unknown solution method.\n";
...
/// Returns a string representation of the current ordinary differential
/// equations object.
...
/// Serializes the ordinary differential equations object into a XML document of
/// the TinyXML library.
...
tinyxml2::XMLDocument* OrdinaryDifferentialEquations::to_XML(void) const {
  ...
  tinyxml2::XMLElement* ordinary_differential_equations_element =
      document->NewElement("OrdinaryDifferentialEquations");
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("InitialIndependentVariable");
    ordinary_differential_equations_element->LinkEndChild(element);
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("FinalIndependentVariable");
    ordinary_differential_equations_element->LinkEndChild(element);
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("InitialDependentVariables");
    ordinary_differential_equations_element->LinkEndChild(element);
  ...
  // Solution method
  {
    ...
    tinyxml2::XMLText* text =
        document->NewText(write_solution_method().c_str());
    element->LinkEndChild(text);
  }
...
/// Deserializes a TinyXML document into this ordinary differential equations
/// object.
/// @param document TinyXML document containing the member data.

void OrdinaryDifferentialEquations::from_XML(
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("InitialIndependentVariable");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("FinalIndependentVariable");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("InitialDependentVariables");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("SolutionMethod");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("PointsNumber");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("Tolerance");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("InitialSize");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("WarningSize");
  ...
  {
    const tinyxml2::XMLElement* element =
        document.FirstChildElement("ErrorSize");
...
void OrdinaryDifferentialEquations::save_data(
    const NeuralNetwork& neural_network, const std::string& file_name) const {
  const Matrix<double> solution =
      calculate_Runge_Kutta_solution(neural_network);
