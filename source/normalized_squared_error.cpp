...
/// Default constructor.
/// It creates a normalized squared error performance term object not associated
/// to any
...
/// Neural network constructor.
/// It creates a normalized squared error performance term associated to a
/// neural network object but not measured on any data set.
/// It also initializes all the rest of class members to their default values.
...
/// XML constructor.
/// It creates a normalized squared error not associated to any neural network
/// and not measured on any data set.
/// It also sets all the rest of class members from a TinyXML document->
/// @param normalized_squared_error_document XML document with the class
/// members.
...
// double calculate_training_normalization_coefficient(const Matrix<double>&,
// const Vector<double>&) const method

/// Returns the normalization coefficient to be used for the performance of the
/// error.
/// This is measured on the training instances of the data set.

double NormalizedSquaredError::calculate_normalization_coefficient(
    const Matrix<double>& target_data,
    const Vector<double>& target_data_mean) const {
  return (target_data.calculate_sum_squared_error(target_data_mean));
}

// void check(void) const method

/// Checks that there are a neural network and a data set associated to the
...
void NormalizedSquaredError::check(void) const {
  ...
  const unsigned multilayer_perceptron_inputs_number =
      multilayer_perceptron_pointer->get_inputs_number();
  const unsigned multilayer_perceptron_outputs_number =
  ...
  if (multilayer_perceptron_inputs_number != data_set_inputs_number) {
    buffer << "OpenNN Exception: NormalizedquaredError class.\n"
           ...
           << multilayer_perceptron_inputs_number
           << ") must be equal to number of inputs in data set ("
           << data_set_inputs_number << ").\n";
  ...
  if (multilayer_perceptron_outputs_number != data_set_targets_number) {
    buffer << "OpenNN Exception: NormalizedquaredError class.\n"
           ...
           << multilayer_perceptron_outputs_number
           << ") must be equal to number of targets in data set ("
           << data_set_targets_number << ").\n";
...
/// Returns the performance value of a neural network according to the
/// normalized squared error on a data set.
...
// Control sentence
  ...
  const Vector<double> training_target_data_mean =
      data_set_pointer->calculate_training_target_data_mean();
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    normalization_coefficient +=
        targets.calculate_sum_squared_error(training_target_data_mean);
  }
...
/// perceptron associated to the performance functional.

double NormalizedSquaredError::calculate_performance(
...
double NormalizedSquaredError::calculate_generalization_performance(
    void) const {
// Control sentence
  ...
  const Vector<double> generalization_target_data_mean =
      data_set_pointer->calculate_generalization_target_data_mean();
  ...
  for (unsigned i = 0; i < generalization_instances_number; i++) {
    ...
    normalization_coefficient +=
        targets.calculate_sum_squared_error(generalization_target_data_mean);
  }
...
/// Returns the normalized squared error function gradient of a multilayer
...
// Control sentence (if debug)
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    gradient += calculate_point_gradient(layers_combination_parameters_Jacobian,
                                         layers_delta);
  }
...
/// Returns the normalized squared error function Hessian of a multilayer
/// perceptron on a data set.
/// It uses the error back-propagation method.
/// @todo
...
/// Returns performance vector of the objective terms function for the
/// normalized squared error.
/// It uses the error back-propagation method.
...
/// Returns which would be the objective terms performance vector of a
/// multilayer perceptron for an hypothetical vector of multilayer perceptron
/// parameters.
/// It does not set that vector of parameters to the multilayer perceptron.
/// @param network_parameters Vector of a potential
/// multilayer_perceptron_pointer parameters for the multilayer perceptron
...
// Control sentence (if debug)
  ...
  normalized_squared_error_copy.set_neural_network_pointer(
      &neural_network_copy);
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    if (!has_conditions_layer)  // No conditions
      ...
      const Vector<double>& output_layer_activation =
          layers_activation[layers_number - 1];
...
/// Returns a first order objective terms performance structure, which contains
/// the values and the Jacobian of the objective terms function.
/// @todo

NormalizedSquaredError::FirstOrderTerms
NormalizedSquaredError::calculate_first_order_terms(void) {
...
/// Returns a string with the name of the normalized squared error performance
/// type, "NORMALIZED_SQUARED_ERROR".
...
/// Serializes the normalized squared error object into a XML document of the
/// TinyXML library.
...
tinyxml2::XMLDocument* NormalizedSquaredError::to_XML(void) const {
  ...
  tinyxml2::XMLElement* normalized_squared_error_element =
      document->NewElement("NormalizedSquaredError");
...
void NormalizedSquaredError::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("NormalizedSquaredError");
