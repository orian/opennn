...
/// Default constructor.
/// It creates a solutions error performance term not associated to any neural
/// network and not measured on any mathematical model.
...
/// Mathematical model constructor.
/// It creates solutions error object not associated to any neural network but
/// to be measured on a given mathematical model object.
/// It also initializes all the rest of class members to their default values.
...
/// Neural network and mathematical model constructor.
/// It creates a solutions error functional associated to a neural network and
/// measured on a mathematical model.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_mathematical_model_pointer Pointer to a mathematical model
/// object.
...
/// It creates a solutions error performance term not associated to any neural
/// network and not measured on any mathematical model.
/// It also initializes all the rest of class members with values taken from a
/// TinyXML document->
...
/// The numerical differentiation object is deleted by the performance term
/// destructor.
...
/// Assignment operator.
/// It assigns to this solutions error object the members from another solutions
/// error object.
/// @param other_solutions_error Solutions error object to be copied.

SolutionsError& SolutionsError::operator=(
    const SolutionsError& other_solutions_error) {
  if (this != &other_solutions_error) {
    ...
    *mathematical_model_pointer =
        *other_solutions_error.mathematical_model_pointer;
    *numerical_differentiation_pointer =
        *other_solutions_error.numerical_differentiation_pointer;
...
/// Equal to operator.
/// It compares this object to another object, and returns true if both have the
/// same member data, and false otherwise.

bool SolutionsError::operator==(
    const SolutionsError& other_solutions_error) const {
  if (*neural_network_pointer ==
          *other_solutions_error.neural_network_pointer &&
      *mathematical_model_pointer ==
          *other_solutions_error.mathematical_model_pointer &&
      *numerical_differentiation_pointer ==
          *other_solutions_error.numerical_differentiation_pointer &&
      display == other_solutions_error.display &&
      solutions_errors_weights ==
          other_solutions_error.solutions_errors_weights) {
    return (true);
  } else {
    return (false);
  }
...
const SolutionsError::SolutionsErrorMethod&
SolutionsError::get_solutions_error_method(void) const {
  return (solutions_error_method);
}

// string:std: write_solutions_error_method(void) const method

/// Returns a string with the name of the method for computing the solutions
/// error.
...
/// Returns the weights for every single solution error, corresponding to a
/// dependent variable.
...
// const double& get_solution_error_weight(const unsigned&) const method

/// Returns the weight for a single solution error, corresponding to a dependent
/// variable.

const double& SolutionsError::get_solution_error_weight(
...
/// Sets a new method for calculating the error between the solution of a
/// mathematical model and a target solution.
/// @param new_solutions_error_method Method for evaluating this performance
/// term.

void SolutionsError::set_solutions_error_method(
    const SolutionsErrorMethod& new_solutions_error_method) {
...
void SolutionsError::set_solutions_error_method(
    const std::string& new_solutions_error_method) {
...
/// @param new_solutions_errors_weights Weights for the errors between the
/// dependent variable solutions and the corresponding target solutions.

void SolutionsError::set_solutions_errors_weights(
    const Vector<double>& new_solutions_errors_weights) {
  solutions_errors_weights = new_solutions_errors_weights;
}

// void set_solution_error_weight(const unsigned&, const double&) method

/// Sets a new weight for the solution error of a single dependent variable.
/// @param i Index of dependent variable.
/// @param new_solution_error_weight Weight for the error between the above
/// dependent variable solution and the corresponding target solution.

void SolutionsError::set_solution_error_weight(
    const unsigned& i, const double& new_solution_error_weight) {
...
void SolutionsError::set_default(void) {
  ...
  if (mathematical_model_pointer) {
    const unsigned dependent_variables_number =
        mathematical_model_pointer->get_dependent_variables_number();
...
// Matrix<double> calculate_target_dependent_variables(const Matrix<double>&)
// const method

/// Returns the default target solution matrix.

Matrix<double> SolutionsError::calculate_target_dependent_variables(
    const Matrix<double>& independent_variables) const {
#ifndef NDEBUG
  ...
  const unsigned& rows_number = independent_variables.get_rows_number();
  const unsigned& dependent_variables_number =
      mathematical_model_pointer->get_dependent_variables_number();

  const Matrix<double> target_dependent_variables(
      rows_number, dependent_variables_number, 0.0);
...
/// <li> The pointer to the multilayer perceptron inside the neural network is
/// not NULL.
/// <li> The pointer to the mathematical method is not NULL.
/// <li> The size of the solutions errors weights is equal to the number of
/// dependent variables.
/// </ul>
...
void SolutionsError::check(void) const {
  ...
  const unsigned solutions_errors_weights_size =
      solutions_errors_weights.size();

  if (solutions_errors_weights_size != dependent_variables_number) {
    buffer << "OpenNN Exception: SolutionsError class.\n"
           << "void check(void) const method.\n"
           << "Size of solutions errors weights ("
           << solutions_errors_weights_size
           << ") is not equal to number of dependent variables ("
...
/// Calculates the solutions error performance as the weighted mean squared
/// error between
...
// Control sentence
  ...
  const unsigned independent_variables_number =
      mathematical_model_pointer->get_independent_variables_number();
  const unsigned dependent_variables_number =
      mathematical_model_pointer->get_dependent_variables_number();

  const unsigned variables_number =
      mathematical_model_pointer->count_variables_number();

  const Matrix<double> solution =
      mathematical_model_pointer->calculate_solutions(*neural_network_pointer);

  const Vector<unsigned> independent_variables_indices(
      0, 1, independent_variables_number - 1);
  const Vector<unsigned> dependent_variables_indices(
      independent_variables_number, 1, variables_number - 1);

  const Matrix<double> independent_variables =
      solution.arrange_submatrix_columns(independent_variables_indices);
  const Matrix<double> dependent_variables =
      solution.arrange_submatrix_columns(dependent_variables_indices);

  const Matrix<double> target_dependent_variables =
      calculate_target_dependent_variables(independent_variables);

  double performance = 0.0;

  for (unsigned i = 0; i < dependent_variables_number; i++) {
    if (solutions_errors_weights[i] != 0.0) {
      const Vector<double> dependent_variable =
          dependent_variables.arrange_column(i);
      const Vector<double> target_dependent_variable =
          target_dependent_variables.arrange_column(i);

      performance +=
          solutions_errors_weights[i] *
...
/// Returns the objective value of a neural network according to the solutions
/// error on a mathematical model.
...
/// Calculates the solutions error value of the neural network for a given set
/// of parameters.
/// @param parameters Vector of potential neural network parameters.

double SolutionsError::calculate_performance(
...
// Control sentence
  ...
  solutions_error_copy.set_mathematical_model_pointer(
      mathematical_model_pointer);
...
/// Returns a string with the name of the solutions error performance type,
/// "SOLUTIONS_ERROR".
...
/// Prints to the screen the string representation of this solutions error
/// object.
...
void SolutionsError::print(void) const {
  if (display) {
    std::cout << "Solutions error:\n"
              ...
              << "\n"
              << "Solutions errors weights: " << solutions_errors_weights
              << std::endl;
  }
}
...
/// @param document TinyXML document with the members of the solutions error
/// object.
