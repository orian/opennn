...
/// Default constructor.
/// It creates a sum squared error performance term not associated to any neural
/// network and not measured on any data set.
...
/// Neural network constructor.
/// It creates a sum squared error performance term associated to a neural
/// network but not measured on any data set.
/// It also initializes all the rest of class members to their default values.
...
/// Data set constructor.
/// It creates a sum squared error not associated to any neural network but to
/// be measured on a data set object.
/// It also initializes all the rest of class members to their default values.
...
/// Neural network and data set constructor.
/// It creates a sum squared error associated to a neural network and measured
/// on a data set.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.
/// @param new_data_set_pointer Pointer to a data set object.
...
/// XML constructor.
/// It creates a sum squared error not associated to any neural network and not
/// measured on any data set.
...
/// It creates a sum squared error not associated to any neural network and not
/// measured on any data set.
/// It also sets all the rest of class members from another sum squared error
/// object.
...
/// Returns the performance value of a neural network according to the sum
/// squared error on a data set.
...
/// Returns which would be the sum squard error performance of a neural network
/// for an hypothetical vector of parameters.
...
double SumSquaredError::calculate_performance(
...
/// Returns the sum squared error of the neural network measured on the
/// generalization instances of the data set.
...
/// Calculates the performance term gradient by means of the back-propagation
/// algorithm,
/// and returns it in a single vector of size the number of neural network
/// parameters.
...
Vector<double> SumSquaredError::calculate_gradient(void) const {
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    layers_inputs = multilayer_perceptron_pointer->arrange_layers_input(
        inputs, layers_activation);

    layers_combination_parameters_Jacobian = multilayer_perceptron_pointer
    ...
    if (!has_conditions_layer) {
      ...
      layers_delta =
          calculate_layers_delta(layers_activation_derivative, output_gradient);
    } else {
      particular_solution =
      ...
      layers_delta = calculate_layers_delta(
          layers_activation_derivative, homogeneous_solution, output_gradient);
    }

    point_gradient = calculate_point_gradient(
        layers_combination_parameters_Jacobian, layers_delta);
...
/// and returns it in a single symmetric matrix of size the number of neural
/// network parameters.
...
Matrix<double> SumSquaredError::calculate_Hessian(void) const {
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    second_order_forward_propagation = multilayer_perceptron_pointer
    ...
    perceptrons_combination_parameters_gradient = multilayer_perceptron_pointer
        ->calculate_perceptrons_combination_parameters_gradient(layers_inputs);

    interlayers_combination_combination_Jacobian = multilayer_perceptron_pointer
        ->calculate_interlayers_combination_combination_Jacobian(inputs);

    if (!has_conditions_layer) {
      output_objective_gradient =
          (layers_activation[layers_number - 1] - targets) * 2.0;
      output_objective_Hessian.initialize_diagonal(2.0);

      layers_delta = calculate_layers_delta(layers_activation_derivative,
                                            output_objective_gradient);
      interlayers_Delta = calculate_interlayers_Delta(
          layers_activation_derivative, layers_activation_second_derivative,
          interlayers_combination_combination_Jacobian,
          output_objective_gradient, output_objective_Hessian, layers_delta);
    } else {
      particular_solution =
          conditions_layer_pointer->calculate_particular_solution(inputs);
      homogeneous_solution =
    ...
    objective_Hessian +=
        calculate_point_Hessian(layers_activation_derivative,
                                perceptrons_combination_parameters_gradient,
                                interlayers_combination_combination_Jacobian,
                                layers_delta, interlayers_Delta);
  }
...
/// Calculates the squared error terms for each instance, and returns it in a
/// vector of size the number training instances.
...
/// @param parameters Neural network parameters for which the performance terms
/// vector is to be computed.

Vector<double> SumSquaredError::calculate_terms(
    const Vector<double>& parameters) const {
// Control sentence (if debug)
  ...
  if (size != parameters_number) {
    ...
    buffer << "OpenNN Exception: SumSquaredError class." << std::endl
           ...
           << std::endl << "Size (" << size
           << ") must be equal to number of neural network parameters ("
...
/// Returns the terms_Jacobian matrix of the sum squared error function, whose
/// elements are given by the
/// derivatives of the squared errors data set with respect to the multilayer
/// perceptron parameters.
/// The terms_Jacobian matrix here is computed using a back-propagation
/// algorithm.
...
Matrix<double> SumSquaredError::calculate_terms_Jacobian(void) const {
  ...
  for (unsigned i = 0; i < training_instances_number; i++) {
    ...
    if (!has_conditions_layer) {
      ...
      const Vector<double>& output_layer_activation =
          first_order_forward_propagation[0][layers_number - 1];

      term = (particular_solution +
              homogeneous_solution * output_layer_activation - targets);
      term_norm = term.calculate_norm();
...
/// This is a structure containing the objective terms vector and the objective
/// terms Jacobian.

PerformanceTerm::FirstOrderTerms SumSquaredError::calculate_first_order_terms(
    void) const {
...
Vector<double> SumSquaredError::calculate_gradient(
...
/// Returns a string with the name of the sum squared error performance type,
/// "SUM_SQUARED_ERROR".
...
void SumSquaredError::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("SumSquaredError");
