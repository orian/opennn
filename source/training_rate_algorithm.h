...
/// This class is used by many different training algorithms to calculate the
/// training rate given a training direction.
...
class TrainingRateAlgorithm {
 ...
 public:
  ...
  ///
  /// Defines a set of three points (A, U, B) for bracketing a directional
  /// minimum.
  ///
  ...
  struct Triplet {
    ...
    /// It returns true if both triplets have the same points A, U and B, and
    /// false otherwise.
    /// @ param other_triplet Triplet to be compared with.

    inline bool operator==(const Triplet& other_triplet) const {
      if (A == other_triplet.A && U == other_triplet.U &&
          B == other_triplet.B) {
        return (true);
      } else {
        return (false);
    ...
    inline void print(void) const { std::cout << to_string(); }
  ...
  Vector<double> calculate_fixed_directional_point(const double&,
                                                   const Vector<double>&,
                                                   const double&) const;
  Vector<double> calculate_golden_section_directional_point(
      const double&, const Vector<double>&, const double&) const;
 ...
 protected:
  ...
  /// Variable containing the actual method used to obtain a suitable
  /// perform_training rate.
