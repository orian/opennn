...
/// Default constructor. It creates a data set object with zero instances and
/// zero inputs and target variables.
...
DataSet::DataSet(const unsigned& new_variables_number,
                 const unsigned& new_instances_number) {
...
/// Instances number, input variables number and target variables number
/// constructor.
/// It creates a data set object with given instances and inputs and target
/// variables numbers.
/// It also initializes the rest of class members to their default values.
...
DataSet::DataSet(const unsigned& new_inputs_number,
                 const unsigned& new_targets_number,
                 const unsigned& new_instances_number) {
...
/// File constructor. It creates a data set object by loading the object members
/// from a XML-type file.
...
bool DataSet::operator==(const DataSet& other_data_set) const {
  if (data_file_name == other_data_set.data_file_name &&
      data == other_data_set.data && variables == other_data_set.variables &&
      instances == other_data_set.instances &&
      display == other_data_set.display) {
    return (true);
  } else {
...
/// Returns a constant reference to the variables object composing this data set
/// object.

const Variables& DataSet::get_variables(void) const { return (variables); }

// Variables* get_variables_pointer(void) const

/// Returns a pointer to the variables object composing this data set object.

Variables* DataSet::get_variables_pointer(void) { return (&variables); }

// const Instances& get_instances(void) const

/// Returns a constant reference to the instances object composing this data set
/// object.

const Instances& DataSet::get_instances(void) const { return (instances); }

// Instances* get_instances_pointer(void)

/// Returns a pointer to the variables object composing this data set object.

Instances* DataSet::get_instances_pointer(void) { return (&instances); }
...
const bool& DataSet::get_display(void) const { return (display); }

// bool empty(void) const method

/// Returns true if the data matrix is empty, and false otherwise.

bool DataSet::empty(void) const { return (data.empty()); }
...
const Matrix<double>& DataSet::get_data(void) const { return (data); }
...
// static ScalingUnscalingMethod get_scaling_unscaling_method(const
// std::string&) method

/// Returns a value of the scaling-unscaling method enumeration from a string
/// containing the name of that method.
/// @param scaling_unscaling_method String with the name of the scaling and
/// unscaling method.

DataSet::ScalingUnscalingMethod DataSet::get_scaling_unscaling_method(
    const std::string& scaling_unscaling_method) {
  if (scaling_unscaling_method == "MinimumMaximum") {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "static ScalingUnscalingMethod "
              "get_scaling_unscaling_method(const std::string).\n"
...
Matrix<double> DataSet::arrange_training_data(void) const {
  ...
  const Vector<unsigned> training_indices =
      instances.arrange_training_indices();
...
Matrix<double> DataSet::arrange_generalization_data(void) const {
  ...
  const Vector<unsigned> generalization_indices =
      instances.arrange_generalization_indices();
...
/// Returns the inputs and target values of a single training instance in the
/// data set.
/// @param training_instance_index Index of the training instance.

Vector<double> DataSet::get_training_instance(
    const unsigned& training_instance_index) const {
// Control sentence (if debug)
  ...
  if (training_instance_index >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_training_instance(const unsigned&) const "
              "method.\n"
           << "Index of training instance must be less than number of training "
              "instances.\n";
...
/// Returns the inputs and target values of a single generalization instance in
/// the data set.
/// @param generalization_instance_index Index of the generalization instance.

Vector<double> DataSet::get_generalization_instance(
    const unsigned& generalization_instance_index) const {
// Control sentence (if debug)
  ...
  if (generalization_instance_index >= generalization_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_generalization_instance(const unsigned&) "
              "const method.\n"
           << "Index of generalization instance must be less than number of "
  ...
  return (
      data.arrange_row(generalization_indices[generalization_instance_index]));
}

// Vector<double> get_testing_instance(const unsigned&) const method

/// Returns the inputs and target values of a single testing instance in the
/// data set.
/// @param testing_instance_index Index of the testing instance.

Vector<double> DataSet::get_testing_instance(
    const unsigned& testing_instance_index) const {
// Control sentence (if debug)
  ...
  if (testing_instance_index >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_testing_instance(const unsigned&) const "
              "method.\n"
           << "Index of testing instance must be less than number of testing "
              "instances.\n";
...
Vector<double> DataSet::get_input_instance(
    const unsigned& instance_index) const {
// Control sentence (if debug)
...
/// Returns a vector with the target values of a single instance in the data
/// set.
/// @param instance_index Index of the instance.

Vector<double> DataSet::get_target_instance(
    const unsigned& instance_index) const {
// Control sentence (if debug)
...
Vector<double> DataSet::get_training_input_instance(
    const unsigned& training_instance_index) const {
// Control sentence (if debug)
  ...
  if (training_instance_index >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_training_input_instance(const unsigned&) "
              "const method.\n"
           << "Index of training instance (" << training_instance_index
           << ") must be less than number of training instances ("
...
Vector<double> DataSet::get_generalization_input_instance(
    const unsigned& generalization_instance_index) const {
// Control sentence (if debug)
  ...
  if (generalization_instance_index >= generalization_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_generalization_input_instance(const "
  ...
  return (data.arrange_row(
      generalization_indices[generalization_instance_index], inputs_indices));
}
...
Vector<double> DataSet::get_testing_input_instance(
    const unsigned& testing_instance_index) const {
// Control sentence (if debug)
  ...
  if (testing_instance_index >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_testing_input_instance(const unsigned&) "
              "const method.\n"
           << "Index of testing instance must be less than number of testing "
              "instances.\n";
...
Vector<double> DataSet::get_training_target_instance(
    const unsigned& training_instance_index) const {
// Control sentence (if debug)
  ...
  if (training_instance_index >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_training_target_instance(const unsigned&) "
              "const method.\n"
           << "Index of training instance must be less than number of training "
              "instances.\n";
...
Vector<double> DataSet::get_generalization_target_instance(
    const unsigned& generalization_instance_index) const {
// Control sentence (if debug)
  ...
  if (generalization_instance_index >= generalization_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_generalization_target_instance(const "
              "unsigned&) const method.\n"
           << "Index of generalization instance must be less than number of "
              "generalization instances.\n";
  ...
  return (data.arrange_row(
      generalization_indices[generalization_instance_index], targets_indices));
}
...
Vector<double> DataSet::get_testing_target_instance(
    const unsigned& testing_instance_index) const {
// Control sentence (if debug)
  ...
  if (testing_instance_index >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<double> get_testing_target_instance(const unsigned&) "
              "const method.\n"
           << "Index of testing instance must be less than number of testing "
              "instances.\n";
...
void DataSet::set(const unsigned& new_variables_number,
                  const unsigned& new_instances_number) {
  data_file_name = "";
...
/// Sets new numbers of instances and inputs and target variables in the data
/// set.
/// The variables in the data set are the number of inputs plus the number of
/// targets.
///
/// @param new_inputs_number Number of input variables.
/// @param new_targets_number Number of target variables.
...
void DataSet::set(const unsigned& new_inputs_number,
                  const unsigned& new_targets_number,
                  const unsigned& new_instances_number) {
  data_file_name = "";
...
/// Sets the members of this data set object with those from another data set
/// object.
/// @param other_data_set Data set object to be copied.
...
void DataSet::set(const std::string& file_name) { load(file_name); }
...
void DataSet::set_display(const bool& new_display) { display = new_display; }
...
void DataSet::set_default(void) { display = true; }
...
/// Indices of all training, generalization and testing instances and inputs and
/// target variables do not change.
...
/// The indices of the training, generalization and testing instances do not
/// change.
...
void DataSet::set_instance(const unsigned& instance_index,
...
// Control sentence (if debug)
  ...
  if (size != variables_number) {
    ...
    buffer
        << "OpenNN Exception: DataSet class.\n"
        << "void set_instance(const unsigned&, const Vector<double>&) method.\n"
        << "Size (" << size << ") must be equal to number of variables ("
        << variables_number << ").\n";
...
/// Sets new inputs and target values of a single training instance in the data
/// set.
/// @param i Index of the training instance.
/// @param new_training_instance New inputs and target values of the training
/// instance.

void DataSet::set_training_instance(
    const unsigned& i, const Vector<double>& new_training_instance) {
// Control sentence (if debug)
  ...
  if (i >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_training_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Training instance index must be less than number of training "
              "instances.\n";
...
/// Sets new inputs and target values of a single generalization instance in the
/// data set.
/// @param i Index of the generalization instance.
/// @param new_generalization_instance New inputs and target values of the
/// generalization instance.

void DataSet::set_generalization_instance(
    const unsigned& i, const Vector<double>& new_generalization_instance) {
// Control sentence (if debug)
  ...
  if (i >= generalization_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_generalization_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Generalization instance index must be less than number of "
              "generalization instances.\n";
...
/// Sets new inputs and target values of a single testing instance in the data
/// set.
/// @param i Index of the testing instance.
/// @param new_testing_instance New inputs and target values of the testing
/// instance.

void DataSet::set_testing_instance(const unsigned& i,
...
// Control sentence (if debug)
  ...
  if (i >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_testing_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Testing instance index must be less than number of testing "
              "instances.\n";
...
void DataSet::set_input_instance(const unsigned& instance_index,
...
// Control sentence (if debug)
  ...
  if (size != inputs_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_input_instance(const unsigned&, const Vector<double>&) "
              "method.\n"
           << "Size of inputs instance must be equal to number of input "
              "variables.\n";
...
void DataSet::set_target_instance(const unsigned& instance_index,
...
// Control sentence (if debug)
  ...
  if (instance_index >= instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_target_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Index of instance must be less than number of instances.\n";
  ...
  if (size != targets_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_target_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Size of targets instance must be equal to number of target "
              "variables.\n";
...
void DataSet::set_training_input_instance(
    const unsigned& i, const Vector<double>& new_training_input_instance) {
// Control sentence (if debug)
  ...
  if (i >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_training_input_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Training instance index must be less than number of training "
              "instances.\n";
...
// void set_generalization_input_instance(const unsigned&, const
// Vector<double>&) method
...
void DataSet::set_generalization_input_instance(
    const unsigned& i,
...
void DataSet::set_testing_input_instance(
    const unsigned& i, const Vector<double>& new_testing_input_instance) {
// Control sentence (if debug)
  ...
  if (i >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_testing_input_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Testing instance index must be less than number of testing "
              "instances.\n";
...
void DataSet::set_training_target_instance(
    const unsigned& i, const Vector<double>& new_training_target_instance) {
// Control sentence (if debug)
  ...
  if (i >= training_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_training_target_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Training instance index must be less than number of training "
              "instances.\n";
...
// void set_generalization_target_instance(const unsigned&, const
// Vector<double>&) method

/// Sets new target values of a single generalization instance in the data set.
/// @param i Index of the generalization instance.
/// @param new_generalization_target_instance New target values of that
/// instance.

void DataSet::set_generalization_target_instance(
    const unsigned& i,
...
void DataSet::set_testing_target_instance(
    const unsigned& i, const Vector<double>& new_testing_target_instance) {
// Control sentence (if debug)
  ...
  if (i >= testing_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void set_testing_target_instance(const unsigned&, const "
              "Vector<double>&) method.\n"
           << "Testing instance index must be less than number of testing "
              "instances.\n";
...
/// Note that resizing is here necessary and therefore computationally
/// expensive.
/// All instances are also set for training.
...
/// @param variable Vector of values. The size must be equal to the number of
/// instances.
...
/// Removes the input of target indices of that variables with zero standard
/// deviation.
/// It might change the size of the vectors containing the inputs and targets
/// indices.
...
/// Removes the training, generalization and testing indices of that instances
/// which are repeated in the data matrix.
/// It might change the size of the vectors containing the training,
/// generalization and testing indices.
...
Vector<Histogram<double> > DataSet::calculate_data_histograms(
...
/// Returns a vector of vectors containing some basic statistics of all the
...
// Vector< Statistics<double> > calculate_training_instances_statistics(void)
// const method

/// Returns a vector of vectors containing some basic statistics of all
/// variables on the training instances.
/// The size of this vector is four. The subvectors are:
/// <ul>
...
Vector<Statistics<double> > DataSet::calculate_training_instances_statistics(
    void) const {
  const Vector<unsigned> training_indices =
      instances.arrange_training_indices();
...
// Vector< Statistics<double> >
// calculate_generalization_instances_statistics(void) const method

/// Returns a vector of vectors containing some basic statistics of all
/// variables on the generalization instances.
/// The size of this vector is four. The subvectors are:
/// <ul>
...
Vector<Statistics<double> >
DataSet::calculate_generalization_instances_statistics(void) const {
  const Vector<unsigned> generalization_indices =
      instances.arrange_generalization_indices();
...
// Vector< Statistics<double> > calculate_testing_instances_statistics(void)
// const method

/// Returns a vector of vectors containing some basic statistics of all
/// variables on the testing instances.
/// The size of this vector is four. The subvectors are:
/// <ul>
...
Vector<Statistics<double> > DataSet::calculate_testing_instances_statistics(
    void) const {
  const Vector<unsigned> testing_indices = instances.arrange_testing_indices();
...
/// Returns a vector of vectors with some basic statistics of the input
/// variables on all instances.
/// The size of this vector is four. The subvectors are:
/// <ul>
...
/// Returns a vector of vectors with some basic statistics of the target
/// variables on all instances.
/// The size of this vector is four. The subvectors are:
/// <ul>
...
/// Returns the mean values of the target variables on the generalization
/// instances.
...
/// It returns a matrix with number of rows the targets number and number of
/// columns the inputs number.
/// Each element contains the linear correlation between a single target and a
/// single output.
...
Matrix<double> DataSet::calculate_linear_correlations(void) const {
  ...
  for (unsigned i = 0; i < targets_number; i++) {
    ...
    for (unsigned j = 0; j < inputs_number; j++) {
      ...
      linear_correlations[i][j] =
          target_variable.calculate_linear_correlation(input_variable);
    }
  }
...
void DataSet::scale_data_mean_standard_deviation(
    const Vector<Statistics<double> >& data_statistics) {
// Control sentence (if debug)
  ...
  if (statistics_size != columns_number) {
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void scale_data_mean_standard_deviation(const Vector< "
              "Statistics<double> >&) method.\n"
...
void DataSet::scale_data_minimum_maximum(
    const Vector<Statistics<double> >& data_statistics) {
  const unsigned variables_number = variables.get_variables_number();
...
// Control sentence (if debug)
  ...
  if (statistics_size != variables_number) {
    buffer << "OpenNN Exception: DataSet class.\n"
           << "void scale_data_minimum_maximum(const Vector< "
              "Statistics<double> >&) method.\n"
  ...
  for (unsigned i = 0; i < variables_number; i++) {
    if (display &&
        data_statistics[i].maximum - data_statistics[i].minimum < 1.0e-99) {
      std::cout << "OpenNN Warning: DataSet class.\n"
                << "void scale_data_minimum_maximum(const Vector< "
                   "Statistics<Type> >&) method.\n"
...
/// @param scaling_unscaling_method_string String with the name of the
/// scaling-unscaling method
/// (MinimumMaximum or MeanStandardDeviation).
/// @param data_statistics Vector of statistics structures for all the variables
/// in the data set.
/// The size of that vector must be equal to the number of variables.

void DataSet::scale_data(const std::string& scaling_unscaling_method_string,
                         const Vector<Statistics<double> >& data_statistics) {
  switch (get_scaling_unscaling_method(scaling_unscaling_method_string)) {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: DataSet class\n"
             << "void scale_data(const std::string&, const Vector< "
                "Vector<double> >&) method.\n"
...
/// Calculates the data statistics, scales the data with that values and returns
/// the statistics.
/// The method to be used is that in the scaling and unscaling method variable.

Vector<Statistics<double> > DataSet::scale_data(
    const std::string& scaling_unscaling_method) {
...
/// @param inputs_statistics Vector of statistics structures for the input
/// variables.
/// The size of that vector must be equal to the number of inputs.

void DataSet::scale_inputs_mean_standard_deviation(
...
/// Scales the input variables with the calculated mean and standard deviation
...
/// It updates the input variables of the data matrix.
/// @param inputs_statistics Vector of statistics structures for all the inputs
/// in the data set.
/// The size of that vector must be equal to the number of input variables.

void DataSet::scale_inputs_minimum_maximum(
    const Vector<Statistics<double> >& inputs_statistics) {
  const Vector<unsigned> inputs_indices = variables.arrange_inputs_indices();
...
/// Scales the input variables with the calculated minimum and maximum values
/// from the data matrix.
/// It updates the input variables of the data matrix.
/// It also returns a vector of vectors with the minimum and maximum values of
/// the input variables.
...
Vector<Statistics<double> > DataSet::scale_inputs(
    const std::string& scaling_unscaling_method) {
  switch (get_scaling_unscaling_method(scaling_unscaling_method)) {
    case MinimumMaximum: {
...
void DataSet::scale_inputs(
  ...
  switch (get_scaling_unscaling_method(scaling_unscaling_method)) {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: DataSet class\n"
             << "void scale_inputs(const std::string&, const Vector< "
                "Statistics<double> >&) method.\n"
             << "Unknown scaling and unscaling method.\n";
...
/// @param targets_statistics Vector of statistics structures for all the
/// targets in the data set.
/// The size of that vector must be equal to the number of target variables.

void DataSet::scale_targets_mean_standard_deviation(
    const Vector<Statistics<double> >& targets_statistics) {
  const Vector<unsigned> targets_indices = variables.arrange_targets_indices();

  data.scale_columns_mean_standard_deviation(targets_statistics,
                                             targets_indices);
}
...
/// Scales the target variables with the calculated mean and standard deviation
/// values from the data matrix.
/// It updates the target variables of the data matrix.
/// It also returns a vector of statistics structures with the basic statistics
...
void DataSet::scale_targets_minimum_maximum(
    const Vector<Statistics<double> >& targets_statistics) {
  const Vector<unsigned> targets_indices = variables.arrange_targets_indices();
...
/// Scales the target variables with the calculated minimum and maximum values
/// from the data matrix.
/// It updates the target variables of the data matrix.
/// It also returns a vector of vectors with the statistics of the input target
/// variables.
...
Vector<Statistics<double> > DataSet::scale_targets(
    const std::string& scaling_unscaling_method) {
  switch (get_scaling_unscaling_method(scaling_unscaling_method)) {
    case MinimumMaximum: {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: DataSet class\n"
             << "Vector< Statistics<double> > scale_targets(const "
                "std::string&) method.\n"
             << "Unknown scaling and unscaling method.\n";
...
void DataSet::scale_targets(
  ...
  switch (get_scaling_unscaling_method(scaling_unscaling_method)) {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: DataSet class\n"
             << "void scale_targets(const std::string&, const Vector< "
                "Statistics<double> >&) method.\n"
             << "Unknown scaling and unscaling method.\n";
...
void DataSet::unscale_data_mean_standard_deviation(
    const Vector<Statistics<double> >& data_statistics) {
...
void DataSet::unscale_data_minimum_maximum(
    const Vector<Statistics<double> >& data_statistics) {
...
void DataSet::unscale_inputs_mean_standard_deviation(
    const Vector<Statistics<double> >& inputs_statistics) {
  const Vector<unsigned> inputs_indices = variables.arrange_inputs_indices();

  data.unscale_columns_mean_standard_deviation(inputs_statistics,
                                               inputs_indices);
}
...
void DataSet::unscale_inputs_minimum_maximum(
    const Vector<Statistics<double> >& inputs_statistics) {
  const Vector<unsigned> inputs_indices = variables.arrange_inputs_indices();
...
void DataSet::unscale_targets_mean_standard_deviation(
    const Vector<Statistics<double> >& targets_statistics) {
  const Vector<unsigned> targets_indices = variables.arrange_targets_indices();

  data.unscale_columns_mean_standard_deviation(targets_statistics,
                                               targets_indices);
}
...
void DataSet::unscale_targets_minimum_maximum(
    const Vector<Statistics<double> >& targets_statistics) {
  const Vector<unsigned> targets_indices = variables.arrange_targets_indices();
...
/// Initializes the data matrix with random values chosen from a uniform
/// distribution
/// with given minimum and maximum.

void DataSet::randomize_data_uniform(const double& minimum,
...
/// Initializes the data matrix with random values chosen from a normal
/// distribution
/// with given mean and standard deviation.

void DataSet::randomize_data_normal(const double& mean,
...
tinyxml2::XMLDocument* DataSet::to_XML(void) const {
  ...
  // Variables
  {
    ...
    const tinyxml2::XMLElement* variables_element =
        variables_document->FirstChildElement("Variables");
  ...
  // Instances
  {
    ...
    const tinyxml2::XMLElement* instances_element =
        instances_document->FirstChildElement("Instances");
...
void DataSet::from_XML(const tinyxml2::XMLDocument& data_set_document) {
  ...
  const tinyxml2::XMLElement* data_set_element =
      data_set_document.FirstChildElement("DataSet");
  ...
  const tinyxml2::XMLElement* data_file_name_element =
      data_set_element->FirstChildElement("DataFileName");
  ...
  const tinyxml2::XMLElement* variables_element =
      data_set_element->FirstChildElement("Variables");

  if (variables_element) {
    tinyxml2::XMLDocument variables_document;

    tinyxml2::XMLElement* variables_element_clone =
        variables_document.NewElement("Variables");
    variables_document.InsertFirstChild(variables_element_clone);

    DeepClone(variables_element_clone, variables_element, &variables_document,
              NULL);
  ...
  const tinyxml2::XMLElement* instances_element =
      data_set_element->FirstChildElement("Instances");

  if (instances_element) {
    tinyxml2::XMLDocument instances_document;

    tinyxml2::XMLElement* instances_element_clone =
        instances_document.NewElement("Instances");
    instances_document.InsertFirstChild(instances_element_clone);

    DeepClone(instances_element_clone, instances_element, &instances_document,
              NULL);
...
/// Saves the members of a data set object to a XML-type file in an XML-type
/// format.
...
/// Returns a vector containing the number of instances of each class in the
/// data set.
/// If the number of target variables is one then the number of classes is two.
/// If the number of target variables is greater than one then the number of
/// classes is equal to the number
/// of target variables.
...
Vector<unsigned> DataSet::calculate_target_class_distribution(void) const {
  ...
  if (targets_number == 1)  // Two classes
    ...
    for (unsigned instance_index = 0; instance_index < instances_number;
         instance_index++) {
  ...
  const unsigned used_instances_number =
  ...
  if (class_distribution.calculate_sum() != used_instances_number) {
    ...
    buffer << "OpenNN Exception: DataSet class.\n"
           << "Vector<int> calculate_target_class_distribution(void) const "
              "method.\n"
           << "Sum of class distributions is not equal to number of used "
              "instances.\n"
