...
/// Default constructor.
/// It creates a random search training algorithm not associated to any
/// performance functional object.
/// It also initializes the class members to their default values.

RandomSearch::RandomSearch(void) : TrainingAlgorithm() { set_default(); }

// PERFORMANCE FUNCTIONAL CONSTRUCTOR

/// Performance functional constructor.
/// It creates a random search training algorithm associated to a performance
/// functional object.
/// It also initializes the class members to their default values.
...
/// @param document TinyXML document containing the members of a random search
/// object.

RandomSearch::RandomSearch(const tinyxml2::XMLDocument& document)
...
const unsigned& RandomSearch::get_maximum_generalization_performance_decreases(
    void) const {
...
const bool& RandomSearch::get_reserve_generalization_performance_history(
    void) const {
...
void RandomSearch::set_first_training_rate(
    const double& new_first_training_rate) {
...
void RandomSearch::set_training_rate_reduction_factor(
    const double& new_training_rate_reduction_factor) {
  training_rate_reduction_factor = new_training_rate_reduction_factor;
}

// void set_training_rate_reduction_period(unsigned) method

/// Sets a new period value for the reduction of the training rate. This is
/// measured in training iterations.
/// @param new_training_rate_reduction_period Reduction period for the training
/// rate.

void RandomSearch::set_training_rate_reduction_period(
    const unsigned& new_training_rate_reduction_period) {
  training_rate_reduction_period = new_training_rate_reduction_period;
}

// void set_reserve_parameters_history(bool) method

/// Makes the potential parameters history vector of vectors to be reseved or
/// not in memory.
/// @param new_reserve_parameters_history True if the potential parameters
/// history is to be reserved, false otherwise.

void RandomSearch::set_reserve_parameters_history(
...
/// Makes the potential parameters norm history vector to be reseved or not in
/// memory.
/// @param new_reserve_parameters_norm_history True if the potential parameters
/// norm history is to be reserved,
/// false otherwise.

void RandomSearch::set_reserve_parameters_norm_history(
...
/// Makes the potential performance history vector to be reseved or not in
/// memory.
/// @param new_reserve_performance_history True if the potential performance
/// history is to be reserved,
/// false otherwise.

void RandomSearch::set_reserve_performance_history(
...
void RandomSearch::set_reserve_all_training_history(
...
void RandomSearch::set_warning_parameters_norm(
...
void RandomSearch::set_warning_training_rate(
...
void RandomSearch::set_error_parameters_norm(
...
void RandomSearch::set_error_training_rate(
...
void RandomSearch::set_maximum_generalization_performance_decreases(
...
void RandomSearch::set_maximum_iterations_number(
...
void RandomSearch::set_reserve_training_direction_history(
...
/// Makes the training direction norm history vector to be reseved or not in
/// memory.
/// @param new_reserve_training_direction_norm_history True if the history of
/// the norm of the training direction is to be reserved,
/// false otherwise.

void RandomSearch::set_reserve_training_direction_norm_history(
    const bool& new_reserve_training_direction_norm_history) {
  reserve_training_direction_norm_history =
      new_reserve_training_direction_norm_history;
}
...
void RandomSearch::set_reserve_training_rate_history(
...
void RandomSearch::set_reserve_elapsed_time_history(
...
void RandomSearch::set_reserve_generalization_performance_history(
...
void RandomSearch::RandomSearchResults::resize_training_history(
...
/// Returns a string representation of the current random search results
/// structure.
...
Matrix<std::string> RandomSearch::RandomSearchResults::write_final_results(
  ...
  const PerformanceFunctional* performance_functional_pointer =
      random_search_pointer->get_performance_functional_pointer();
...
/// to the random search training algorithm.
...
// Control sentence (if debug)
  ...
  for (unsigned iteration = 0; iteration <= maximum_iterations_number;
    ...
    //      {
    //         results_pointer->potential_parameters_history[iteration] =
    // potential_parameters;
    //      }

    //      if(reserve_potential_parameters_norm_history)
    //      {
    //         results_pointer->potential_parameters_norm_history[iteration] =
    // potential_parameters_norm;
    //      }
...
void RandomSearch::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("RandomSearch");
  ...
  // First training rate
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("FirstTrainingRate");
  ...
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("TrainingRateReductionFactor");
  ...
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("TrainingRateReductionPeriod");
  ...
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("ReserveTrainingDirectionNormHistory");

    if (element) {
      const std::string new_reserve_training_direction_norm_history =
          element->GetText();

      try {
        set_reserve_training_direction_norm_history(
            new_reserve_training_direction_norm_history != "0");
      }
      catch (const std::logic_error & e) {
        std::cout << e.what() << std::endl;
      }
