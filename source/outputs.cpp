...
Outputs::Outputs(void) { set(); }

// OUTPUTS NUMBER CONSTRUCTOR

/// Outputs number constructor.
/// It creates a outputs object with a given number of outputs.
/// This constructor initializes the members of the object to their default
/// values.
...
Outputs::Outputs(const Outputs& other_outputs) { set(other_outputs); }
...
/// Such a name is only used to give the user basic information about the
/// problem at hand.
...
/// Such a description is only used to give the user basic information about the
/// problem at hand.
/// @param index Index of output variable.
...
Vector<std::string> Outputs::arrange_units(void) const { return (units); }
...
/// Returns all the available information about the outputs as a single matrix
/// of strings.
/// The number of rows is the number of outputs.
/// The number of columns is three.
/// Each row contains the information of a single output (name, units and
/// description).
...
const bool& Outputs::get_display(void) const { return (display); }
...
/// Sets the members of this outputs object with those from another object of
/// the same class.
/// @param other_outputs Outputs object to be copied.
...
void Outputs::set(const Vector<Item>& new_items) { items = new_items; }
...
// Control sentence (if debug)
  ...
  if (size != outputs_number) {
    ...
    buffer << "OpenNN Exception: Outputs class.\n"
           << "void set_names(const Vector<std::string>&) method.\n"
           << "Size of name of outputs vector must be equal to number of "
              "output variables.\n";
...
/// Such value is only used to give the user basic information on the problem at
/// hand.
/// @param index Index of output variable.
...
void Outputs::set_description(const unsigned& index,
...
void Outputs::set_display(const bool& new_display) { display = new_display; }
...
/// Serializes the outputs information object into a XML document of the TinyXML
/// library.
...
void Outputs::from_XML(const tinyxml2::XMLDocument& document) {
  ...
  const tinyxml2::XMLElement* outputs_element =
      document.FirstChildElement("Outputs");
  ...
  const tinyxml2::XMLElement* outputs_number_element =
      outputs_element->FirstChildElement("OutputsNumber");
