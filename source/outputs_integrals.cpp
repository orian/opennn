...
/// Default constructor.
/// It creates an outputs integrals performance term not associated to any
/// neural network.
/// It also initializes all the rest of class members to their default values.

OutputsIntegrals::OutputsIntegrals(void) : PerformanceTerm() { set_default(); }

// NEURAL NETWORK CONSTRUCTOR

/// Neural network constructor.
/// It creates an outputs integrals performance term associated to a neural
/// network.
/// It also initializes all the rest of class members to their default values.
...
/// @param outputs_integrals_document Pointer to a TinyXML document with the
/// object data.
...
/// Returns a constant reference to the numerical integration object inside the
/// outputs integrals object.

const NumericalIntegration& OutputsIntegrals::get_numerical_integration(
    void) const {
...
/// Returns a pointer to the numerical integration object inside the outputs
/// integrals object.
...
const Vector<double>& OutputsIntegrals::get_outputs_integrals_weights(
    void) const {
...
const double& OutputsIntegrals::get_output_integral_weight(
...
void OutputsIntegrals::set_numerical_integration(
    const NumericalIntegration& new_numerical_integration) {
...
void OutputsIntegrals::set_outputs_integrals_weights(
    const Vector<double>& new_outputs_integrals_weights) {
...
void OutputsIntegrals::set_output_integral_weight(
    const unsigned& i, const double& new_output_integral_weight) {
...
/// to the outputs integrals performance term,
...
/// Returns the objective value of a multilayer perceptron according to the sum
...
/// Returns which would be the performance of a neural network for an
...
/// algorithm,
/// and returns it in a single vector of size the number of multilayer
...
/// Returns a string with the name of the outputs integrals performance type,
/// "OUTPUTS_INTEGRALS".
...
tinyxml2::XMLDocument* OutputsIntegrals::to_XML(void) const {
  ...
  tinyxml2::XMLElement* outputs_integrals_element =
      document->NewElement("OutputsIntegrals");
  ...
  {
    tinyxml2::XMLElement* element =
        document->NewElement("OutputsIntegralsWeights");
    outputs_integrals_element->LinkEndChild(element);
...
void OutputsIntegrals::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("OutputsIntegrals");
