...
BoundingLayer::BoundingLayer(void) { set(); }
...
BoundingLayer::BoundingLayer(const unsigned& bounding_neurons_number) {
...
BoundingLayer& BoundingLayer::operator=(
    const BoundingLayer& other_bounding_layer) {
...
bool BoundingLayer::operator==(
    const BoundingLayer& other_bounding_layer) const {
...
// unsigned get_bounding_neurons_number(void) const method

/// Returns the number of bounding neurons in the layer.

unsigned BoundingLayer::get_bounding_neurons_number(void) const {
...
// double get_lower_bound(const unsigned&) const const method

/// Returns the lower bound value of a single bounding neuron.
/// @param i Index of bounding neuron.

double BoundingLayer::get_lower_bound(const unsigned& i) const {
// Control sentence (if debug)

#ifndef NDEBUG

  const unsigned bounding_neurons_number = get_bounding_neurons_number();
...
double BoundingLayer::get_upper_bound(const unsigned& i) const {
// Control sentence (if debug)
...
void BoundingLayer::set(const unsigned& new_bounding_neurons_number) {
...
/// @param bounding_layer_document Pointer to a TinyXML document containing the
/// member data.
...
/// This value is used for unscaling that variable so that it is not less than
/// the lower bound.
/// @param index Index of bounding neuron.
/// @param new_lower_bound New lower bound for the neuron with index i.

void BoundingLayer::set_lower_bound(const unsigned& index,
...
// Control sentence (if debug)
  ...
  if (index >= bounding_neurons_number) {
    ...
    buffer << "OpenNN Exception: BoundingLayer class.\n"
           << "void set_lower_bound(const unsigned&, const double&) method.\n"
           << "Index of bounding neurons must be less than number of bounding "
              "neurons.\n";
...
/// These values are used for unscaling variables so that they are not greater
...
/// This value is used for unscaling that variable so that it is not greater
/// than the upper bound.
/// @param index Index of bounding neuron.
/// @param new_upper_bound New upper bound for the bounding neuron with that
/// index.

void BoundingLayer::set_upper_bound(const unsigned& index,
...
// Control sentence (if debug)
  ...
  if (index >= bounding_neurons_number) {
    ...
    buffer << "OpenNN Exception: BoundingLayer class.\n"
           << "void set_upper_bound(const unsigned&, const double&) method.\n"
           << "Index of bounding neuron must be less than number of bounding "
              "neurons.\n";
...
/// Sets both the lower bounds and the upper bounds of all the neurons in the
/// layer.
/// The format is a vector of two real vectors.
/// The first element must contain the lower bound values for the bounding
/// neurons.
/// The second element must contain the upper bound values for the bounding
/// neurons.
/// These values are used for unscaling variables so that they are neither less
/// than the lower bounds nor greater than the upper bounds.
...
// Control sentence (if debug)
  ...
  const unsigned size = new_bounds.size();
...
void BoundingLayer::set_default(void) { display = true; }

// void prune_bounding_neuron(const unsigned&) method

/// Removes a given bounding neuron from the bounding layer.
/// @param index Index of neuron to be pruned.

void BoundingLayer::prune_bounding_neuron(const unsigned& index) {
// Control sentence (if debug)
  ...
  if (index >= bounding_neurons_number) {
    ...
    buffer << "OpenNN Exception: BoundingLayer class.\n"
           << "void prune_bounding_neuron(const unsigned&) method.\n"
           << "Index of bounding neuron is equal or greater than number of "
              "bounding neurons.\n";
...
/// Initializes the lower and upper bounds of all the bounding neurons with
/// random values.
...
/// Calculates the outputs from the bounding layer for a set of inputs to that
/// layer.
/// @param inputs Set of inputs to the bounding layer.

Vector<double> BoundingLayer::calculate_outputs(
...
Vector<double> BoundingLayer::calculate_derivative(
    const Vector<double>& inputs) const {
  const unsigned bounding_neurons_number = get_bounding_neurons_number();
  ...
  for (unsigned i = 0; i < bounding_neurons_number; i++) {
  ...
  for (unsigned i = 0; i < bounding_neurons_number; i++) {
    if (outputs[i] == lower_bounds[i]) {
      buffer << "OpenNN Exception: BoundingLayer class.\n"
             << "Vector<double> calculate_outputs(const Vector<double>&) const "
                "method.\n"
             << "Output is equal to lower bound. The bounding function is not "
      ...
      buffer << "OpenNN Exception: BoundingLayer class.\n"
             << "Vector<double> calculate_outputs(const Vector<double>&) const "
                "method.\n"
             << "Output is equal to upper bound. The bounding function is not "
                "differentiable at this point.\n";
...
/// The Jacobian matrix is composed of the partial derivatives of the layer
/// outputs with respect to the layer inputs.
/// @param derivatives Vector of outputs-inputs derivatives of each bounding
/// neuron.
...
// Control sentence (if debug)
  ...
  const unsigned derivatives_size = derivatives.size();
  ...
  Matrix<double> Jacobian(bounding_neurons_number, bounding_neurons_number,
                          0.0);
...
/// The Hessian form is composed of the second partial derivatives of the layer
/// outputs with respect to the layer inputs.

Vector<Matrix<double> > BoundingLayer::arrange_Hessian_form(
...
/// Returns a string with the expression of the lower and upper bounds
/// functions.

std::string BoundingLayer::write_expression(
  ...
  for (unsigned i = 0; i < bounding_neurons_number; i++) {
    expression << outputs_name[i] << " < " << lower_bounds[i] << " ? "
               << lower_bounds[i] << " : " << inputs_name[i] << "\n";
    expression << outputs_name[i] << " > " << upper_bounds[i] << " ? "
               << upper_bounds[i] << " : " << inputs_name[i] << "\n";
  }
...
/// Serializes the bounding layer object into a XML document of the TinyXML
/// library.
...
tinyxml2::XMLDocument* BoundingLayer::to_XML(void) const {
  ...
  tinyxml2::XMLElement* bounding_layer_element =
      document->NewElement("BoundingLayer");
