...
/// XML constructor.
/// It creates a instances object by loading the object members from a TinyXML
/// document.
...
/// Assignment operator.
/// It assigns to the current object the members of an existing instances
/// object.
...
/// Returns a value of the splitting method enumeration from a string with that
/// method.
/// @param splitting_method String with the method for splitting the instances.

Instances::SplittingMethod Instances::get_splitting_method(
    const std::string& splitting_method) {
...
/// Returns the use of every instance (training, generalization, testing) in a
/// vector.
...
/// Returns the number of instances in the data set which will be used for
/// training.
...
const bool& Instances::get_display(void) const { return (display); }
...
/// @param instances_document Pointer to a TinyXML document containing the
/// member data.
...
void Instances::set_default(void) { display = true; }
...
/// Possible values for the elements are "Training", "Generalization", "Testing"
/// and "Unused".
/// The size must be equal to the number of instances.
...
/// @param i Index of instance.
/// @param new_use String with the use name ("Training", "Generalization",
/// "Testing" or "Unused")
...
void Instances::set_display(const bool& new_display) { display = new_display; }
...
void Instances::from_XML(const tinyxml2::XMLDocument& instances_document) {
  ...
  const tinyxml2::XMLElement* instances_number_element =
      instances_element->FirstChildElement("InstancesNumber");
  ...
  for (unsigned i = 0; i < instances_number; i++) {
    const tinyxml2::XMLElement* item_element =
        start_element->NextSiblingElement("Item");
    ...
    const tinyxml2::XMLElement* use_element =
        item_element->FirstChildElement("Use");
...
/// @param generalization_instances_ratio Ratio of generalization instances in
/// the data set.
...
void Instances::split_random_indices(
  ...
  const double total_ratio =
      training_instances_ratio + generalization_instances_ratio +
      testing_instances_ratio;

  // Get number of instances for training, generalization and testing

  const unsigned generalization_instances_number = (unsigned)(
      generalization_instances_ratio * used_instances_number / total_ratio);
  const unsigned testing_instances_number =
      (unsigned)(testing_instances_ratio * used_instances_number / total_ratio);
  const unsigned training_instances_number =
      used_instances_number - generalization_instances_number -
      testing_instances_number;

  const unsigned sum_instances_number =
      training_instances_number + generalization_instances_number +
      testing_instances_number;
  ...
  if (sum_instances_number != used_instances_number) {
    ...
    buffer << "OpenNN Warning: Instances class.\n"
           << "void split_random_indices(const double&, const double&, const "
              "double&) method.\n"
           << "Sum of numbers of training, generalization and testing "
              "instances is not equal to number of used instances.\n";
...
/// Creates new training, generalization and testing indices with sequential
/// indices.
/// @param training_instances_ratio Ratio of training instances in the data set.
/// @param generalization_instances_ratio Ratio of generalization instances in
/// the data set.
...
void Instances::split_sequential_indices(
    const double& training_instances_ratio,
    const double& generalization_instances_ratio,
    const double& testing_instances_ratio) {
  const unsigned used_instances_number = count_used_instances_number();
...
// void split_instances(const SplittingMethod& splitting_method = Random, const
// double& training_ratio = 0.6, const double& generalization_ratio = 0.2, const
// double& testing_ratio = 0.2) method

/// Divides the instances into training, generalization and testing subsets.
/// @param splitting_method Instances splitting method (Sequential or Random,
/// Random by default).
/// @param training_ratio Ratio of training instances (0.6 by default).
/// @param generalization_ratio Ratio of generalization instances (0.2 by
/// default).
/// @param testing_ratio Ratio of testing instances (0.2 by default).

void Instances::split_instances(const SplittingMethod& splitting_method,
                                const double& training_ratio,
                                const double& generalization_ratio,
                                const double& testing_ratio) {
  ...
  if (training_ratio < 0) {
    buffer << "OpenNN Exception: Instances class.\n"
           << "void split_instances(const SplittingMethod&, const double&, "
              "const double&, const double&) method.\n"
  ...
  if (training_ratio == 0.0 && generalization_ratio == 0.0 &&
      testing_ratio == 0.0) {
    buffer << "OpenNN Exception: Instances class.\n"
           << "void split_instances(const SplittingMethod&, const double&, "
              "const double&, const double&) method.\n"
  ...
  switch (splitting_method) {
    case Instances::Sequential: {
      split_sequential_indices(training_ratio, generalization_ratio,
                               testing_ratio);
    } break;
    ...
    default: {
      ...
      buffer << "Neural Engine Exception: Instances class.\n"
             << "void split_instances(const double&, const double&, const "
                "double&) method.\n"
