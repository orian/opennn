...
///
/// This class represents the concept of a mathematical model defined by
...
class OrdinaryDifferentialEquations : public MathematicalModel {
 ...
 public:
  ...
  OrdinaryDifferentialEquations& operator=(
  ...
  /// Enumeration of available methods for numerical integration of ordinary
  ...
  /// This pure virtual method calculates the derivatives of the dependent
  /// variables with respect to the independent variable.

  virtual Vector<double> calculate_dependent_variables_dots(
      const NeuralNetwork&, const Vector<double>&) const = 0;

  // Numerical solution methods

  Matrix<double> calculate_Runge_Kutta_solution(const NeuralNetwork&) const;
  Vector<double> calculate_Runge_Kutta_final_solution(
      const NeuralNetwork&) const;

  Matrix<double> calculate_Runge_Kutta_Fehlberg_solution(
      const NeuralNetwork&) const;
  Vector<double> calculate_Runge_Kutta_Fehlberg_final_solution(
      const NeuralNetwork&) const;
 ...
 protected:
  ...
  /// Number of points at which the the Runge-Kutta-Fehlberg method displays a
  /// warning message.

  unsigned warning_size;

  /// Number of points at which the the Runge-Kutta-Fehlberg method throws an
  /// exception.
