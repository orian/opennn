...
// const NumericalIntegrationMethod& get_numerical_integration_method(void)
// const method

/// Returns the method used for numerical integration (trapezoid method or
/// Simpson's method).

const NumericalIntegration::NumericalIntegrationMethod&
NumericalIntegration::get_numerical_integration_method(void) const {
...
std::string NumericalIntegration::write_numerical_integration_method(
    void) const {
...
const bool& NumericalIntegration::get_display(void) const { return (display); }

// void set(const NumericalIntegration&) method

/// Sets the members of this object to be equal to those of another object.
/// @param other_numerical_integration Numerical integration object to be
/// copied.

void NumericalIntegration::set(
    const NumericalIntegration& other_numerical_integration) {
  numerical_integration_method =
      other_numerical_integration.numerical_integration_method;
...
/// Sets the method to be used for numerical integration (trapezoid method or
/// Simpson's method).
...
/// @param new_numerical_integration_method Numerical integration method name
/// string.

void NumericalIntegration::set_numerical_integration_method(
    const std::string& new_numerical_integration_method) {
...
// double calculate_trapezoid_integral(const Vector<double>&, const
// Vector<double>&) const method

/// This method evaluates the integral of a function given as a set of n pairs
/// of data (x,y) using the composite
/// trapezoid rule.
/// @param x Vector of x data.
/// @param y Vector of y data.

double NumericalIntegration::calculate_trapezoid_integral(
...
// double calculate_Simpson_integral(const Vector<double>&, const
// Vector<double>&) const method
...
double NumericalIntegration::calculate_Simpson_integral(
    const Vector<double>& x, const Vector<double>& y) const {
  ...
  if (n % 2 != 0) {
    ...
    for (unsigned i = 0; i < m; i++) {
      ...
      wa =
          (c - a) / ((a - b) * (a - c)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                                           0.5 * (a + c) * (b + c) + b * c);
      wb =
          (c - a) / ((b - a) * (b - c)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                                           0.5 * (a + c) * (a + c) + a * c);
      wc =
          (c - a) / ((c - a) * (c - b)) * (1.0 / 3.0 * (a * a + c * c + a * c) -
                                           0.5 * (a + c) * (a + b) + a * b);
...
double NumericalIntegration::calculate_integral(const Vector<double>& x,
  ...
  switch (numerical_integration_method) {
    ...
    default: {
      ...
      buffer << "OpenNN Exception: NumericalIntegration class.\n"
             << "double calculate_integral(const Vector<double>&, const "
                "Vector<double>&) const method.\n"
             << "Unknown numerical integration method.\n";
...
tinyxml2::XMLDocument* NumericalIntegration::to_XML(void) const {
  ...
  tinyxml2::XMLElement* root_element =
      document->NewElement("NumericalIntegration");
...
void NumericalIntegration::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("NumericalIntegration");
  ...
  // Numerical integration method
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("NumericalIntegrationMethod");
