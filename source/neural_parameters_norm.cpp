...
/// Default constructor.
/// It creates a neural parameters norm functional not associated to any neural
/// network.
/// It also initializes all the rest of class members to their default values.

NeuralParametersNorm::NeuralParametersNorm(void) : PerformanceTerm() {
  set_default();
}
...
/// Neural network constructor.
/// It creates a neural parameters norm functional associated to a neural
/// network.
/// It also initializes all the rest of class members to their default values.
/// @param new_neural_network_pointer Pointer to a neural network object.

NeuralParametersNorm::NeuralParametersNorm(
...
/// XML constructor.
/// It creates a neural parameters norm object not associated to any neural
/// network.
...
/// OpenNN manual.
/// @param neural_parameters_norm_document TinyXML document with the neural
/// parameters norm elements.
...
/// Returns the weight value for the neural parameters norm in the performance
/// term expression.

const double& NeuralParametersNorm::get_neural_parameters_norm_weight(
    void) const {
...
/// Sets a new weight value for the neural parameters norm in the performance
/// term expression.

void NeuralParametersNorm::set_neural_parameters_norm_weight(
    const double& new_neural_parameters_norm_weight) {
...
/// It is equal to the weighted norm of the parameters from the associated
/// neural network.
...
double NeuralParametersNorm::calculate_performance(void) const {
  ...
  const Vector<double> neural_parameters =
...
/// Calculates the objective gradient by means of the back-propagation
/// algorithm,
...
// Control sentence (if debug)
  ...
  return (neural_parameters.calculate_norm_gradient() *
...
/// and returns it in a single symmetric matrix of size the number of multilayer
...
// Control sentence (if debug)
  ...
  return (neural_parameters.calculate_norm_Hessian() *
          neural_parameters_norm_weight);
}

// double calculate_performance(const Vector<double>&) method

/// Returns the neural parameters norm value of a neural network for a vector of
/// parameters.
...
double NeuralParametersNorm::calculate_performance(
...
Vector<double> NeuralParametersNorm::calculate_gradient(
    const Vector<double>& parameters) const {
// Control sentence (if debug)
  ...
  if (neural_network_pointer->has_independent_parameters()) {
    ...
    return (parameters.calculate_norm_gradient() *
            neural_parameters_norm_weight);
  }
}
...
Matrix<double> NeuralParametersNorm::calculate_Hessian(
    const Vector<double>& parameters) const {
// Control sentence (if debug)
  ...
  if (neural_network_pointer->has_independent_parameters()) {
    ...
    return (parameters.calculate_norm_Hessian() *
            neural_parameters_norm_weight);
  }
}
...
/// Returns a string with the name of the neural parameters norm performance
/// type,
...
tinyxml2::XMLDocument* NeuralParametersNorm::to_XML(void) const {
  ...
  tinyxml2::XMLElement* neural_network_parameters_norm_element =
      document->NewElement("NeuralParametersNorm");

  document->InsertFirstChild(neural_network_parameters_norm_element);

  // Neural parameters norm weight
  {
    tinyxml2::XMLElement* weight_element =
        document->NewElement("NeuralParametersNormWeight");
...
void NeuralParametersNorm::from_XML(const tinyxml2::XMLDocument& document) {
  const tinyxml2::XMLElement* root_element =
      document.FirstChildElement("NeuralParametersNorm");
  ...
  // Neural parameters norm weight
  {
    const tinyxml2::XMLElement* element =
        root_element->FirstChildElement("NeuralParametersNormWeight");
