...
ModelSelection::ModelSelection(void) : training_strategy_pointer(NULL) {
  set_default();
}
...
/// XML constructor.
/// @param model_selection_document Pointer to a TinyXML document containing the
/// model selection data.

ModelSelection::ModelSelection(
    const tinyxml2::XMLDocument& model_selection_document)
    : training_strategy_pointer(NULL) {
...
/// Returns the complexities of the neural networks given by the numbers of
/// hidden perceptrons.

const Vector<unsigned>& ModelSelection::get_hidden_perceptrons_numbers(
    void) const {
...
/// Returns true if the neural network parameters are to be reserved, and false
/// otherwise.
...
/// Returns true if the performance functional performances are to be reserved,
/// and false otherwise.
...
/// Returns true if the performance functional generalization performances are
/// to be reserved, and false otherwise.
...
/// Returns true if the parameters vector of the neural network with minimum
/// performance functional performance is to be reserved, and false otherwise.
...
/// Returns true if the statistics of the performance data are to be inclued in
/// the model selection results.
...
/// Returns true if the statistics of the generalization performance data are to
/// be inclued in the model selection results.

const bool&
ModelSelection::get_reserve_generalization_performance_data_statistics(void) {
  return (reserve_generalization_performance_data_statistics);
}

// const bool& get_reserve_model_order_selection_plot(void) method

/// Returns true if the necessary data for plotting the model selection results
...
const bool& ModelSelection::get_display(void) const { return (display); }
...
void ModelSelection::set_training_strategy_pointer(
    TrainingStrategy* new_training_strategy_pointer) {
  training_strategy_pointer = new_training_strategy_pointer;
}
...
/// Sets the number of complexities to be compared in the model order selection
/// process.
/// @param new_hidden_perceptrons_numbers Vector with different hidden layers
/// sizes.

void ModelSelection::set_hidden_perceptrons_numbers(
    const Vector<unsigned>& new_hidden_perceptrons_numbers) {
  hidden_perceptrons_numbers = new_hidden_perceptrons_numbers;
}

// void set_parameters_assays_number(unsigned) method

/// Sets the number of times that each different neural network is to be
/// trained.
/// @param new_parameters_assays_number Number of assays for each set of
/// parameters.

void ModelSelection::set_parameters_assays_number(
    const unsigned& new_parameters_assays_number) {
...
/// @param new_parameters_assays_number Number of trainings for each different
/// neural network.

void ModelSelection::set_assays_numbers(
    const unsigned& new_complexity_assays_number,
    const unsigned& new_parameters_assays_number) {
// Control sentence (if debug)
  ...
  if (training_strategy_pointer->get_performance_functional_pointer()
  ...
  // training_strategy_pointer->get_performance_functional_pointer()->get_multilayer_perceptron_pointer()->get_inputs_number();
  //  const unsigned outputs_number =
  // training_strategy_pointer->get_performance_functional_pointer()->get_multilayer_perceptron_pointer()->get_outputs_number();

  //  const unsigned minimum_hidden_layer_size =
  // (int)sqrt((double)(inputs_number*inputs_number) +
  // (double)(outputs_number*outputs_number));
  //  const unsigned maximum_hidden_layer_size =
  // 2*(int)sqrt((double)(inputs_number*inputs_number) +
  // (double)(outputs_number*outputs_number));

  const unsigned minimum_hidden_layer_size = 1;
  const unsigned maximum_hidden_layer_size = 12;

  int hidden_layer_size_increment =
      (maximum_hidden_layer_size - minimum_hidden_layer_size) /
      new_complexity_assays_number;
  ...
  for (unsigned i = 0; i < new_complexity_assays_number; i++) {
    hidden_perceptrons_numbers[i] =
        minimum_hidden_layer_size + hidden_layer_size_increment * i;
  }
...
void ModelSelection::set_reserve_parameters_data(
    const bool& new_reserve_parameters_data) {
...
void ModelSelection::set_reserve_performance_data(
    const bool& new_reserve_performance_data) {
...
void ModelSelection::set_reserve_generalization_performance_data(
    const bool& new_reserve_generalization_performance_data) {
  reserve_generalization_performance_data =
      new_reserve_generalization_performance_data;
}
...
void ModelSelection::set_reserve_minimal_parameters(
    const bool& new_reserve_minimal_parameters) {
...
void ModelSelection::set_reserve_performance_data_statistics(
    const bool& new_reserve_performance_data_statistics) {
...
void ModelSelection::set_reserve_generalization_performance_data_statistics(
    const bool& new_reserve_generalization_performance_data_statistics) {
  reserve_generalization_performance_data_statistics =
      new_reserve_generalization_performance_data_statistics;
}
...
void ModelSelection::set_reserve_model_order_selection_plot(
    const bool& new_reserve_model_order_selection_plot) {
...
/// Checks that the different pointers needed for performing the model selection
/// are not NULL.
...
void ModelSelection::check(void) const {
  ...
  const PerformanceFunctional* performance_functional_pointer =
      training_strategy_pointer->get_performance_functional_pointer();
...
ModelSelection::ModelSelectionResults
ModelSelection::perform_model_inputs_selection(void) const {
  std::ostringstream buffer;
  ...
  multilayer_perceptron_pointer->get_layers_number();
     ...
     if(layers_number != 2) 
     {
       std::ostringstream buffer;
        ...
        buffer << "OpenNN Exception: ModelSelection class.\n"
               << "ModelSelectionResults perform_model_inputs_selection(void)
  method.\n"
     ...
     model_selection_results.parameters_data.set(inputs_assays_number,
  parameters_assays_number);
     model_selection_results.performance_data.set(inputs_assays_number,
  parameters_assays_number);
     model_selection_results.generalization_performance_data.set(inputs_assays_number,
  parameters_assays_number);
     ...
     // Model order selection
        ...
        model_selection_results.parameters_data[i][j] =
  multilayer_perceptron_pointer->arrange_parameters();
  
        model_selection_results.performance_data[i][j] =
  training_strategy_pointer->get_performance_functional_pointer()->calculate_performance();
        model_selection_results.generalization_performance_data[i][j] =
  training_strategy_pointer->get_performance_functional_pointer()->calculate_generalization_performance();			
  ...
  model_selection_results.generalization_performance_data[i][j] << "\n"
     ...
     const Vector<unsigned> minimal_indices =
  model_selection_results.generalization_performance_data.calculate_minimal_indices();
  
     const Vector<double> minimal_parameters =
  model_selection_results.parameters_data[minimal_indices
  [0]][minimal_indices[1]];
...
ModelSelection::ModelSelectionResults
ModelSelection::perform_model_order_selection(void) const {
  std::ostringstream buffer;
  ...
  performance_functional_pointer->get_objective_pointer()->get_data_set_pointer();
  ...
  multilayer_perceptron_pointer->get_layers_number();
     ...
     if(layers_number != 2) 
     {
       std::ostringstream buffer;
        ...
        buffer << "OpenNN Exception: ModelSelection class.\n"
               << "ModelSelectionResults perform_model_order_selection(void)
  method.\n"
               << "Number of layers in multilayer perceptron must be two.\n";
     ...
     const unsigned complexity_assays_number =
  hidden_perceptrons_numbers.size();
  
     ModelSelectionResults model_order_selection_results;
  
     if(reserve_parameters_data)
     {
        model_order_selection_results.parameters_data.set(complexity_assays_number,
  parameters_assays_number);
     }  
    
     if(reserve_performance_data)
     {
        model_order_selection_results.performance_data.set(complexity_assays_number,
  parameters_assays_number);
     }
  
     if(reserve_generalization_performance_data)
     {
        model_order_selection_results.generalization_performance_data.set(complexity_assays_number,
  parameters_assays_number);
     }
  ...
  //	  variables_pointer->set_inputs_indices(inputs_indices[i]);
        ...
        //multilayer_perceptron_pointer->set_layer_perceptrons_number(0,
  hidden_perceptrons_numbers[i]);
     ...
     // Model order selection
              ...
              if(reserve_parameters_data)
              {
           model_order_selection_results.parameters_data[i][j] =
  multilayer_perceptron_pointer->arrange_parameters();
     }
  
              if(reserve_parameters_data)
              {
           model_order_selection_results.performance_data[i][j] =
  training_strategy_pointer->get_performance_functional_pointer()->calculate_performance();
     }
  
              if(reserve_parameters_data)
              {
           model_order_selection_results.generalization_performance_data[i][j] =
  training_strategy_pointer->get_performance_functional_pointer()->calculate_generalization_performance();			
     }
  ...
  model_order_selection_results.generalization_performance_data[i][j] << "\n"
     ...
     const Vector<unsigned> minimal_indices =
  model_order_selection_results.generalization_performance_data.calculate_minimal_indices();
  
     const Vector<double> minimal_parameters =
  model_order_selection_results.parameters_data[minimal_indices
  [0]][minimal_indices[1]];
  ...
  // Vector< Vector<double> > calculate_performance_data_statistics(const
  ModelSelectionResults&) const method
  //
  return(model_order_selection_data.performance_data.calculate_transpose().calculate_statistics());

  
  // Vector< Vector<double> >
  calculate_generalization_performance_data_statistics(const
  ModelSelectionResults&) const method
     //
  ...
  //	  {
  //	     file <<
  model_order_selection_data.generalization_performance_data[i][j] << " ";
  //	  }
...
ModelSelection::ModelSelectionResults ModelSelection::perform_model_selection(
    void) const {
  std::ostringstream buffer;
...
/// Serializes the model selection object into a XML document of the TinyXML
/// library.
...
tinyxml2::XMLDocument* ModelSelection::to_XML(void) const {
  ...
  tinyxml2::XMLElement* model_selection_element =
      document->NewElement("ModelSelection");

  document->InsertFirstChild(model_selection_element);

  // Hidden layer sizes

  tinyxml2::XMLElement* hidden_perceptrons_numbers_element =
      document->NewElement("HiddenLayerSizes");
  model_selection_element->LinkEndChild(hidden_perceptrons_numbers_element);

  buffer.str("");
  buffer << hidden_perceptrons_numbers;

  tinyxml2::XMLText* hidden_perceptrons_numbers_text =
      document->NewText(buffer.str().c_str());
  hidden_perceptrons_numbers_element->LinkEndChild(
      hidden_perceptrons_numbers_text);

  // ParametersAssaysNumber

  tinyxml2::XMLElement* parameters_assays_number_element =
      document->NewElement("ParametersAssaysNumber");
  model_selection_element->LinkEndChild(parameters_assays_number_element);

  buffer.str("");
  buffer << parameters_assays_number;

  tinyxml2::XMLText* parameters_assays_number_text =
      document->NewText(buffer.str().c_str());
...
void ModelSelection::load(const std::string& file_name) {
  ...
  tinyxml2::XMLElement* model_selection_element =
      document->FirstChildElement("ModelSelection");
  ...
  tinyxml2::XMLElement* hidden_perceptrons_numbers_element =
      model_selection_element->FirstChildElement("HiddenLayerSizes");

  if (hidden_perceptrons_numbers_element) {
    Vector<unsigned> new_hidden_perceptrons_numbers;
    new_hidden_perceptrons_numbers.parse(
        hidden_perceptrons_numbers_element->GetText());
  ...
  tinyxml2::XMLElement* parameters_assays_number_element =
      model_selection_element->FirstChildElement("ParametersAssaysNumber");

  if (parameters_assays_number_element) {
    parameters_assays_number =
        atoi(parameters_assays_number_element->GetText());
  }
}
}
