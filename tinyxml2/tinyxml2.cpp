...
XMLError XMLDocument::LoadFile( const char* file_name )
{
    Clear();
    FILE* fp = 0;
    ...
    if ( !fp) {
#endif
        SetError( XML_ERROR_FILE_NOT_FOUND, file_name, 0 );
        return _errorID;
    }
    LoadFile( fp );
    fclose( fp );
...
XMLError XMLDocument::SaveFile( const char* file_name, bool compact )
{
    FILE* fp = 0;
#if defined(_MSC_VER) && (_MSC_VER >= 1400 )
    ...
    if ( !fp) {
#endif
        SetError( XML_ERROR_FILE_COULD_NOT_BE_OPENED, file_name, 0 );
        return _errorID;
    }
    SaveFile(fp, compact);
    fclose( fp );
...
void tinyxml2::DeepClone(XMLNode* newNode, const XMLNode* nodeSrc, XMLDocument *DocDest, const XMLNode*)
{
